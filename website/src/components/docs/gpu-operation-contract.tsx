...
export const GPUGRAPH_OPERATION_CONTRACTS = {
  ...
  'gpu-ancestor-projection': {
    ...
    readsWrites: 'Reads parents and a visibility mask; writes projected ancestor IDs and validation status.',
    ownership: COMMON.callerOwned,
    output: 'One source-aligned projected identity per node, plus bounded cycle/depth diagnostics.',
    work: 'Bounded pointer jumping over the configured maximum hierarchy depth.',
  ...
  'gpu-transpose': {
    ...
    work: 'One bounded tiled dispatch over the source extent.',
  ...
  'gpu-convolution': {
    ...
    readsWrites: 'Reads packed input and kernel views; writes a separate packed output view.',
    ownership: COMMON.callerOwned,
    output: 'One same-size convolved field with explicit zero or wrap boundaries.',
    work: 'Uses one direct pass or an explicit FFT pipeline selected from the configured strategy.',
    chunks: 'Requires one packed 2D domain.',
    execution: COMMON.noSubmission,
