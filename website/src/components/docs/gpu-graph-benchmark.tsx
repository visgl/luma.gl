  ...
  return (
    <div>
      ...
      <div style={{display: 'flex', flexWrap: 'wrap', gap: 18, marginBottom: 12}}>
        <label
          htmlFor="gpu-graph-benchmark-dataset"
          style={{alignItems: 'center', display: 'flex', gap: 8}}
        >
          Graph dataset
          <select
            id="gpu-graph-benchmark-dataset"
            onChange={event =>
              setDatasetKind(event.target.value as GPUGraphBenchmarkDatasetKind)
            }
            value={datasetKind}
          >
        ...
        <label
          htmlFor="gpu-graph-benchmark-vertex-count"
          style={{alignItems: 'center', display: 'flex', gap: 8}}
        >
          Vertices
          <select
            id="gpu-graph-benchmark-vertex-count"
...
function GPUGraphBenchmarkResults({
  report,
  deviceLabel
}: {
...
function GPUGraphBenchmarkRow({
  path,
  showTimestamp
}: {
  path: GPUGraphBenchmarkPathReport;
...
function formatAlgorithm(algorithm: GPUGraphBenchmarkAlgorithm): string {
  const labels: Record<GPUGraphBenchmarkAlgorithm, string> = {
...
function formatConvergence(path: GPUGraphBenchmarkPathReport): string {
  if (
