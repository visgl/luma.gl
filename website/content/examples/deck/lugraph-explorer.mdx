...
A real deck.gl `Effect` schedules luGraph work inside the existing deck.gl frame. Custom node and
edge `Layer` implementations consume its results directly, and an `OrthographicView` provides
familiar pan and zoom. Graph analytics remain in `@luma.gl/experimental/lugraph`; the optional
deck.gl integration stays entirely inside this unpackaged example.
