...
import {parseDataUriToBuffer} from 'loaders.gl/common/loader-utils/load-uri.js';

test('parseDataUriToBuffer', t => {
  let obj;
  let buf;

  obj = parseDataUriToBuffer('data:text/html;base64,PGh0bWw+');
  t.equals(obj.mimeType, 'text/html', 'should record down correct MIME type');

  obj = parseDataUriToBuffer('data:text/plain;base64,SSBsb3ZlIHlvdSE');
  buf = obj.buffer;
  ...
  obj = parseDataUriToBuffer('data:text/plain,important content!');
  buf = obj.buffer;
  t.ok(Buffer.isBuffer(buf));
  t.equals(buf.toString(), 'important content!', 'should work with plain data URIs');

  obj = parseDataUriToBuffer('data:,important content!');
  ...
  obj = parseDataUriToBuffer('data:;charset=utf-8,important content!');
  t.equals(
    obj.mimeType,
