  ...
  [true, false].forEach(isSubCopy => {
    [true, false].forEach(sourceIsFramebuffer => {
      ...
      const destinationTexture = new Texture2D(gl, {
        ...
        width: 2,
        height: 2,
        // When perfomring sub copy, texture memory is not allcoated by 'copyFramebufferToTexture', allocate it here
      ...
      const color = copyFramebufferToArray({framebuffer: destinationTexture});
...
test('WebGL#copyFramebufferToTexture', t => {
  const {gl} = fixture;
  testCopyFramebufferToTexture(t, gl);
});

test('WebGL2#copyFramebufferToTexture', t => {
  const {gl2} = fixture;
  if (gl2) {
