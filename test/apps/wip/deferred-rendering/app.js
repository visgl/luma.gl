...
function height(x, z) {
  if (heightMemo[[x, z]] === undefined) {
    heightMemo[[x, z]] =
      ...
      0.5 * noise.perlin2(x, z) +
      0.25 * noise.perlin2(x * 2, z * 2);
  }
...
function generateTerrain(size, resolution) {
  ...
  for (var i = 0; i < resolution; i++) {
    for (var j = 0; j < resolution; j++) {
      ...
      positions.push([x0, y0, z0, x2, y2, z2, x1, y1, z1, x0, y0, z0, x3, y3, z3, x2, y2, z2]);
      normals.push([
        n0.x,
        n0.y,
        n0.z,
...
window.onload = function() {
  ...
  function render() {
    ...
    pLights.setBuffers(quadBuffers).setUniforms({
      uView: view,
      uProjection: projection,
