...
class Mesh {
  ...
  initBuffers(gl, gltf) {
    ...
    if (
      !this.initArrayBuffer(
        ...
        gl.FLOAT,
        'a_Position',
        this.verticesAccessor.byteStride,
        this.verticesAccessor.byteOffset
      )
    ) {
    ...
    if (this.normalsAccessor) {
      if (
        !this.initArrayBuffer(
          ...
          gl.FLOAT,
          'a_Normal',
          this.normalsAccessor.byteStride,
    ...
    if (this.tangentsAccessor) {
      if (
        !this.initArrayBuffer(
          ...
          gl.FLOAT,
          'a_Tangent',
          this.tangentsAccessor.byteStride,
          this.tangentsAccessor.byteOffset
        )
      ) {
    ...
    if (this.texcoordsAccessor) {
      if (
        !this.initArrayBuffer(
          ...
          gl.FLOAT,
          'a_UV',
          this.texcoordsAccessor.byteStride,
          this.texcoordsAccessor.byteOffset
        )
      ) {
  ...
  initTextures(gl, gltf) {
    ...
    // Base Color
    var baseColorFactor =
      pbrMat && pbrMat.baseColorFactor ? pbrMat.baseColorFactor : [1.0, 1.0, 1.0, 1.0];
    ...
    if (pbrMat && pbrMat.baseColorTexture && gltf.textures.length > pbrMat.baseColorTexture.index) {
      imageInfos['baseColor'] = this.getImageInfo(
        gl,
        gltf,
        pbrMat.baseColorTexture.index,
        'uniform1i',
        'u_BaseColorSampler',
        this.hasSRGBExt ? this.hasSRGBExt.SRGB_EXT : gl.RGBA
      );
    ...
    if (
      ...
      gltf.textures.length > pbrMat.metallicRoughnessTexture.index
    ) {
      imageInfos['metalRoughness'] = this.getImageInfo(
        gl,
        gltf,
        pbrMat.metallicRoughnessTexture.index,
        'uniform1i',
    ...
    if (
      ...
      gltf.textures.length > this.material.normalTexture.index
    ) {
      imageInfos['normal'] = this.getImageInfo(
        gl,
        gltf,
        this.material.normalTexture.index,
        'uniform1i',
        'u_NormalSampler',
        gl.RGBA
      );
    ...
    imageInfos['brdfLUT'] = {
      uri: brdfLUT,
      samplerIndex: samplerIndex,
      colorSpace: this.hasSRGBExt ? this.hasSRGBExt.SRGB_EXT : gl.RGBA,
      clamp: true
    };
    ...
    if (this.material && this.material.emissiveTexture) {
      imageInfos['emissive'] = this.getImageInfo(
        gl,
        gltf,
        this.material.emissiveTexture.index,
        'uniform1i',
        'u_EmissiveSampler',
        this.hasSRGBExt ? this.hasSRGBExt.SRGB_EXT : gl.RGBA
      );
      this.defines.HAS_EMISSIVEMAP = 1;
      var emissiveFactor = this.material.emissiveFactor
        ? this.material.emissiveFactor
        : [0.0, 0.0, 0.0];
    ...
    if (this.material && this.material.occlusionTexture) {
      imageInfos['occlusion'] = this.getImageInfo(
        gl,
        gltf,
        this.material.occlusionTexture.index,
        'uniform1i',
        'u_OcclusionSampler',
        gl.RGBA
      );
      var occlusionStrength = this.material.occlusionTexture.strength
        ? this.material.occlusionTexture.strength
        : 1.0;
      this.localState.uniforms['u_OcclusionStrength'] = {
        funcName: 'uniform1f',
        vals: [occlusionStrength]
      };
