...
import {runGPUDataFrameBenchmark} from '../../examples/experimental/gpu-data-analysis/src/gpu-dataframe-benchmark';
...
describe('Arrow-driven luDF dataframe example', () => {
  test('validates real filtering, grouping, stable sorting, and joins against equivalent CPU work', async () => {
    ...
    const result = await runGPUDataFrameBenchmark(device, {
      rowCount: 128,
      iterations: 2,
      warmupIterations: 1
    });
  ...
  test('runs the real WebGPU benchmark only after an explicit interactive request', async () => {
    ...
    try {
      ...
      const button = getRequiredElement<HTMLButtonElement>(
        root,
        '#analysis-gpu-dataframe-benchmark-run'
      );
      const benchmarkRows = getRequiredElement<HTMLSelectElement>(
        root,
        '[data-gpu-dataframe-benchmark-rows]'
      );
      const benchmarkIterations = getRequiredElement<HTMLSelectElement>(
        root,
        '[data-gpu-dataframe-benchmark-iterations]'
      );
      const status = getRequiredElement<HTMLElement>(
        root,
        '#analysis-gpu-dataframe-benchmark-status'
      );
      const results = getRequiredElement<HTMLElement>(
        root,
        '#analysis-gpu-dataframe-benchmark-results'
      );

      expect(button.matches('[data-gpu-dataframe-benchmark]')).toBe(true);
      expect(status.matches('[data-gpu-dataframe-benchmark-status]')).toBe(true);
      expect(results.matches('[data-gpu-dataframe-benchmark-phases]')).toBe(true);
      expect(results.dataset.state).toBe('idle');
      expect(results.dataset.validated).toBe('false');
      expect(results.querySelectorAll('[data-gpu-dataframe-phase]')).toHaveLength(0);
      ...
      for (const phase of BENCHMARK_PHASES) {
        const row = getRequiredElement<HTMLElement>(
          results,
          `[data-gpu-dataframe-phase="${phase}"]`
        );
      ...
      expect(results.querySelectorAll('[data-gpu-dataframe-workload]')).toHaveLength(4);
      expect(results.querySelector('[data-gpu-dataframe-crossover]')?.textContent).toMatch(
        /crossover/i
      );
    } finally {
      example?.destroy();
  ...
  test('keeps the opt-in benchmark disabled when WebGPU initialization fails', async () => {
    ...
    try {
      ...
      expect(
        getRequiredElement<HTMLButtonElement>(root, '#analysis-gpu-dataframe-benchmark-run')
          .disabled
      ).toBe(true);
      expect(
        getRequiredElement<HTMLElement>(root, '#analysis-gpu-dataframe-benchmark-status')
          .textContent
      ).toMatch(/WebGPU is unavailable/i);
      expect(
        getRequiredElement<HTMLElement>(root, '#analysis-gpu-dataframe-benchmark-results').dataset
          .state
      ).toBe('idle');
    } finally {
      example?.destroy();
