...
import {
  createGPUDataFrameBenchmarkReference,
  summarizeGPUDataFrameBenchmarkSamples
} from '../../examples/experimental/gpu-data-analysis/src/gpu-dataframe-benchmark';
...
describe('million-row luDF benchmark measurements', () => {
  test('materializes adjusted fares for every row of all four CPU workloads', () => {
    ...
    try {
      const reference = createGPUDataFrameBenchmarkReference(
        {
  ...
  test('reports median CPU/GPU durations, observed throughput, and honest speedup', () => {
    expect(summarizeGPUDataFrameBenchmarkSamples(1_000_000, [24, 8, 12], [10, 4, 6])).toEqual({
      cpuMilliseconds: 12,
      gpuMilliseconds: 6,
  ...
  test('averages the middle observations for an even number of samples', () => {
    const result = summarizeGPUDataFrameBenchmarkSamples(4096, [14, 6, 18, 10], [9, 3, 7, 5]);
  ...
  test('does not invent throughput or speedup from zero-duration observations', () => {
    expect(summarizeGPUDataFrameBenchmarkSamples(1024, [0], [0])).toEqual({
      cpuMilliseconds: 0,
      gpuMilliseconds: 0,
      cpuRowsPerSecond: 0,
      gpuRowsPerSecond: 0,
  ...
  test('rejects invalid row counts and invalid timing observations', () => {
    expect(() => summarizeGPUDataFrameBenchmarkSamples(0, [1], [1])).toThrow(/row count/i);
    expect(() => summarizeGPUDataFrameBenchmarkSamples(1024, [], [1])).toThrow(/samples/i);
    expect(() => summarizeGPUDataFrameBenchmarkSamples(1024, [1], [Number.NaN])).toThrow(
      /samples/i
    );
    expect(() => summarizeGPUDataFrameBenchmarkSamples(1024, [-1], [1])).toThrow(/samples/i);
  });
});
