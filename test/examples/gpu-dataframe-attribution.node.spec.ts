...
const GPU_DATAFRAME_SOURCE_DIRECTORY = new URL(
  '../../modules/experimental/src/gpu-dataframe/',
  import.meta.url
);
const GPU_DATAFRAME_DOCUMENTATION = new URL(
  '../../docs/api-reference/experimental/gpu-dataframe-operations.md',
  import.meta.url
);
const GPU_DATAFRAME_SOURCE_FILES = readdirSync(GPU_DATAFRAME_SOURCE_DIRECTORY)
  .filter(fileName => fileName.endsWith('.ts'))
  .sort();
const GPU_DATAFRAME_SPDX_HEADER = [
  '// luma.gl',
  '// SPDX-License-Identifier: MIT',
  '// SPDX-FileCopyrightText: Copyright (c) vis.gl contributors',
...
describe('luDF RAPIDS attribution and independent MIT licensing', () => {
  test('discovers every production TypeScript implementation', () => {
    expect(GPU_DATAFRAME_SOURCE_FILES.length).toBeGreaterThan(0);
    expect(GPU_DATAFRAME_SOURCE_FILES).toContain('index.ts');
    expect(GPU_DATAFRAME_SOURCE_FILES).toContain('gpu-data-frame.ts');
  });

  test.each(GPU_DATAFRAME_SOURCE_FILES)('preserves accurate SPDX attribution in %s', fileName => {
    const source = readFileSync(new URL(fileName, GPU_DATAFRAME_SOURCE_DIRECTORY), 'utf8');

    expect(source.startsWith(`${GPU_DATAFRAME_SPDX_HEADER}\n`)).toBe(true);
  ...
  test('documents the distinct upstream Apache and original vis.gl MIT licenses', () => {
    const documentation = readFileSync(GPU_DATAFRAME_DOCUMENTATION, 'utf8');
