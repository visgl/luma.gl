...
import WebGLDevice, {WebGLDeviceProps} from '../webgl-device';
import {FEATURES} from '../device/get-webgl-features';
...
export function hasFeatures(gl: WebGLRenderingContext, features: string | string[]): boolean {
  ...
  return featureArray.every((feature) => webglDevice.webglFeatures.has(feature));
}
