...
export type OrbitControllerProps = {
...
type ResolvedOrbitControllerProps = Required<OrbitControllerProps>;

const DEFAULT_PROPS: ResolvedOrbitControllerProps = {
  target: [0, 0, 0],
  distance: 10,
  yaw: 0,
  pitch: 0.25,
...
/**
 * Reusable pointer orbit and wheel zoom controller for canvas examples.
 *
 * Call {@link update} once per frame before reading {@link getEyePosition}. Manual dragging
 * temporarily pauses auto-rotation and resumes it from the new angle when the pointer is released.
 */
export class OrbitController {
  readonly canvas: HTMLCanvasElement;
  readonly props: ResolvedOrbitControllerProps;
  yaw: number;
  pitch: number;
  distance: number;
  ...
  constructor(canvas: HTMLCanvasElement, props: OrbitControllerProps = {}) {
    this.canvas = canvas;
