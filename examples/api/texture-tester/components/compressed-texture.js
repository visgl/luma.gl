...
export class CompressedTexture extends React.PureComponent {
    ...
    async renderTexturePreview(device) {
        ...
        try {
            ...
            const result = usesBasisLoader
                ? await loadWithHandledBasisRuntimeRejections(() => load(arrayBuffer, loader, options))
    ...
    renderCompressedTexture(device, model, images, loaderName, texturePath) {
        ...
        if ((typeof format === 'number' && !isCompressedImageFormatSupported(device, format)) ||
        ...
        const webgpuPreviewError = getCompressedTexturePreviewError(device, textureFormat, width, height);
    ...
    renderStats() {
        ...
        const additionalStats = stats.filter(stat => stat.name !== 'luma.gl Texture Format' &&
        ...
        return (React.createElement("ul", { style: {
            ...
            formatStat ? (React.createElement("li", { style: {
                    fontWeight: 700,
                    fontFamily: 'monospace',
                    marginBottom: 1
                } }, String(formatStat.value))) : null,
            dimensionsStat ? (React.createElement("li", null, `Dimensions: ${dimensionsStat.value}${dimensionsStat.units}`)) : null,
            uploadTimeStat ? (React.createElement("li", null, `Upload time: ${uploadTimeStat.value}${uploadTimeStat.units}`)) : null,
            React.createElement("li", null, `Mip levels: ${mipStats.length}`),
            React.createElement("li", { style: { width: '100%' } },
                React.createElement("table", { style: {
                    ...
                    React.createElement("tbody", null,
                        fileSizeStat ? (React.createElement("tr", null,
                            React.createElement("td", { style: { padding: '0 8px 1px 0', whiteSpace: 'nowrap', textAlign: 'left' } }, "File Size"),
                            React.createElement("td", { style: { padding: '0 0 1px 0', whiteSpace: 'nowrap', textAlign: 'right' } }, `${fileSizeStat.value}${fileSizeStat.units}`))) : null,
                        memorySizeStat ? (React.createElement("tr", null,
                            React.createElement("td", { style: { padding: '0 8px 1px 0', whiteSpace: 'nowrap', textAlign: 'left' } }, "Memory Size"),
                            React.createElement("td", { style: { padding: '0 0 1px 0', whiteSpace: 'nowrap', textAlign: 'right' } }, `${memorySizeStat.value}${memorySizeStat.units}`))) : null,
                        additionalStats.map(stat => (React.createElement("tr", { key: stat.name },
                            React.createElement("td", { colSpan: 2, style: {
                                    paddingTop: 1,
                                    whiteSpace: 'nowrap',
                                    textAlign: 'left'
                                } }, `${stat.name}: ${stat.value}${stat.units}`)))),
                        mipStats.map(stat => (React.createElement("tr", { key: stat.name },
    ...
    render() {
        ...
        return (React.createElement("button", { style: {
                ...
                padding: 0,
                overflow: 'hidden'
            }, onMouseEnter: () => this.setState({ showStats: true }), onMouseLeave: () => this.setState({ showStats: false }) },
            React.createElement("canvas", { ref: this.canvasRef, width: 256, height: 256, style: {
                    display: 'block',
                    width: '100%',
                    height: '100%',
                    opacity: isReady ? 1 : 0
                } }),
            !textureError ? (React.createElement(React.Fragment, null,
                React.createElement("div", { style: {
                    ...
                    textureFormatLabel ? (React.createElement("div", { style: {
                            color: 'white',
                            fontSize: 11,
                            lineHeight: 1.1,
                            fontFamily: 'monospace'
                        } }, textureFormatLabel)) : null))) : (React.createElement("div", { style: {
                    position: 'absolute',
                    inset: 0,
                    display: 'flex',
                    alignItems: 'center',
    ...
    renderTextureToPresentationContext(device, model, texture) {
        if (!this.isPresentationAvailable()) {
            return;
        }
    ...
    isPresentationAvailable() {
        const canvas = this.canvasRef.current;
        return Boolean(this.isComponentMounted &&
...
function getSupportedGPUTextureFormatFamilies(device) {
    const supportedFormats = new Set();
    for (const textureFormat of device.getSupportedCompressedTextureFormats()) {
...
function formatTextureByteSize(byteSize) {
...
function getCompressedTextureLevelStats(textureFormat, images) {
    const levels = images.map((image, mipLevel) => {
        const levelByteSize = image.levelSize ??
...
function getGPUTextureFormatFamily(textureFormat) {
    ...
    if (textureFormat.startsWith('bc1-') ||
...
async function loadWithHandledBasisRuntimeRejections(loadTexture) {
...
function isBasisRuntimeInitializationError(error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return (errorMessage.includes('RuntimeError: null function') ||
...
function resolveCompressedTextureFormat(image) {
    if (image.textureFormat) {
        return image.textureFormat;
    }
...
function getTextureFormatFromInternalFormat(format) {
    switch (format) {
        ...
        default:
            throw new Error(`No luma.gl texture format mapping for internal format ${format}. This loader result should provide textureFormat and the example should use that first.`);
    }
}
function isCompressedImageFormatSupported(device, format) {
    switch (format) {
        case COMPRESSED_RGB_S3TC_DXT1_EXT:
        case COMPRESSED_RGBA_S3TC_DXT1_EXT:
        case COMPRESSED_RGBA_S3TC_DXT3_EXT:
...
function getCompressedTextureUploadLevels(device, textureFormat, images) {
...
function getCompressedTexturePreviewError(device, textureFormat, width, height) {
    if (device.type !== 'webgpu') {
        return null;
    }
...
function canUploadCompressedTextureLevel(device, textureFormat, image) {
    if (device.type !== 'webgpu') {
        return true;
    }
...
function getDeviceLabel(device) {
    ...
    const normalizedArchitecture = gpuArchitecture && gpuArchitecture !== 'unknown' ? gpuArchitecture : '';
    const normalizedGpu = gpu && gpu !== 'unknown' ? gpu : '';
    const normalizedGpuType = gpuType && gpuType !== 'unknown' ? gpuType : '';
    const rendererChipMatch = renderer.match(/\b(Apple\s+[A-Za-z0-9]+|NVIDIA\s+[^,)]*|AMD\s+[^,)]*|Intel(?:\(R\))?\s+[^,)]*)\b/i);
    const shortRenderer = (rendererChipMatch?.[1] || renderer)
        ...
        .replace(/\s+/g, ' ')
        .trim();
    const compactDeviceLabel = shortRenderer ||
