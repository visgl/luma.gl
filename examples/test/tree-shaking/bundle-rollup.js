...
var luma_gl = require('luma.gl');

/* eslint-disable no-var, max-statements */
const SIDE = 256;

const animationFrame = new luma_gl.AnimationFrame()
.context(() => luma_gl.createGLContext({canvas: 'render-canvas'}))
.init(({gl}) => {
  gl.clearColor(1, 1, 1, 1);
  gl.clearDepth(1);
  gl.enable(luma_gl.GL.DEPTH_TEST);
  gl.depthFunc(luma_gl.GL.LEQUAL);

  return {cube: makeInstancedCube(gl)};
})
.frame(({gl, tick, aspect, cube}) => {
  gl.clear(luma_gl.GL.COLOR_BUFFER_BIT | luma_gl.GL.DEPTH_BUFFER_BIT);

  cube.render({
    uTime: tick * 0.1,
    uModel: new luma_gl.Matrix4().rotateX(tick * 0.01).rotateY(tick * 0.013),
    uView: luma_gl.Matrix4.lookAt({
      center: [0, 0, 0],
      eye: [
    ...
    uProjection:
      luma_gl.Matrix4.perspective({fov: luma_gl.radians(60), aspect, near: 1, far: 2048.0})
  });
});
