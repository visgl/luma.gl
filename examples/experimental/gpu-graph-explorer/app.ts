...
export const title = 'GPU Graph GPU Graph Explorer';
...
export default class GPUGraphExplorerAnimationLoopTemplate extends AnimationLoopTemplate {
  ...
  communities!: GPUGraphLabelPropagation;
  search!: GPUGraphBreadthFirstSearch;
  layout!: GPUGraphForceLayout;
  spatialLayout: GPUGraphSpatialForceLayout | null = null;
  ...
  constructor({
    ...
    if (device.type !== 'webgpu') {
      throw new Error('GPU Graph GPU Graph Explorer requires WebGPU');
    }
    this.device = device;
    ...
    this.panels = new ExamplePanelManager({
      panel: makeHtmlCustomPanel({
        id: 'gpu-graph-explorer-controls',
  ...
  private initializeGraph(dataset: GraphExplorerDataset): void {
    ...
    this.topology = new GPUGraphTopology({
      id: 'gpu-graph-explorer-topology',
      graph: this.graph,
      forward,
      reverse,
    ...
    this.components = new GPUGraphConnectedComponents({
      id: 'gpu-graph-explorer-components',
      topology: this.topology,
    ...
    this.communities = new GPUGraphLabelPropagation({
      id: 'gpu-graph-explorer-communities',
      topology: this.topology,
    ...
    this.search = new GPUGraphBreadthFirstSearch({
      id: 'gpu-graph-explorer-neighborhood',
      topology: this.topology,
      seeds: this.seeds,
    ...
    if (useSpatialLayout) {
      ...
      this.spatialLayout = new GPUGraphSpatialForceLayout({
        id: 'gpu-graph-explorer-spatial-layout',
        layout: this.layout,
        gridSize,
  ...
  private createAnalysisGraph(): CompiledGPUCommandGraph<void> {
    const graph = new GPUCommandGraph<void>(this.device, {id: 'gpu-graph-explorer-analysis'});
    ...
    if (this.graph.vertexCount >= GRAPH_EXPLORER_LINEAR_LAYOUT_VERTEX_COUNT) {
      for (const [name, contributor] of [
        ...
        const stage = new GPUCommandGraph<void>(this.device, {
          id: `gpu-graph-explorer-${name}-analysis`
        });
  ...
  private createSearchGraph(): CompiledGPUCommandGraph<void> {
    const graph = new GPUCommandGraph<void>(this.device, {id: 'gpu-graph-explorer-search'});
  ...
  private createFrameGraph(
    ...
    graph.addRenderPass({
      id: 'gpu-graph-render-edges-and-vertices',
      attachments: {
  ...
  private createPickingGraph(
    ...
    const target = new GPUIndexPickingTarget(graph, {
      id: 'gpu-graph-vertex-picking',
      width,
      height
    });
    const renderId = 'gpu-graph-render-picking';
    graph.addRenderPass({
      id: renderId,
    ...
    for (const [chunkIndex, source] of this.graph.sourceVertices.data.entries()) {
      ...
      edgeModels.push({
        ...
        model: new Model(this.device, {
          id: `gpu-graph-explorer-edges-${chunkIndex}`,
  ...
  private createPickingModel(): Model {
    return new Model(this.device, {
      id: 'gpu-graph-explorer-picking-nodes',
  ...
  private createChunkedVector(name: string, chunks: Uint32Array[]): GPUVector<'uint32'> {
    const data = chunks.map((values, chunkIndex) => {
      const buffer = this.device.createBuffer({
        id: `gpu-graph-explorer-${name}-${chunkIndex}`,
