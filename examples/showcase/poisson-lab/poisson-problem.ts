...
export function makeManufacturedPoissonProblem(resolution = 128): PoissonProblem2D {
  if (!Number.isInteger(resolution) || resolution < 4)
    throw new Error('Poisson resolution must be an integer >= 4');
  ...
  for (let iy = 0; iy < interior; iy++) {
    ...
    for (let ix = 0; ix < interior; ix++) {
      ...
      if (iy > 0) {
        columns.push(index(ix, iy - 1));
        coefficients.push(-invH2);
      }
      if (ix > 0) {
        columns.push(index(ix - 1, iy));
        coefficients.push(-invH2);
      }
      columns.push(row);
      coefficients.push(4 * invH2);
      if (ix + 1 < interior) {
        columns.push(index(ix + 1, iy));
        coefficients.push(-invH2);
      }
  ...
  return {
    ...
    rhs,
    rowOffsets,
    columnIndices: Uint32Array.from(columns),
    values: Float32Array.from(coefficients)
  };
}
...
export function applyPoissonCSR(problem: PoissonProblem2D, vector: Float32Array): Float64Array {
  ...
  for (let row = 0; row < vector.length; row++) {
    ...
    for (let i = problem.rowOffsets[row]; i < problem.rowOffsets[row + 1]; i++)
      sum += problem.values[i] * vector[problem.columnIndices[i]];
    output[row] = sum;
  }
  return output;
}
...
export function relativePoissonResidual(problem: PoissonProblem2D, solution: Float32Array): number {
  const ax = applyPoissonCSR(problem, solution);
  let residual2 = 0;
  let rhs2 = 0;
  for (let i = 0; i < solution.length; i++) {
    const r = problem.rhs[i] - ax[i];
    residual2 += r * r;
    rhs2 += problem.rhs[i] * problem.rhs[i];
  }
