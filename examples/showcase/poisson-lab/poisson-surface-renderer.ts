...
export class PoissonSurfaceRenderer {
  ...
  constructor(
    ...
    this.model = new Model(device, {
      ...
      bindings: {solution, frame: this.frameBuffer}
    });
  }
  render(time = 0): void {
    ...
    const eye: [number, number, number] = [
      2.45 * Math.cos(time * 0.08),
      1.75,
      2.45 * Math.sin(time * 0.08)
    ];
    const view = new Matrix4().lookAt({eye, center: [0, 0.32, 0], up: [0, 1, 0]});
    const vp = new Matrix4(projection).multiplyRight(view);
    const data = new ArrayBuffer(80);
    new Float32Array(data, 0, 16).set(vp);
    const dv = new DataView(data);
    dv.setFloat32(64, 0.72, true);
    dv.setUint32(68, this.resolution, true);
    dv.setUint32(72, this.resolution - 2, true);
    this.frameBuffer.write(data);
    const pass = this.device.beginRenderPass({
      id: 'poisson-surface-pass',
      framebuffer: canvas.getCurrentFramebuffer(),
      clearColor: [0.003, 0.008, 0.02, 1],
      clearDepth: 1
    });
    try {
