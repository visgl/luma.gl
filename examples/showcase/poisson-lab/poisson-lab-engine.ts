...
export class PoissonLabEngine {
  ...
  constructor(device: Device, resolution = 128, iterations = DEFAULT_PCG_ITERATIONS) {
    ...
    this.buffers = {
      rowOffsets: makeBuffer(
        device,
        'poisson-row-offsets',
        this.problem.rowOffsets,
        Buffer.STORAGE
      ),
      columnIndices: makeBuffer(
        device,
        'poisson-columns',
        this.problem.columnIndices,
        Buffer.STORAGE
      ),
      ...
      solution: device.createBuffer({
        id: 'poisson-solution',
        byteLength: n * 4,
        usage: Buffer.STORAGE | Buffer.COPY_SRC | Buffer.COPY_DST
      })
    };
    const built = this.createGraph(iterations);
    this.graph = built.graph;
    this.metrics = {
      resolution,
      unknowns: n,
      nonZeros: this.problem.values.length,
      graphNodes: this.graph.stats.nodeOrder.length,
      spmvStrategy: built.spmvStrategy,
      preconditioner: 'jacobi',
  ...
  measureAsync(): Promise<PoissonLabMetrics> {
    if (!this.measurementPromise) this.measurementPromise = this.readMetrics();
  ...
  private async readMetrics(): Promise<PoissonLabMetrics> {
    const bytes = await this.buffers.solution.readAsync();
    const solution = new Float32Array(bytes);
    const applied = applyPoissonCSR(this.problem, solution);
    let residual2 = 0,
      rhs2 = 0,
      error2 = 0,
      exact2 = 0;
    for (let i = 0; i < solution.length; i++) {
      const residual = this.problem.rhs[i] - applied[i];
      residual2 += residual * residual;
      rhs2 += this.problem.rhs[i] * this.problem.rhs[i];
      const error = solution[i] - this.problem.exact[i];
      error2 += error * error;
      exact2 += this.problem.exact[i] * this.problem.exact[i];
    }
    this.metrics.relativeResidual = Math.sqrt(residual2 / Math.max(rhs2, Number.MIN_VALUE));
    this.metrics.relativeError = Math.sqrt(error2 / Math.max(exact2, Number.MIN_VALUE));
    return this.metrics;
  }
  private createGraph(iterations: number): {
    graph: CompiledGPUCommandGraph<{}>;
    spmvStrategy: string;
    iterations: number;
  } {
    const graph = new GPUCommandGraph<{}>(this.device, {id: 'poisson-lab-pcg'});
    const n = this.problem.rhs.length;
    const rowOffsets = importView(graph, this.buffers.rowOffsets, 'rowOffsets', 'uint32', n + 1),
      columns = importView(
        graph,
        this.buffers.columnIndices,
        'columns',
        'uint32',
        this.problem.columnIndices.length
      ),
      values = importView(
        graph,
        this.buffers.values,
