...
import { AnimationLoopTemplate, DynamicTexture, loadImageBitmap, ShaderPassRenderer } from '@luma.gl/engine';
...
`;
    ...
    constructor({ device }) {
        ...
        this.selector = createSelector(document.body, shaderPassNames, passName => {
            const shaderPasses = this.shaderPassMap[passName]
...
function createSelector(parent, array, onChange) {
    ...
    selectList?.addEventListener('change', e => onChange(e.target.value));
    return selectList;
}
/*
