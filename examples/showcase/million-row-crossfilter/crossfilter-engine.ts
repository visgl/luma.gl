...
import {
  addCrossfilterDataFrameCategoryCount,
...
export class CrossfilterEngine {
  ...
  constructor(device: Device, options: CrossfilterEngineOptions = {}) {
    ...
    let initializedDataFrame: GPUDataFrame<CrossfilterDataFrameSchema> | undefined;
    try {
      const longitude = this.createSourceColumn('longitude', dataset.longitude, 'float32'),
        latitude = this.createSourceColumn('latitude', dataset.latitude, 'float32'),
        value = this.createSourceColumn('value', dataset.value, 'float32'),
        risk = this.createSourceColumn('risk', dataset.risk, 'float32'),
        hour = this.createSourceColumn('hour', dataset.hour, 'float32'),
        category = this.createSourceColumn('category', dataset.category, 'uint32');
      initializedDataFrame = createCrossfilterDataFrame({
      ...
      const categories = this.createOutputColumn(
        ...
        selectedCount = this.createOutputColumn('selected-count', 1, true),
        selectionMask = this.createOutputColumn('global-selection-mask', this.rowCount, false),
        categoryAggregationMask = this.createOutputColumn(
          'category-aggregation-mask',
          this.rowCount,
          false
        ),
        visibleIndices = this.createOutputColumn('visible-indices', this.rowCount, false);
      ...
      initializedFilter = new GPUCrossfilter(this.graph, {
        ...
        views: [
          ...
          {
            id: 'category-dataframe-counts',
            kind: 'mask',
            dimension: 'category',
            includeOwnSelection: false,
            output: categoryAggregationMask.view
          },
          {
      ...
      addCrossfilterDataFrameCategoryCount(
        this.graph,
        initializedDataFrame,
        categoryAggregationMask.view,
        categories.view
      );
      ...
      this.dataFrame = initializedDataFrame;
  ...
  setMapBounds(b: CrossfilterBounds | null): this {
    this.assertAvailable();
    b ? this.filter.setBounds('map', b) : this.filter.clear('map');
    return this;
  }
  setScatterBounds(b: CrossfilterBounds | null): this {
    this.assertAvailable();
    b ? this.filter.setBounds('scatter', b) : this.filter.clear('scatter');
    return this;
  }
  setRange(d: CrossfilterRangeDimension, r: readonly [number, number] | null): this {
    this.assertAvailable();
    r ? this.filter.setRange(d, r) : this.filter.clear(d);
    return this;
  }
  setCategory(c: number | null): this {
    this.assertAvailable();
    if (c === null) {
      this.filter.clear('category');
      return this;
    }
    if (!Number.isInteger(c) || c < 0 || c >= CROSS_FILTER_CATEGORY_NAMES.length)
      throw new Error('Crossfilter category is outside the available dense group range');
    this.filter.setRange('category', [c, c]);
    return this;
  }
  clearAll(): this {
    this.assertAvailable();
  ...
  update(): Promise<CrossfilterSummary> {
    ...
    const p = this.executeUpdate();
    this.pendingUpdate = p;
    const clear = () => {
      if (this.pendingUpdate === p) this.pendingUpdate = null;
    };
    void p.then(clear, clear);
    return p;
  }
  render(o: CrossfilterRenderOptions): void {
    this.assertAvailable();
  ...
  destroy(): void {
    ...
    this.filter.destroy();
    this.dataFrame.destroy();
    this.releaseOwnedBuffers();
  }
  private async executeUpdate(): Promise<CrossfilterSummary> {
    const e = this.device.createCommandEncoder({id: `crossfilter-update-${this.executionCount}`}),
      encoding = this.compiledGraph.encode(e, {parameters: undefined}),
      columns = getSummaryOutputColumns(this.summaryOutputs);
    let offset = 0;
    for (const o of columns) {
      e.copyBufferToBuffer({
        sourceBuffer: o.buffer,
        destinationBuffer: this.packedSummaryBuffer,
        destinationOffset: offset,
        size: o.buffer.byteLength
      });
      offset += o.buffer.byteLength;
    }
    this.device.submit(e.finish());
    this.executionCount++;
    const start = nowMilliseconds(),
      bytes = await this.packedSummaryBuffer.readAsync();
    offset = 0;
    const v = columns.map(o => {
        const x = new Uint32Array(bytes.buffer, bytes.byteOffset + offset, o.view.length);
        offset += o.buffer.byteLength;
        return x;
      }),
      [vs, vb, rs, rb, hs, hb, categoryCounts, selectedCount] = v;
    return {
      rowCount: this.rowCount,
      selectedCount: selectedCount[0],
      histograms: {
        value: {bins: vs, baselineBins: vb, domain: CROSS_FILTER_DOMAINS.value},
        risk: {bins: rs, baselineBins: rb, domain: CROSS_FILTER_DOMAINS.risk},
        hour: {bins: hs, baselineBins: hb, domain: CROSS_FILTER_DOMAINS.hour}
      },
      categoryCounts,
      encodeTimeMilliseconds: encoding.stats.cpuEncodeTimeMilliseconds,
      readbackTimeMilliseconds: nowMilliseconds() - start,
  ...
  private createSourceColumn<Format extends 'float32' | 'uint32'>(
    ...
    const buffer = this.device.createBuffer({
      ...
      data = new GPUData<Format>({buffer, format, length: values.length, ownsBuffer: true});
    this.ownedSourceData.push(data);
    return {buffer, data, view: this.graph.importGPUData(`crossfilter-source-${name}`, data)};
  }
  private createHistogramOutputs(d: CrossfilterRangeDimension, n: number) {
    if (!Number.isSafeInteger(n) || n <= 0)
      throw new Error('Crossfilter histogram requires a positive, integral bin count');
    return {
      selected: this.createOutputColumn(`${d}-selected-bins`, n, true),
      baseline: this.createOutputColumn(`${d}-baseline-bins`, n, true)
    };
  }
  ...
  private releaseOwnedBuffers(): void {
    for (const b of this.ownedOutputBuffers.splice(0)) b.destroy();
    for (const d of this.ownedSourceData.splice(0)) d.destroy();
  }
  private assertAvailable(): void {
    if (this.destroyed) throw new Error('Crossfilter showcase engine has been destroyed');
  }
}
function getSummaryOutputColumns(o: SummaryOutputs): readonly OutputColumn[] {
  return [
    o.histograms.value.selected,
    o.histograms.value.baseline,
    o.histograms.risk.selected,
    o.histograms.risk.baseline,
    o.histograms.hour.selected,
    o.histograms.hour.baseline,
    o.categories,
    o.selectedCount
  ];
}
