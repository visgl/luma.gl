...
test('ShaderFactory#createShader', async (t) => {
    ...
    t.deepEqual([shader1.id, shader2.id, shader3.id], ['1-cached', '1-cached', '3-cached'], 'Annotates IDs of cached shaders');
...
test('ShaderFactory#release', async (t) => {
    ...
    factory.release(shader2);
    factory.release(shader3);
    t.deepEqual([shader1.destroyed, shader2.destroyed, shader3.destroyed], [false, false, true], 'Keeps used shaders');
    factory.release(shader1);
    t.deepEqual([shader1.destroyed, shader2.destroyed, shader3.destroyed], [true, true, true], 'Destroys unused shaders');
    t.end();
});
