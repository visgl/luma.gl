...
`,
    ...
    t.is(stats.get('Buffers Active').count - initialActiveBuffers, 2, 'Created new buffers for attributes');
...
test('Model#pipeline caching with defines and modules', async (t) => {
    ...
    t.ok(defineModel1.pipeline !== defineModuleModel1.pipeline, 'Module and define triggers new pipeline');
    t.ok(moduleModel1.pipeline !== defineModuleModel1.pipeline, 'Module and define triggers new pipeline');
    ...
    t.ok(defineModuleModel1.pipeline === defineModuleModel2.pipeline, 'Got cached pipeline with modules and defines');
    t.end();
});
/*
