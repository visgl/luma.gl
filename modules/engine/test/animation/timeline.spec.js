...
test('Animation#Timeline', t => {
    ...
    t.equals(animationChannel1.time, timeline.getTime(channel1), 'Animation updated for channel timeline');
    timeline.setTime(1);
    t.equals(timeline.getTime(), 1, 'Timeline was set');
    t.equals(timeline.getTime(channel1), 0 * CHANNEL1_RATE, 'Channel 1 time does not elapse before delay');
    t.equals(timeline.getTime(channel2), 0 * CHANNEL2_RATE, 'Channel 2 time does not elapse before delay');
    timeline.setTime(4);
    ...
    t.equals(timeline.getTime(channel1), 2 * CHANNEL1_RATE, 'Channel 1 was set on update while playing');
    t.equals(timeline.getTime(channel2), 1 * CHANNEL2_RATE, 'Channel 2 was set on update while playing');
    timeline.reset();
    t.equals(timeline.getTime(), 0, 'Timeline was reset');
    timeline.pause();
    timeline.update(4);
    ...
    t.equals(timeline.getTime(channel1), 0 * CHANNEL1_RATE, 'Channel 1 was not set on update while paused');
    t.equals(timeline.getTime(channel2), 0 * CHANNEL2_RATE, 'Channel 2 was not set on update while paused');
