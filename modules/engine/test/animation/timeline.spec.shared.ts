...
export function registerTimelineTests(test: typeof it): void {
  test('Animation#Timeline', () => {
    const timeline = new Timeline();
    timeline.pause();
    timeline.reset();
    expect(timeline.getTime(), 'Timeline was reset').toBe(0);
    expect(Boolean(!timeline.isFinished(-1)), 'Unknown channels are never finished').toBe(true);
    ...
    expect(animationChannel1.time, 'Animation updated for channel timeline').toBe(
      timeline.getTime(channel1)
    );

    timeline.setTime(1);
    expect(timeline.getTime(), 'Timeline was set').toBe(1);
    expect(timeline.getTime(channel1), 'Channel 1 time does not elapse before delay').toBe(
      0 * CHANNEL1_RATE
    );
    expect(timeline.getTime(channel2), 'Channel 2 time does not elapse before delay').toBe(
      0 * CHANNEL2_RATE
    );

    timeline.setTime(4);
    expect(timeline.getTime(channel1), 'Channel 1 set').toBe(2 * CHANNEL1_RATE);
    expect(Boolean(!timeline.isFinished(channel1)), 'Channel 1 is not finished').toBe(true);
    expect(timeline.getTime(channel2), 'Channel 2 set').toBe(1 * CHANNEL2_RATE);
    expect(Boolean(!timeline.isFinished(channel2)), 'Channel 2 is not finished').toBe(true);
    expect(animationBase.time, 'Animation updated for base timeline').toBe(timeline.getTime());
    expect(animationChannel1.time, 'Animation updated for channel timeline').toBe(
      timeline.getTime(channel1)
    );
    ...
    expect(Boolean(timeline.isFinished(channel1)), 'Channel 1 is finished').toBe(true);
    ...
    expect(Boolean(timeline.isFinished(channel2)), 'Channel 2 is finished').toBe(true);
    expect(animationBase.time, 'Animation updated for base timeline').toBe(timeline.getTime());
    expect(animationChannel1.time, 'Animation updated for channel timeline').toBe(
      timeline.getTime(channel1)
    );
    ...
    expect(timeline.getTime(channel1), 'Channel 1 was set on update while playing').toBe(
      2 * CHANNEL1_RATE
    );
    expect(timeline.getTime(channel2), 'Channel 2 was set on update while playing').toBe(
      1 * CHANNEL2_RATE
    );
    ...
    expect(timeline.getTime(channel1), 'Channel 1 was not set on update while paused').toBe(
      0 * CHANNEL1_RATE
    );
    expect(timeline.getTime(channel2), 'Channel 2 was not set on update while paused').toBe(
      0 * CHANNEL2_RATE
    );
    ...
    timeline.setTime(timeline.getTime() + 1);
    expect(animationChannel1.time, 'Channel deletion detaches animation').toBe(
      channel1AnimationTime
    );
