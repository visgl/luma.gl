...
export function registerTimelineTests(test: TapeTestFunction): void {
  test('Animation#Timeline', t => {
    const timeline = new Timeline();
    timeline.pause();
    timeline.reset();
    t.is(timeline.getTime(), 0, 'Timeline was reset');
    t.ok(!timeline.isFinished(-1), 'Unknown channels are never finished');

    const defaultChannel = timeline.addChannel({});
    t.equals(timeline.getTime(defaultChannel), 0, 'Default channel starts at 0');
    timeline.removeChannel(99999);
    t.equals(timeline.getTime(defaultChannel), 0, 'Removing unknown channels is a no-op');
    ...
    timeline.play();
    timeline.update(4);
    t.equals(timeline.getTime(), 0, 'First update while playing seeds engine time');
    timeline.update(8);
