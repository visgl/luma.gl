...
const TEST_CASES: {
  ...
  props: {
    ...
    indices: {
      value: new Uint16Array([0, 1, 2, 3]),
      isIndexed: true
    },
    attributes: {
      positions: {
        value: new Float32Array([0, 0, 1, 0, 1, 1, 1, 0]),
        size: 2
      }
    },
    vertexCount: 3
...
test('Geometry#constructor', () => {
  for (const testCase of TEST_CASES) {
    if (testCase.shouldThrow) {
      expect(() => new Geometry(testCase.props)).toThrow(`${testCase.title}: should throw`);
    } else {
      ...
      expect(geometry.getVertexCount(), `${testCase.title}: vertexCount is correct`).toBe(testCase.vertexCount);
    }
  }
});
