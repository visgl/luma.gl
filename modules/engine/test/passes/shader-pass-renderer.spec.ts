...
it('ShaderPassRenderer resolves module-scoped bindings inside ShaderPassPipeline steps', async () => {
  const devices = await getTestDevices();
  for (const device of devices) {
    if (device.type === 'webgpu') {
      continue; // eslint-disable-line no-continue
...
it('ShaderPassRenderer supports ShaderPassPipeline targets', async () => {
  const devices = await getTestDevices();
  for (const device of devices) {
    if (device.type === 'webgpu') {
      continue; // eslint-disable-line no-continue
...
it('ShaderPassRenderer validates ShaderPassPipeline routing', async () => {
  const devices = await getTestDevices();
