...
test('ShaderInputs#dependencies', t => {
    ...
    const shaderInputs = new ShaderInputs({ custom });
    ...
    t.deepEqual(shaderInputs.moduleUniforms.picking.highlightedObjectColor, [1, 2, 3], 'highlight object color updated');
    t.end();
});
test('ShaderInputs#bindings', t => {
    [true, false].map(callback => {
        ...
        const shaderInputs = new ShaderInputs({ custom });
        const MOCK_TEXTURE = 'MOCK_TEXTURE';
        shaderInputs.setProps({
