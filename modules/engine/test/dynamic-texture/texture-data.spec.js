...
import { isTextureSliceData, getFirstMipLevel, getTextureSizeFromData } from '../../src/dynamic-texture/texture-data';
import { isExternalImage, getExternalImageSize } from '@luma.gl/core';
test('isTextureSliceData: typed array image vs not', t => {
    t.equal(isTextureSliceData(mkImageData(4, 2)), true, 'true for TextureImageData with typed array');
    // Random non-image objects
    t.equal(isTextureSliceData({}), false, 'false for random object');
    t.equal(isTextureSliceData([]), false, 'false for array');
    t.equal(isTextureSliceData(null), false, 'false for null');
    ...
    if (image) {
        ...
        t.comment('ExternalImage test skipped (no native external image type available in this environment).');
    }
    t.end();
});
...
test('getTextureSizeFromData: 1d', t => {
    const props = { dimension: '1d', data: mkImageData(32, 1) };
    t.deepEqual(getTextureSizeFromData(props), { width: 32, height: 1 });
    t.end();
});
test('getTextureSizeFromData: 2d (single & mips)', t => {
    t.deepEqual(getTextureSizeFromData({ dimension: '2d', data: mkImageData(64, 32) }), { width: 64, height: 32 }, '2d single mip');
    t.deepEqual(getTextureSizeFromData({ dimension: '2d', data: [mkImageData(64, 64), mkImageData(32, 32)] }), { width: 64, height: 64 }, '2d first mip from array');
    t.end();
});
test('getTextureSizeFromData: 2d with ExternalImage (env-dependent)', t => {
    ...
    t.ok(reported && typeof reported.width === 'number' && typeof reported.height === 'number', 'getExternalImageSize returns width/height');
    const props = { dimension: '2d', data: ext };
    t.deepEqual(getTextureSizeFromData(props), reported, 'uses getExternalImageSize for ExternalImage');
    t.end();
});
test('getTextureSizeFromData: 3d & 2d-array', t => {
    ...
    t.deepEqual(getTextureSizeFromData({ dimension: '3d', data: threeD }), { width: 16, height: 8 }, '3d first slice, first mip');
    const arr = [mkImageData(20, 10), mkImageData(20, 10)];
    t.deepEqual(getTextureSizeFromData({ dimension: '2d-array', data: arr }), { width: 20, height: 10 }, '2d-array first layer, first mip');
    t.equal(getTextureSizeFromData({ dimension: '3d', data: [] }), null, '3d empty → null');
    t.equal(getTextureSizeFromData({ dimension: '2d-array', data: [] }), null, '2d-array empty → null');
    t.end();
});
test('getTextureSizeFromData: cube & cube-array', t => {
    ...
    t.deepEqual(getTextureSizeFromData({ dimension: 'cube', data: cube }), { width: 128, height: 128 }, 'cube: picks first face first mip');
    const cube0 = {
    ...
    t.deepEqual(getTextureSizeFromData({ dimension: 'cube-array', data: [cube0, cube1] }), { width: 64, height: 64 }, 'cube-array: first cube, first face, first mip');
    t.equal(getTextureSizeFromData({ dimension: 'cube', data: {} }), null, 'cube empty map → null');
    t.equal(getTextureSizeFromData({ dimension: 'cube-array', data: [] }), null, 'cube-array empty → null');
    t.end();
});
test('getTextureSizeFromData: invalid 2d payload throws', t => {
    // When first mip is neither ExternalImage nor has width/height keys.
    const bad = { foo: 'bar' };
    t.throws(() => getTextureSizeFromData({ dimension: '2d', data: bad }), /Unsupported mip-level data/, 'throws for unsupported mip-level data');
    t.end();
});
// ---------------- Helpers ----------------
function mkImageData(width, height, bytes = width * height * 4) {
    return {
...
function maybeMakeExternalImage() {
    ...
    try {
        ...
        if (typeof OffscreenCanvas !== 'undefined' &&
            ...
            const bmp = globalThis.createImageBitmap(oc);
