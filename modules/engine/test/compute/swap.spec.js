...
test('SwapFramebuffers#resize', async (t) => {
    ...
    t.equal(swap.current.colorAttachments[0].texture, currentTexture, 'current framebuffer texture unchanged');
    ...
    t.notEqual(swap.current.colorAttachments[0].texture, currentTexture, 'current framebuffer texture replaced after resize');
    ...
    t.equal(swap.current.colorAttachments[0].texture, newCurrent, 'current framebuffer texture unchanged after no-op');
    t.equal(swap.next.colorAttachments[0].texture, newNext, 'next framebuffer texture unchanged after no-op');
    swap.destroy();
    t.end();
});
