...
export async function convertArrowPathFloat64ToGPUVector(
  ...
  const pathFormat = getArrowPathCoordinateFormat(componentCount);
  const outputData: GPUData<VertexList<'float32x2' | 'float32x3' | 'float32x4'>>[] = [];
  ...
  for (const [chunkIndex, data] of paths.data.entries()) {
    ...
    outputData.push(
      new GPUData<VertexList<'float32x2' | 'float32x3' | 'float32x4'>>({
  ...
  return {
    paths: new GPUVector<VertexList<'float32x2' | 'float32x3' | 'float32x4'>>({
      type: 'data',
      name: 'paths',
      dataType: pathType,
      format: pathFormat,
