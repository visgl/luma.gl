...
export function getArrowPaths(arrowObject) {
    const data = getArrowDataArray(arrowObject)[0];
    return getArrowPathsRecursive(data, []);
}
export function getArrowPathsRecursive(arrowData, currentPath) {
...
export function getArrowDataByPath(arrowObject, columnPath) {
    const data = getArrowDataArray(arrowObject)[0];
    const path = decomposePath(columnPath);
    let nestedData = data;
    for (const key of path) {
        if (!arrow.DataType.isStruct(nestedData.type)) {
            throw new Error(`Arrow table nested column is a not a struct: '${key} in '${path.join('.')}'`);
        }
        const fields = nestedData.type.children;
        const indexByField = fields.findIndex(field => field.name === key);
        if (indexByField === -1) {
            throw new Error(`Arrow table schema does not contain nested column '${key} in '${path.join('.')}'`);
        }
...
export function getArrowVectorByPath(arrowTable, columnPath) {
...
export function getArrowDataArray(arrowObject) {
