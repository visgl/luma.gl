...
test('linestring vertex expansion#1', () => {
  ...
  expect(arraysEqual(expanded, expected), 'expands correctly (size = 1)').toBeTruthy();
});
test('linestring vertex expansion#3', () => {
  const input = new Uint8Array([0, 1, 2, 3, 4, 5, 6, 7, 8]);
  const size = 3;
  const geomOffsets = new Int32Array([0, 2, 5, 9]);
  const expanded = expandArrayToCoords(input, size, geomOffsets);
  const expected = new Uint8Array([0, 1, 2, 0, 1, 2, 3, 4, 5, 3, 4, 5, 3, 4, 5, 6, 7, 8, 6, 7, 8, 6, 7, 8, 6, 7, 8]);
  expect(arraysEqual(expanded, expected), 'expands correctly (size = 3)').toBeTruthy();
});
