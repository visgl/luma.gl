...
```typescript
if (device.features.has('webgl-instanced-rendering')) {
...
if (device.features.has('webgl-timer-query')) {
   ...
}
...
There are a few additional capability query functions in the luma.gl API. In particular, WebGL 2 specific classes have an `isSupported` method that duplicates some of the queryies that can be made using the capability system
...
| ---------------------------------------- | ------ | ------ | ----- | ------------------------------------------------------ |
| `webgpu`                                 | Y      | N      | N     | Device is using WebGPU.                                |
| `webgl2`                                 | N      | Y      | N     | Device is using WebGL2.                                |
| `webgl`                                  | N      | Y      | Y     | Device is using WebGL or WebGL2.                       |
| `depth-clip-control` (WebGPU)            | \*     | N      | N     |                                                        |
| `depth24unorm-stencil8` (WebGPU)                   | \*     | Y      | -     | `UNSIGNED_INT_24_8_WEBGL`                              |
...
| `texture-compression-astc`               | \*     | \*     | \*    | ASTC compressed textures. AMD proprietary.             |
| `webgl-texture-compression-etc1`         | \*     | \*     | \*    | ASTC compressed textures. Qualcomm Snapdragon Android. |
| `webgl-texture-compression-pvrtc`        | \*     | \*     | \*    | PVRTC compressed textures. iOS devices.                |
| `webgl-texture-compression-atc`          | \*     | \*     | \*    | ASTC compressed textures. .                            |
| `webgl-timer-query`                      | N      | \*     | \*    | GPU timer support (cf WebGPU timestamp-query?)       |
...
| `webgl-float-blend`                      | Y      | \*     | \*    | blending                                               |
| `webgl-color-encoding-srgb`              | Y      | \*     | \*    | textures and renderbuffers                             |
| `webgl-texture-depth`                    | Y      | \*     | \*    | textures                                               |
| `webgl-texture-float`                    | Y      | \*     | \*    | textures                                               |
| `webgl-texture-half-float`               | Y      | \*     | \*    | // textures                                            |
| `webgl-texture-filter-linear-float`      | Y      | \*     | \*    |
| `webgl-texture-filter-linear-half-float` | Y      | \*     | \*    |
| `webgl-texture-filter-anisotropic`       | Y      | \*     | \*    |
| `webgl-color-attachment-rgba32f`         | Y      | \*     | \*    | framebuffers, textures and renderbuffers               |
| `webgl-color-attachment-float`           | Y      | \*     | \*    | framebuffers, textures and renderbuffers               |
| `webgl-color-attachment-half-float`      | Y      | \*     | \*    | framebuffers, textures and renderbuffers               |
| `glsl-frag-data`                         | Y      | Y      | \*    |                                                        |
| `glsl-frag-depth`                        | Y      | Y      | \*    |                                                        |
