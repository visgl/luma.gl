...
export type GPUScalarComparisonOperation =
  | 'equal'
  | 'not-equal'
  | 'less-than'
  | 'less-than-or-equal'
...
export type GPUScalarOperationProps = {
  id?: string;
...
/**
 * Applies one arithmetic or comparison operation to graph-native GPU scalars.
 *
 * All operands must live in the same graph-owned value arena, so the entire operation uses one
 * storage-buffer binding regardless of scalar count. Comparison results are encoded as uint32
 * values: `1u` for true and `0u` for false.
 */
export class GPUScalarCompute {
  readonly id: string;
  readonly operation: GPUScalarOperation;
  readonly left: GPUScalar;
  readonly right?: GPUScalar;
  readonly output: GPUScalar;

  constructor(props: GPUScalarOperationProps) {
  ...
  addToGraph<Parameters>(graph: GPUCommandGraph<Parameters>): void {
    ...
    if (this.output.arena !== arena) {
      throw new Error(`${this.id} output must belong to the same GPUValueArena`);
    }
    if (arena.graph !== (graph as unknown as GPUCommandGraph<unknown>)) {
      throw new Error(`${this.id} scalars must belong to the target graph`);
    }

    const {buffer} = arena.seal();
...
function validateOperation(operation: GPUScalarCompute): void {
  const isUnary = operation.operation === 'copy' || operation.operation === 'sqrt';
  const isComparison = isComparisonOperation(operation.operation);

  if (!isUnary && !operation.right) {
...
function makeShaderSource(operation: GPUScalarCompute): string {
...
function getExpression(operation: GPUScalarOperation, left: string, right?: string): string {
  switch (operation) {
    ...
    case 'equal': return `select(0u, 1u, ${left} == ${right})`;
    case 'not-equal': return `select(0u, 1u, ${left} != ${right})`;
    case 'less-than': return `select(0u, 1u, ${left} < ${right})`;
    case 'less-than-or-equal': return `select(0u, 1u, ${left} <= ${right})`;
    case 'greater-than': return `select(0u, 1u, ${left} > ${right})`;
    case 'greater-than-or-equal': return `select(0u, 1u, ${left} >= ${right})`;
  }
}

function isComparisonOperation(operation: GPUScalarOperation): operation is GPUScalarComparisonOperation {
  return [
    'equal',
    'not-equal',
    'less-than',
