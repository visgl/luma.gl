...
export type GPUValueArenaSlotInfo = {
  id: string;
  format: GPUValueFormat;
  byteOffset: number;
  wordOffset: number;
};
export type GPUValueArenaInfo = {
  id: string;
  bufferId: string;
  usedByteLength: number;
  capacityByteLength: number;
  availableByteLength: number;
  utilization: number;
  slotCount: number;
  slots: readonly GPUValueArenaSlotInfo[];
};
...
export function inspectGPUValueArena(arena: GPUValueArena): GPUValueArenaInfo {
  const slots = arena.getSlots().map(slot =>
    Object.freeze({
      id: slot.id,
      format: slot.format,
      byteOffset: slot.byteOffset,
      wordOffset: slot.byteOffset >>> 2
    })
  );
  return Object.freeze({
    id: arena.id,
    bufferId: arena.buffer.id,
    usedByteLength: arena.byteLength,
    capacityByteLength: arena.capacityByteLength,
    availableByteLength: arena.availableByteLength,
    utilization: arena.byteLength / arena.capacityByteLength,
    slotCount: arena.size,
...
export function formatGPUValueArenaLayout(info: GPUValueArenaInfo): string {
  ...
  return [
    header,
    ...info.slots.map(slot => `  ${slot.id} ${slot.format} @ ${slot.byteOffset}`)
  ].join('\n');
}
