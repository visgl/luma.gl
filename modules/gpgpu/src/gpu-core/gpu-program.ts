...
import {
  GPUProgramScalar,
  GPUProgramVector,
  type GPUProgramScalarFormat,
  type GPUProgramVectorFormat
} from './gpu-program-value';
...
export class GPUProgram {
  ...
  vector<T extends GPUProgramVectorFormat>(
    id: string,
    format: T,
    length: number,
  ...
  get vectors(): readonly GPUProgramVector[] {
    return Object.freeze([...this.programVectors.values()]);
  }
  ...
  private assertValueId(id: string): void {
    if (this.programScalars.has(id) || this.programVectors.has(id))
      throw new Error(`${this.id} value "${id}" already exists`);
  }
