 ...
 */
export class GPUConjugateGradientExecutable<Parameters = unknown> {
  readonly id: string;
  constructor(
    readonly props: {
      id?: string;
    ...
    if (props.rhs.length !== props.solution.length)
      throw new Error(`${this.id} rhs and solution lengths must match`);
    if (!Number.isInteger(props.maxIterations) || props.maxIterations < 1)
      throw new Error(`${this.id} maxIterations must be positive`);
  ...
  getCommandNodes(graph: GPUCommandGraph<Parameters>): readonly GPUCommandNode<Parameters>[] {
    ...
    const ax = createTransientView(graph, `${this.id}-ax`, 'float32', n);
    // Scalar state is declared before any operation seals the graph-owned arena.
    const rr = createGPUScalar(graph, `${this.id}-rr`, 'float32');
    const pq = createGPUScalar(graph, `${this.id}-pq`, 'float32');
    const alpha = createGPUScalar(graph, `${this.id}-alpha`, 'float32');
    const newRR = createGPUScalar(graph, `${this.id}-new-rr`, 'float32');
    const beta = createGPUScalar(graph, `${this.id}-beta`, 'float32');
    const minusOne = createGPUScalar(graph, `${this.id}-minus-one`, 'float32');
    const negAlpha = createGPUScalar(graph, `${this.id}-neg-alpha`, 'float32');
    const tolerance = createGPUScalar(graph, `${this.id}-tolerance-squared`, 'float32');
    const converged = createGPUScalar(graph, `${this.id}-converged`, 'uint32');
    ...
    const vectorGate = new GPUScalarDispatchGate(graph, {
      id: `${this.id}-vector-gate`,
      active,
      workgroups: [vectorWG, 1, 1]
    });
    const scalarGate = new GPUScalarDispatchGate(graph, {
      id: `${this.id}-scalar-gate`,
      active,
      workgroups: [1, 1, 1]
    });
...
function addVectorInitPass<Parameters>(
  graph: GPUCommandGraph<Parameters>,
  id: string,
  rhs: GraphDataView<'float32'>,
  ax: GraphDataView<'float32'>,
  r: GraphDataView<'float32'>,
  p: GraphDataView<'float32'>,
  minusOne: any,
  tolerance: any,
  active: any,
  toleranceSquared: number
  ...
  void tolerance;
  void active;
  void toleranceSquared;
}
function addActiveInvertPass<Parameters>(
  graph: GPUCommandGraph<Parameters>,
  id: string,
  converged: any,
  active: any
