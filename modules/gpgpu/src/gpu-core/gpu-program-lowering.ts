...
export type GPUOperationLoweringContext<Parameters=void>={
  ...
  /** Lowers an operation subtree under one semantic runtime predicate. */
  lowerConditional:(predicate:GPUOperationPredicate,operation:GPUProgramOperation)=>void;
  /** Resolves backend-independent scalar state to this compilation's WebGPU scalar allocation. */
  resolveScalar:<T extends 'float32'|'uint32'|'sint32'>(scalar:GPUProgramScalar<T>)=>GPUScalar<T>;
