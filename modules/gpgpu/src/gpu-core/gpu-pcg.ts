...
export class GPUJacobiPCG {
  ...
  addToGraph<Parameters>(graph: GPUCommandGraph<Parameters>): GPUJacobiPCGResult {
    ...
    addCopyViewPass(graph, `${this.id}-r0`, rhs, r, n * 4);
    ...
    addCopyViewPass(graph, `${this.id}-p0`, z, p, n * 4);
    ...
    new GPUScalarConstant({id: `${this.id}-minus-one`, output: minusOne, value: -1}).addToGraph(
      graph
    );
    let spmvStrategy = '';
