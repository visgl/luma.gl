...
export class GPUScalarDispatchGate {
  ...
  constructor<Parameters>(graph: GPUCommandGraph<Parameters>, props: {
    id: string;
    ...
    if (props.active.arena.graph !== (graph as unknown as GPUCommandGraph<unknown>)) {
  ...
  /** Refreshes indirect workgroup counts from the current GPU-resident active scalar. */
  addUpdateToGraph<Parameters>(graph: GPUCommandGraph<Parameters>, id = `${this.id}-update`): void {
    const arena = this.active.arena;
    const {buffer: arenaBuffer} = arena.seal();
...
@compute @workgroup_size(1) fn main() {
  ...
  dispatch[2] = ${z}u;
    ...
    graph.addComputePass({
      id,
      workload: {operation: 'GPUScalarDispatchGate', commandCount: 1, maximumWorkgroupCount: 1, maximumInvocationCount: 1, readByteLength: 4, writeByteLength: 12},
      resources: [{buffer: arenaBuffer, usage: 'storage-read'}, {buffer: this.dispatchBuffer, usage: 'storage-write'}],
      compile: ({device}) => {
        const computation = new Computation(device, {id, source, shaderLayout: {bindings: [
          {name: 'gpuValues', type: 'read-only-storage', group: 0, location: 0},
          {name: 'dispatch', type: 'storage', group: 0, location: 1}
        ]}});
        return {encode: ({computePass, getBuffer}) => {
          const bindings: Record<string, Binding> = {gpuValues: getBuffer(arenaBuffer), dispatch: getBuffer(this.dispatchBuffer)};
  ...
  get condition() {
    return {id: `${this.id}-active`, source: 'gpu' as const, mode: 'indirect' as const, buffer: this.dispatchBuffer};
  }
}
