...
type GPUValueArenaGraph = Pick<GPUCommandGraph<unknown>, 'createTransientBuffer'> & object;
...
/** Graph-owned packed storage for small GPU-produced values. Offsets remain stable for graph lifetime. */
export class GPUValueArena {
  ...
  constructor(
    ...
    this.graph = graph;
    if (
      !Number.isSafeInteger(capacityByteLength) ||
      capacityByteLength <= 0 ||
      capacityByteLength % 4 !== 0
    ) {
  ...
  getSlots(): readonly GPUValueSlot[] {
