...
export function createGPUConjugateGradientProgram(props:{id?:string;size:number;nonZeros:number;maxIterations:number;toleranceSquared:number;statistics?:GPUSpMVRowStatistics}) {
  ...
  if(!Number.isSafeInteger(props.size)||props.size<1)throw new Error(`${id} size must be positive`);
  if(!Number.isSafeInteger(props.nonZeros)||props.nonZeros<0)throw new Error(`${id} nonZeros must be non-negative`);
  ...
  const n=props.size;const program=new GPUProgram({id});
  const rowOffsets=program.vector(`${id}-row-offsets`,'uint32',n+1,{external:true});
  const columnIndices=program.vector(`${id}-column-indices`,'uint32',props.nonZeros,{external:true});
  const values=program.vector(`${id}-values`,'float32',props.nonZeros,{external:true});
  ...
  program.add([new GPUProgramScalarLiteral({output:minusOne,value:-1}),new GPUProgramScalarLiteral({output:tolerance,value:props.toleranceSquared}),new GPUProgramScalarLiteral({output:active,value:1}),new GPUProgramDotProduct({id:`${id}-rr0`,left:residual,right:residual,output:rr})]);
  ...
  return {program,matrix,rowOffsets,columnIndices,values,solution,residual,search};
}
