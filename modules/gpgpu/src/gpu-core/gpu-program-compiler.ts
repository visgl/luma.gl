...
import {GPUHistogram} from './gpu-histogram';
...
import {GPUProgramHistogram,GPUProgramReduction} from './gpu-semantic-analytics-operation';
...
export type GPUProgramBindings={
  vectors?:Readonly<Record<string,Buffer>>;
  /** Existing views let a program share logical graph resources with application-owned work. */
  views?:Readonly<Record<string,GraphDataView>>;
};
...
export class GPUProgramCompiler<Parameters=void>{
  ...
  transform(program:GPUProgram,graphOrBindings:GPUCommandGraph<Parameters>|GPUProgramBindings={},bindings:GPUProgramBindings={}):GPUProgramCompilation<Parameters>{
    ...
    for(const vector of program.vectors){
      const view=resolvedBindings.views?.[vector.id];
      if(view){if(view.buffer.graph!==graph||view.format!==vector.format||view.length!==vector.length)throw new Error(`GPUProgram view binding "${vector.id}" must belong to the target graph and match its logical vector`);vectors.set(vector.id,view);continue;}
      if(vector.external){const buffer=resolvedBindings.vectors![vector.id];if(buffer.byteLength<Math.max(1,vector.length)*4)throw new Error(`GPUProgram external vector "${vector.id}" buffer is too small`);const handle=graph.importBuffer({id:vector.id,byteLength:buffer.byteLength,usage:buffer.usage},buffer);vectors.set(vector.id,graph.createDataView(handle,{format:vector.format,length:vector.length}));}
      else vectors.set(vector.id,createTransientView(graph,vector.id,vector.format,vector.length));
    }
    const state:LoweringState={path:[],nodes:[],decisions:[],predicates:[],scalars,vectors,predicateConjunctions:new Map()}; for(const operation of program.operations)this.lower(graph,operation,state);
  ...
  protected registerCoreLowerings():void{
    ...
    this.lowerings.register<GPUProgramGroupCount>('group-count',(o,c)=>{new GPUGroupAggregation({id:o.id,keys:c.resolveVector(o.props.keys),mask:c.resolveVector(o.props.mask),output:c.resolveVector(o.props.output),operation:'count'}).addToGraph(c.graph);c.recordDecision({operationId:o.id,operationType:o.type,lowering:'webgpu-dense-group-count',reason:'dense group count reuses GPUGroupAggregation'});});
    this.lowerings.register<GPUProgramHistogram>('histogram',(o,c)=>{new GPUHistogram({id:o.id,input:c.resolveVector(o.props.input),mask:o.props.mask?c.resolveVector(o.props.mask):undefined,output:c.resolveVector(o.props.output),domain:o.props.domain}).addToGraph(c.graph);c.recordDecision({operationId:o.id,operationType:o.type,lowering:'webgpu-histogram',reason:'semantic histogram reuses GPUHistogram'});});
    this.lowerings.register<GPUProgramReduction>('reduction',(o,c)=>{new GPUReduction({id:o.id,input:c.resolveVector(o.props.input),mask:o.props.mask?c.resolveVector(o.props.mask):undefined,output:c.resolveVector(o.props.output),operation:o.props.operation}).addToGraph(c.graph);c.recordDecision({operationId:o.id,operationType:o.type,lowering:'webgpu-reduction',reason:'semantic reduction reuses GPUReduction'});});
  ...
  protected lower(graph:GPUCommandGraph<Parameters>,operation:GPUProgramOperation,state:LoweringState):void{const id='id'in operation&&typeof operation.id==='string'?operation.id:operation.constructor?.name??'legacy-contributor';state.path.push(id);const previousState=this.currentState;this.currentState=state;try{if(operation instanceof GPUCompositeOperation&&operation.type==='composite'){state.decisions.push({operationId:operation.id,operationType:operation.type,lowering:'flatten',reason:'WebGPU has no executable child-graph primitive'});for(const child of operation.operations)this.lower(graph,child,state);return;}if(isGPUOperation(operation)){const lowerer=this.lowerings.get(operation.type);if(lowerer){lowerer(operation,{graph,capabilities:this.capabilities,lower:child=>this.lower(graph,child,state),lowerConditional:(predicate,child)=>{state.predicates.push(predicate);try{this.lower(graph,child,state);}finally{state.predicates.pop();}},resolveScalar:s=>this.resolveScalar(s,state),resolveVector:v=>this.resolveVector(v,state),recordDecision:d=>state.decisions.push(d)});return;}}if(isGPUCommandGraphContributor(operation)){if(state.predicates.length)throw new Error(`legacy command-graph contributor "${id}" cannot be transformed inside semantic runtime control flow; migrate it to GPUCommandNodeProducer`);state.decisions.push({operationId:id,operationType:'legacy-contributor',lowering:'legacy-addToGraph',reason:'compatibility bridge; migrate contributor to GPUCommandNodeProducer'});operation.addToGraph(graph);return;}throw new Error(`GPUProgram operation "${id}" has no WebGPU transform`);}finally{this.currentState=previousState;state.path.pop();}}
  private emitProducer(graph:GPUCommandGraph<Parameters>,producer:GPUCommandNodeProducer<Parameters>,dispatches:[number,number,number][],state:LoweringState):void{const nodes=producer.getCommandNodes(graph);if(nodes.length!==dispatches.length)throw new Error(`transform produced ${nodes.length} command nodes but declared ${dispatches.length} dispatches`);const decorated=nodes.map((node,index)=>this.decorateNode(graph,this.attachDispatch(node,dispatches[index]),state));addGPUCommandNodes(graph,decorated);}
  private attachDispatch(node:GPUCommandNode<Parameters>,dispatch:[number,number,number]):GPUCommandNode<Parameters>{if(node.type!=='compute')throw new Error(`dispatch geometry can only be attached to compute command nodes; got ${node.type} node "${node.id}"`);return setGPUComputeDispatchWorkgroups(node,dispatch) as GPUCommandNode<Parameters>;}
  private decorateNode(graph:GPUCommandGraph<Parameters>,node:GPUCommandNode<Parameters>,state:LoweringState):GPUCommandNode<Parameters>{state.nodes.push({nodeId:node.id,nodeType:node.type,operationPath:[...state.path]});if(node.type!=='compute'||!state.predicates.length)return node;if(node.condition)throw new Error(`compute node "${node.id}" already has condition`);const workgroups=getGPUComputeDispatchWorkgroups(node);if(!workgroups)throw new Error(`compute node "${node.id}" under runtime predicate must declare exact dispatch geometry`);let active:GPUScalar<'uint32'>;if(state.predicates.length===1){active=state.scalars.get(state.predicates[0].value.id) as GPUScalar<'uint32'>;}else{const key=state.predicates.map(p=>p.value.id).join('&&');let conjunction=state.predicateConjunctions.get(key);if(!conjunction){conjunction=createGPUScalar(graph,`predicate-and-${state.predicateConjunctions.size}`,'uint32');state.predicateConjunctions.set(key,conjunction);}const inputs=state.predicates.map(p=>state.scalars.get(p.value.id) as GPUScalar<'uint32'>);const support=new GPUPredicateConjunction({id:`${node.id}-predicate-and`,inputs,output:conjunction!}).getCommandNodes(graph).map(n=>this.attachDispatch(n,[1,1,1]));addGPUCommandNodes(graph,support);active=conjunction;}if(!active)throw new Error(`runtime predicate for "${node.id}" could not be resolved`);const gate=new GPUScalarDispatchGate(graph,{id:`${node.id}-predicate`,active,workgroups});const gateNodes=gate.getUpdateCommandNodes(graph,`${node.id}-predicate-update`).map(n=>this.attachDispatch(n,[1,1,1]));addGPUCommandNodes(graph,gateNodes);return{...node,condition:gate.condition};}
  ...
  private resolveVector<T extends 'float32'|'uint32'|'sint32'>(v:GPUProgramVector<T>,state:LoweringState):GraphDataView<T>{const r=state.vectors.get(v.id);if(!r||r.format!==v.format||r.length!==v.length)throw new Error(`GPUProgram vector "${v.id}"