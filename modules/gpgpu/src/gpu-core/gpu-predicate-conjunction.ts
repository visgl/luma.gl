...
export class GPUPredicateConjunction {
  ...
  constructor(
    readonly props: {
      id?: string;
      inputs: readonly GPUScalar<'uint32'>[];
      output: GPUScalar<'uint32'>;
    }
  ) {
    this.id = props.id ?? 'gpu-predicate-conjunction';
    if (props.inputs.length < 2) throw new Error(`${this.id} requires at least two predicates`);
    const arena = props.output.arena;
    if (props.inputs.some(input => input.arena !== arena))
      throw new Error(`${this.id} predicates must share one GPUValueArena`);
  }
  getCommandNodes<Parameters>(
    graph: GPUCommandGraph<Parameters>
  ): readonly GPUCommandNode<Parameters>[] {
    const {inputs, output} = this.props;
    if (output.arena.graph !== (graph as unknown as GPUCommandGraph<unknown>))
      throw new Error(`${this.id} predicates must belong to target graph`);
    const buffer = output.arena.buffer;
    const expression = inputs.map(input => `(${getGPUScalarWGSLLoad(input)} != 0u)`).join(' && ');
    const source = `${getGPUValueArenaWGSLBinding(0, 0)}\n@compute @workgroup_size(1)fn main(){${getGPUScalarWGSLStore(output, `select(0u,1u,${expression})`)}}`;
    return [
      createGPUComputeCommandNode<Parameters>({
        id: this.id,
        workload: {
          operation: 'GPUPredicateConjunction',
          commandCount: 1,
          maximumWorkgroupCount: 1,
          maximumInvocationCount: 1,
          readByteLength: inputs.length * 4,
          writeByteLength: 4
        },
