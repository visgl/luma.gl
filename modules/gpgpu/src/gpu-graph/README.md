...
The experimental `@luma.gl/gpgpu/gpu-graph` subpath analyzes connected data directly on a browser
WebGPU device. Its
...
`GPUGraphSpatialForceLayout` can approximate distant
...
## When to use GPU Graph

Use GPU Graph to explore relationships that already live on the GPU: inspect social connections,
...
`GPUGraphSingleSourceShortestPath` when edges represent actual nonnegative travel times, distances,
latencies, or transaction costs and the cheapest route matters more than the fewest hops. Choose
`GPUGraphLocalClusteringCoefficient` when a vertex's neighbor count alone cannot distinguish a
tightly connected friend circle or transaction ring from a loose hub.

Choose `GPUGraphCoreNumber` when you need to find a genuinely resilient network backbone: a star
hub with 100 unconnected followers has degree 100 but core number one, whereas each member of a
four-person clique has core number three. Choose `GPUGraphModularity` when you already have
...
or optimize communities. Choose `GPUGraphModularityOptimization` when you actually need to improve
...
islands; `GPUGraphLabelPropagation` can expose friend circles, related service groups, or
...
validation. GPU Graph's local CPU/WebGPU benchmark is not that official driver, an audited
...
`GPUGraphSingleSourceShortestPath` follows outgoing, incoming, or bidirectional relationships from
...
`GPUGraphLocalClusteringCoefficient` asks whether each vertex's distinct incoming-or-outgoing
...
`GPUGraphCoreNumber` finds each vertex's highest supported `k` in the **simple undirected weak
...
`GPUGraphModularity` consumes existing `uint32` community identifiers and publishes one `float32`
...
`GPUGraphModularityOptimization` improves identity-initialized or caller-supplied `uint32`
...
See the [GPU Graph graph analytics guide](/docs/api-reference/experimental/gpu-graph) for when to use
