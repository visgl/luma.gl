...
/** Deferred pairwise division operation. */
class DivideOperation extends Operation<{x: GPUTableEvaluator; y: GPUTableEvaluator}> {
  /** Operation name used for backend lookup. */
  name = 'divide';

  /** Lazy output table for the division result. */
...
/**
 * Divides corresponding row elements from two or more tables.
 *
 * The returned table is lazy; no CPU or GPU work is performed until
 * {@link GPUTableEvaluator.evaluate} is called on the result.
 */
export function divide(...args: GPUTableEvaluator[]): GPUTableEvaluator {
  let result = args[0];
  for (let i = 1; i < args.length; i++) {
    result = new DivideOperation(result, args[i]).output;
  }
  return result;
}
