...
/** Deferred pairwise subtraction operation. */
class SubtractOperation extends Operation<{x: GPUTableEvaluator; y: GPUTableEvaluator}> {
  /** Operation name used for backend lookup. */
  name = 'subtract';

  /** Lazy output table for the subtraction result. */
...
/**
 * Subtracts corresponding row elements from two or more tables.
 *
 * The returned table is lazy; no CPU or GPU work is performed until
 * {@link GPUTableEvaluator.evaluate} is called on the result.
 */
export function subtract(...args: GPUTableEvaluator[]): GPUTableEvaluator {
  let result = args[0];
  for (let i = 1; i < args.length; i++) {
    result = new SubtractOperation(result, args[i]).output;
  }
  return result;
}
