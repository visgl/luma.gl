...
export class BatchConformanceFixture {
  ...
  column<T extends Format>(
    ...
    if (lengths.reduce((total, length) => total + length, 0) !== values.length) {
    ...
    const data = lengths.map(length => {
      const ArrayType = format === 'uint32' ? Uint32Array : Float32Array;
      // Prefix and padding sentinels expose accidental packed indexing of strided views.
      const storage = new ArrayType(Math.max(1, length * stride) + 1).fill(999);
      for (let row = 0; row < length; row++) storage[1 + row * stride] = values[offset + row];
      offset += length;
      ...
      buffers.push(buffer);
      return new GPUData({buffer, format, length, byteOffset: 4, byteStride: stride * 4});
    });
    const vector = this.graph.importGPUVector(id, {format, length: values.length, data});
  ...
  output<T extends Format>(id: string, format: T, length: number): GraphDataView<T> {
    return this.column(id, format, Array(length).fill(77), [length], {
      atomic: true
    }) as GraphDataView<T>;
  }
  ...
  async read<T extends Format>(view: GraphDataView<T> | GraphVectorView<T>): Promise<number[]> {
    ...
    for (const chunk of data) {
      ...
      for (let row = 0; row < chunk.length; row++) {
        values.push(storage[chunk.byteOffset / 4 + (row * chunk.byteStride) / 4]);
      }
    }
    return values;
  }
