...
function addShape(
  ...
  function getBevelVector(point: Vector2, previousPoint: Vector2, nextPoint: Vector2): Vector2 {
    ...
    if (Math.abs(collinear) > Number.EPSILON) {
      const previousLength = Math.sqrt(
        previousOffsetX * previousOffsetX + previousOffsetY * previousOffsetY
      );
      ...
      const scaleFactor =
        ((nextShiftX - previousShiftX) * nextOffsetY -
          (nextShiftY - previousShiftY) * nextOffsetX) /
      ...
      if (directionMatches) {
        ...
        shrinkBy = Math.sqrt(
          (previousOffsetX * previousOffsetX + previousOffsetY * previousOffsetY) / 2
        );
      }
    }
  ...
  for (let i = 0; i < vlen; i++) {
    const vertex = bevelEnabled
      ? scalePoint(vertices[i], combinedMovements[i], baseSize)
      : vertices[i];
