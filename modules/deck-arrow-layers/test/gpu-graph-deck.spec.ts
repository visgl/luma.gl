...
test('GPU Graph deck.gl effect composes actual GPU analytics, zero-copy selection, and pinned layout', async tapeTest => {
...
test('GPU Graph Deck effect executes real spatial indexing and community analytics on a bounded fixture', async tapeTest => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    tapeTest.comment('WebGPU is not available');
    tapeTest.end();
...
test('GPU Graph Deck preserves every original GPU vertex and edge while sampling only forces and visible edges', async tapeTest => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    tapeTest.comment('WebGPU is not available');
    tapeTest.end();
...
test('GPU Graph deck.gl renders real source-chunk layers and asynchronously picks stable GPU node IDs', async tapeTest => {
  ...
  try {
    ...
    tapeTest.ok(
      effect instanceof GPUGraphDeckEffect,
    ...
    tapeTest.deepEqual(
      ...
      ['gpu-graph-edges-0', 'gpu-graph-edges-2'],
      'the empty source batch is preserved and not rendered or concatenated'
    );
    tapeTest.ok(
      nodeLayer instanceof GPUGraphNodeLayer,
