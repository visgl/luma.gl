...
test('Integer/unorm formats round-trip', t => {
    const formats = ['rgba4unorm-webgl', 'rgb565unorm-webgl', 'rgb5a1unorm-webgl', 'rgb10a2unorm', 'rgb10a2uint'];
    const vals = [0.2, 0.5, 0.8, 1.0];
...
test('Precomputed integer/unorm decodes', t => {
    ...
    for (const { format, bits, expected } of cases) {
        const decoded = rgbaDecoder.decodeRGBA(bits, format);
...
test.skip('Float-packed formats round-trip', t => {
    const formats = ['rgb9e5ufloat', 'rg11b10ufloat'];
    const samples = [
        [0.1, 0.2, 0.3],
        [1, 2, 3],
        [0, 0, 0],
