...
import { decodeRGBA, decodeRGBAFloat, encodeRGBA, encodeRGBAFloat } from '@luma.gl/experimental';
...
test('Integer/unorm formats round-trip', t => {
  const formats: Array<'rgba4unorm-webgl' | 'rgb565unorm-webgl' | 'rgb5a1unorm-webgl' | 'rgb10a2unorm' | 'rgb10a2uint'> = [
  ...
  for (const fmt of formats) {
    const bits = encodeRGBA(vals, fmt);
    const decoded = decodeRGBA(bits, fmt);
...
test('Precomputed integer/unorm decodes', t => {
  ...
  for (const {format, bits, expected} of cases) {
    const decoded = decodeRGBA(bits, format as any);
...
test('Float-packed formats round-trip', t => {
  ...
  for (const fmt of formats) {
    for (const sample of samples) {
      const bits = encodeRGBAFloat(sample, fmt);
      const [r,g,b,a] = decodeRGBAFloat(bits, fmt);
...
test('Precomputed float-packed decodes', t => {
  // zero case
  t.deepEqual(decodeRGBAFloat(0, 'rgb9e5ufloat'), [0, 0, 0, 1], 'rgb9e5ufloat zero');
  t.deepEqual(decodeRGBAFloat(0, 'rg11b10ufloat'), [0, 0, 0, 1], 'rg11b10ufloat zero');

  // rgb9e5ufloat: mantR=512 => 1.0, mantG/B=0, exp=15 => value = mant/512 * 2^(15-15) = 1
  const rgb9e5Bits = (15 << 27) | (0 << 18) | (0 << 9) | 512;
  t.deepEqual(
    decodeRGBAFloat(rgb9e5Bits, 'rgb9e5ufloat'),
    [1, 0, 0, 1],
    'rgb9e5ufloat [1,0,0]'
  );
  ...
  t.deepEqual(
    decodeRGBAFloat(rg11Bits, 'rg11b10ufloat'),
    [1, 1, 1, 1],
    'rg11b10ufloat [1,1,1]'
  );
