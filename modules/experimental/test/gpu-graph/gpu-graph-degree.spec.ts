...
} from '../../src/gpu-graph/gpu-graph-degree-internals';
...
function createExecutionFixture(
  ...
  const degree = new GPUGraphDegree({topology, output, direction: scenario.direction});
  return {
    device,
    buffers,
    vectors,
...
function executeMetrics(fixture: DegreeExecutionFixture): void {
  const commandEncoder = fixture.device.createCommandEncoder({id: 'gpu-graph-degree-test'});
