...
import {
  ...
  LuDataFrameGroupedAggregationQuery,
  parameter,
  type LuDataFrameAggregationDefinitions,
  type LuDataFrameGroupedAggregationResult
...
describe('LuDataFrame immutable dense grouped-aggregation planning', () => {
  test('plans all supported statistics without allocating GPU resources or retaining source leases', () => {
    ...
    expect(grouped).toBeInstanceOf(LuDataFrameGroupByQuery);
    expect(aggregated).toBeInstanceOf(LuDataFrameGroupedAggregationQuery);
  ...
  test('infers dictionary domains and requires matching explicit unsigned group counts', () => {
    const fixture = createGroupSourceFixture([2]);
    const dictionarySource = new LuDataFrame({
      table: fixture.table,
      dictionaries: {category: {values: GROUP_LABELS, ordered: false}}
    });
    const plainSource = new LuDataFrame({table: fixture.table});
  ...
  test('rejects unsupported keys, hidden inputs, invalid operations, and non-float32 metrics', () => {
    ...
    const invalidOperation = {
      broken: {median: 'fare'}
    } as unknown as LuDataFrameAggregationDefinitions<GroupSourceColumns>;
...
function createGroupSourceFixture(
  ...
  const device = new NullDevice({id: 'ludf-group-aggregation-node-device'});
