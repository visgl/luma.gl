...
test('LuDataFrame preserves GPU source batches through borrowed projections', async testContext => {
  ...
  const fixture = createLuDataFrameFixture(device);
  ...
  let frame: LuDataFrame | undefined;
  const projections: LuDataFrame[] = [];

  try {
    frame = new LuDataFrame({table: fixture.table, ownership: 'borrowed'});
...
test('LuDataFrame defers owned GPU destruction until its last borrowed view', async testContext => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testContext.comment('WebGPU is not available');
    testContext.end();
  ...
  const frame = new LuDataFrame({table: fixture.table, ownership: 'owned'});
...
function createLuDataFrameFixture(device: Device): LuDataFrameFixture {
