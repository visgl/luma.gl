...
type LuDataFrameQueryFixture = {
  frame: LuDataFrame<LuDataFrameQuerySchema>;
  sourceBuffers: Buffer[];
};

test('LuDataFrame filters nullable GPU batches while preserving stable source rows', async testContext => {
  ...
  let compiled:
    | ReturnType<ReturnType<LuDataFrame<LuDataFrameQuerySchema>['filter']>['compile']>
    | undefined;
  ...
  try {
    ...
    const commandEncoder = device.createCommandEncoder({id: 'ludf-nullable-batch-filter'});
...
test('LuDataFrame expressions apply SQL-style nullable Boolean semantics and signed arithmetic', async testContext => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testContext.comment('WebGPU is not available');
    testContext.end();
  ...
  const nullableGraph = new GPUCommandGraph<LuDataFrameQueryParameters>(device, {
    id: 'ludf-nullable-boolean-semantics'
  });
  ...
  try {
    const commandEncoder = device.createCommandEncoder({id: 'ludf-nullable-and-signed'});
...
test('LuDataFrame reuses a compiled graph with ordered parameter updates in one encoder', async testContext => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testContext.comment('WebGPU is not available');
    testContext.end();
  ...
  const countSnapshots = compiled.selectedCounts.data.map((_, batchIndex) =>
    device.createBuffer({
      id: `ludf-first-count-${batchIndex}`,
  ...
  try {
    const commandEncoder = device.createCommandEncoder({id: 'ludf-two-parameter-encodes'});
...
test('LuDataFrame compiles schema-only empty queries and rejects unresolved nonempty validity', async testContext => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testContext.comment('WebGPU is not available');
    testContext.end();
  ...
  const schemaOnlyFrame = new LuDataFrame<LuDataFrameQuerySchema>({
    table: new GPUTable<LuDataFrameQuerySchema>({
      schema: {
        fields: [
  ...
  const emptyQuery = schemaOnlyFrame
    ...
    .compile(emptyGraph);
  const fixture = createLuDataFrameQueryFixture(device);
  const unresolvedFrame = new LuDataFrame<LuDataFrameQuerySchema>({
  ...
  try {
    ...
    const commandEncoder = device.createCommandEncoder({id: 'ludf-schema-only-encode'});
    emptyQuery.encode(commandEncoder);
    device.submit(commandEncoder.finish());

    const unresolvedGraph = new GPUCommandGraph<LuDataFrameQueryParameters>(device, {
      id: 'ludf-unresolved-validity'
    });
    const createBufferSpy = vi.spyOn(device, 'createBuffer');
    try {
      testContext.throws(
...
function createLuDataFrameQueryFixture(device: Device): LuDataFrameQueryFixture {
  ...
  const batches = fares.map((fareValues, batchIndex) => {
    const batch = new GPURecordBatch<LuDataFrameQuerySchema>({
      gpuData: {
        fare: createOwnedGPUData(
          device,
          sourceBuffers,
          `ludf-query-fare-${batchIndex}`,
          fareValues,
          'float32'
        ),
        category: createOwnedGPUData(
          device,
          sourceBuffers,
          `ludf-query-category-${batchIndex}`,
    ...
    fareValidityChunks.push(
      createOwnedGPUData(
        device,
        sourceBuffers,
        `ludf-query-fare-validity-${batchIndex}`,
    ...
    categoryValidityChunks.push(
      createOwnedGPUData(
        device,
        sourceBuffers,
        `ludf-query-category-validity-${batchIndex}`,
  ...
  return {
    frame: new LuDataFrame<LuDataFrameQuerySchema>({
      table: new GPUTable<LuDataFrameQuerySchema>({batches}),
      validity: {
        fare: new GPUVector<'uint32'>({
          type: 'data',
          name: 'ludf-query-fare-validity',
          format: 'uint32',
        ...
        category: new GPUVector<'uint32'>({
          type: 'data',
          name: 'ludf-query-category-validity',
          format: 'uint32',
