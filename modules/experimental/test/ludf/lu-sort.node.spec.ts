...
describe('LuDataFrame immutable stable scalar sorting', () => {
  ...
  test('clones immutable options and keeps precise projected and derived source output types', () => {
    ...
    expectTypeOf(projected.compile).returns.toEqualTypeOf<
      CompiledLuDataFrameSort<{score: 'float32'; signed: 'sint32'}>
    >();
    expectTypeOf(derived.compile).returns.toEqualTypeOf<
      CompiledLuDataFrameSort<{score: 'float32'; adjustedScore: 'float32'}>
    >();
  ...
  test('rejects unknown, hidden, vector-valued, and constant sort keys before GPU work', () => {
    ...
    const constantSource = new LuDataFrame({table: constantTable});
  ...
  test('rejects strided scalar sort keys before allocating or planning GPU output buffers', () => {
    ...
    const source = new LuDataFrame({table: fixture.table});
    const graph = new GPUCommandGraph<LuDataFrameQueryParameters>(fixture.device, {
      id: 'ludf-strided-sort-key'
    });
    const createBuffer = vi.spyOn(fixture.device, 'createBuffer');
  ...
  test('rejects non-closed ordering options and non-uint32 per-batch top-K limits', () => {
    ...
    for (const invalid of [
      ...
      expect(() => source.sortBy('score', invalid as LuDataFrameSortOptions)).toThrow(
        /direction|null|nan|algorithm/i
      );
    }
    expect(() => source.sortBy('score', null as unknown as LuDataFrameSortOptions)).toThrow(
      /options/i
    );
...
function createSortSourceFixture(
  ...
  const device = new NullDevice({id: 'ludf-stable-sort-node-device'});
