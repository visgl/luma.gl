...
import {
  ...
  type GPUDataFrameGlobalAggregationResult,
...
describe('GPUDataFrame immutable global-reduction planning', () => {
  test('plans every scalar statistic without GPU allocation, submission, or source retention', () => {
    ...
    expect(aggregated).toBeInstanceOf(GPUDataFrameAggregationQuery);
  ...
  test('rejects empty operations, hidden metrics, unsupported formats, and invalid operation shapes', () => {
    ...
    const unsupported = {
      broken: {median: 'fare'}
    } as unknown as GPUDataFrameGlobalAggregationDefinitions<AnalyticSourceColumns>;
  ...
  test('rejects strided scalar metrics before allocating global or histogram GPU outputs', () => {
    ...
    const source = new GPUDataFrame({table: fixture.table});
    const globalGraph = new GPUCommandGraph<GPUDataFrameQueryParameters>(fixture.device, {
      id: 'gpu-dataframe-strided-global-metric'
    });
    const histogramGraph = new GPUCommandGraph<GPUDataFrameQueryParameters>(fixture.device, {
      id: 'gpu-dataframe-strided-histogram-metric'
    });
    const createBuffer = vi.spyOn(fixture.device, 'createBuffer');
...
describe('GPUDataFrame immutable explicit-domain histogram planning', () => {
  test('deep-clones explicit domains and edges without allocating or retaining GPU resources', () => {
    ...
    expect(uniform).toBeInstanceOf(GPUDataFrameHistogramQuery);
    expect(irregular).toBeInstanceOf(GPUDataFrameHistogramQuery);
  ...
  test('supports exact typed floating, signed, unsigned, filtered, and derived histogram columns', () => {
    ...
    expectTypeOf(floating.compile).returns.toEqualTypeOf<CompiledGPUDataFrameHistogram>();
  ...
  test('rejects absent domains, invalid counts, nonrepresentable boundaries, and ambiguous options', () => {
    ...
    const mixed = {bins: 2, domain: [0, 2], edges: [0, 1, 2]} as GPUDataFrameHistogramOptions;
...
function createAnalyticSourceFixture(
  ...
  const device = new NullDevice({id: 'gpu-dataframe-analytic-reductions-node-device'});
