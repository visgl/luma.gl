...
describe('GPUDataFrame immutable derived-column planning', () => {
  test('plans chained derived fields without allocating GPU storage or retaining source leases', () => {
    ...
    expect(adjusted).toBeInstanceOf(GPUDataFrameQuery);
  ...
  test('retains precise source and derived scalar formats across typed chained projections', () => {
    ...
    >();
    expectTypeOf<
      GPUDataFrameDerivedColumnFormatForExpression<DerivedSourceColumns, 'distance'>
...
function createDerivedSourceFixture(
  ...
  const device = new NullDevice({id: 'gpu-dataframe-derived-node-device'});
