...
import {
  column,
  CompiledGPUDataFrameJoin,
  CompiledGPUDataFrameLookup,
  literal,
  GPUDataFrame,
...
describe('GPUDataFrame immutable unique-right joins and bounded lookups', () => {
  test('plans mismatched preserved batch topologies without GPU work or source retention', () => {
    ...
    const left = new GPUDataFrame({table: leftFixture.table, ownership: 'owned'});
    const right = new GPUDataFrame({table: rightFixture.table, ownership: 'owned'});
    ...
    expect(joined).toBeInstanceOf(GPUDataFrameJoinQuery);
    expect(lookup).toBeInstanceOf(GPUDataFrameLookupQuery);
  ...
  test('retains precise left/right schemas through filtered, projected, and derived join plans', () => {
    ...
    const left = new GPUDataFrame({table: leftFixture.table});
    const right = new GPUDataFrame({table: rightFixture.table});
    const filtered = left
    ...
    expectTypeOf(joined.compile).returns.toEqualTypeOf<
      CompiledGPUDataFrameJoin<{key: 'uint32'; amount: 'float32'}, RightJoinColumns>
    >();
    expectTypeOf(derived.compile).returns.toEqualTypeOf<
      CompiledGPUDataFrameLookup<{shiftedKey: 'uint32'; amount: 'float32'}, RightJoinColumns>
    >();
  ...
  test('plans frozen left outer, semi, and anti joins without touching either source', () => {
    ...
    expect(plans.every(plan => plan instanceof GPUDataFrameJoinQuery)).toBe(true);
    ...
    expectTypeOf(plans[1].compile).returns.toEqualTypeOf<
      CompiledGPUDataFrameJoin<LeftJoinColumns, RightJoinColumns>
    >();
  ...
  test('clones bounded options and safely clamps default cumulative hash probing', () => {
    ...
    const left = new GPUDataFrame({table: leftFixture.table});
    const right = new GPUDataFrame({table: rightFixture.table});
    const mutableOptions: GPUDataFrameJoinOptions<'key', 'lookupKey'> = {
      leftOn: 'key',
      rightOn: 'lookupKey',
      capacity: 0,
  ...
  test('rejects unknown, hidden, signed, floating, or constant keys without GPU allocation', () => {
    ...
    const constantLeft = new GPUDataFrame({table: constantLeftTable});
    expect(() =>
    ...
    const constantRight = new GPUDataFrame({table: constantRightTable});
  ...
  test('rejects unsafe index capacities, unbounded probes, and invalid per-batch output limits', () => {
    ...
    const lookupWithCapacity = {
      ...
      rightOn: 'lookupKey',
      capacity: 1
    } as GPUDataFrameLookupOptions<'key', 'lookupKey'>;
...
function createJoinSourceFixture<Columns extends GPUTypeMap>(
  ...
  const device = new NullDevice({id: `gpu-dataframe-${side}-join-node-device`});
