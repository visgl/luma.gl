...
describe('GPUDataFrame immutable filter planning', () => {
  test('plans filters and projections without creating GPU resources or retaining source leases', () => {
    ...
    expect(query).toBeInstanceOf(GPUDataFrameQuery);
  ...
  test('preserves predicate inputs that are absent from the eventual output projection', () => {
    ...
    expectTypeOf(query).toEqualTypeOf<GPUDataFrameQuery<QueryColumns, 'category'>>();
...
function createQueryFixture(
  ...
  const device = new NullDevice({id: 'gpu-dataframe-query-node-device'});
