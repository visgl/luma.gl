...
describe('@luma.gl/experimental/gpu-dataframe package boundary', () => {
  test('publishes one optional, Arrow-free, side-effect-free package entry point', () => {
    ...
    expect(packageJson.exports?.['./gpu-dataframe']).toEqual({
      import: './dist/gpu-dataframe/index.js',
  ...
  test('keeps dataframe runtime exports out of the experimental root entry point', () => {
    expect(gpuDataFrameModule.GPUDataFrame).toBe(GPUDataFrame);
    expect(typeof gpuDataFrameModule.GPUDataFrame).toBe('function');
    expect('GPUDataFrame' in experimentalModule).toBe(false);
  });
});

describe('GPUDataFrame source-table inspection', () => {
  test('preserves schema, source batches, chunk boundaries, and stable source identities', () => {
    ...
    const frame = new GPUDataFrame({table: fixture.table});
    ...
    expect(frame.schema.metadata.get('dataset')).toBe('gpu-dataframe-fixture');
...
describe('GPUDataFrame non-destructive projections', () => {
  ...
  test('isolates projected schema and batch metadata from source and sibling views', () => {
    ...
    expect(fixture.table.schema.metadata.get('dataset')).toBe('gpu-dataframe-fixture');
    ...
    expect(siblingProjection.schema.metadata.get('dataset')).toBe('gpu-dataframe-fixture');
...
describe('GPUDataFrame explicit validity and dictionaries', () => {
  ...
  test('rejects missing columns and validity vectors with incompatible row or chunk topology', () => {
    ...
    expect(
      () => new GPUDataFrame({table: fixture.table, validity: {missing: missingColumnValidity}})
    ).toThrow(/validity|column|missing/i);
    expect(
      () => new GPUDataFrame({table: fixture.table, validity: {fare: invalidRowCount}})
    ).toThrow(/validity|length|rows/i);
    expect(
      () => new GPUDataFrame({table: fixture.table, validity: {fare: invalidChunkCount}})
    ).toThrow(/validity|batch|chunk/i);
    expect(
      () => new GPUDataFrame({table: fixture.table, validity: {fare: invalidChunkLengths}})
    ).toThrow(/validity|batch|chunk/i);
    expect(
      () => new GPUDataFrame({table: fixture.table, dictionaries: {missing: ['unknown']}})
  ...
  test('rejects non-uint32 validity vectors and sidecars attached to constant columns', () => {
    ...
    expect(
      () =>
        ...
        new GPUDataFrame({table: fixture.table, validity: {fare: fixture.table.gpuVectors['fare']}})
    ).toThrow(/validity|uint32/i);
    expect(
      () => new GPUDataFrame({table: fixture.table, validity: {radius: constantValidity}})
...
describe('GPUDataFrame ownership and destruction', () => {
  ...
  test('does not destroy borrowed source storage or borrowed validity sidecars', () => {
    ...
    const frame = new GPUDataFrame({table: fixture.table, validity: {fare: validity}});
  ...
  test('rejects invalid runtime ownership values without taking ownership of source buffers', () => {
    ...
    expect(
      () =>
        // @ts-expect-error Runtime validation also protects callers without TypeScript.
        new GPUDataFrame({table: fixture.table, ownership: 'shared'})
...
type GPUDataFrameFixtureOptions = {
...
function createDataFrameFixture(
  ...
  {nullableFare = false, constants}: GPUDataFrameFixtureOptions = {}
): GPUDataFrameFixture {
  const device = new NullDevice({id: 'gpu-dataframe-foundation-fixture'});
  const metadata = new Map([['dataset', 'gpu-dataframe-fixture']]);
