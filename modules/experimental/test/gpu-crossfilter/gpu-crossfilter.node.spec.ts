...
const LUX_FILTER_RUNTIME_EXPORTS = ['GPUCrossfilter', 'GPUCrossfilterSelection'] as const;

describe('@luma.gl/experimental/gpu-crossfilter package boundary', () => {
  test('publishes one side-effect-free conditional experimental subpath', () => {
    ...
    expect(packageJson.exports?.['./gpu-crossfilter']).toEqual({
      import: './dist/gpu-crossfilter/index.js',
      require: './dist/gpu-crossfilter/index.cjs',
      types: './dist/gpu-crossfilter/index.d.ts'
    });
  });

  test('keeps linked-filter runtime exports on their optional subpath', () => {
    for (const exportName of LUX_FILTER_RUNTIME_EXPORTS) {
      expect(typeof gpuCrossfilterModule[exportName]).toBe('function');
      expect(exportName in experimentalModule).toBe(false);
    }
  });
});

describe('GPUCrossfilterSelection control-state contracts without GPU execution', () => {
  test('preserves full uint32 precision and destroys only its private control buffer', async () => {
    ...
    const selection = new GPUCrossfilterSelection(fixture.graph, {
      id: 'unsigned',
      kind: 'range',
      input
    });
  ...
  test('stores signed and floating-point endpoints using native scalar bit patterns', async () => {
    ...
    const createBufferSpy = vi.spyOn(fixture.device, 'createBuffer');
    const signedSelection = new GPUCrossfilterSelection(fixture.graph, {
      id: 'signed',
      kind: 'range',
      input: signedInput
    });
    const signedState = createBufferSpy.mock.results.at(-1)?.value;
    const floatingSelection = new GPUCrossfilterSelection(fixture.graph, {
      id: 'floating',
      kind: 'range',
      input: floatingInput
    });
  ...
  test('retains empty vector chunks and emits passes only for nonempty source chunks', () => {
    ...
    expect(addComputePassSpy.mock.calls.map(([pass]) => pass.id)).toEqual([
      'gpu-crossfilter-chunked-chunk-0',
      'gpu-crossfilter-chunked-chunk-2'
    ]);
  ...
  test('rejects mismatched bounds, mismatched caller masks, and aliased output buffers', () => {
    ...
    expect(
      () =>
        new GPUCrossfilterSelection(fixture.graph, {
  ...
  test('controller ownership leaves imported source buffers alive after destruction', () => {
    ...
    const sourceBuffer = fixture.buffers[0];
    const controller = new GPUCrossfilter(fixture.graph, {
      id: 'dashboard',
...
describe('GPUCrossfilter controller contracts without GPU execution', () => {
  test('rejects excluded histogram and group views with different source row counts', () => {
    ...
    expect(
      () =>
        new GPUCrossfilter(fixture.graph, {
...
function createScalarView<Format extends GPUCrossfilterScalarFormat>(
...
function createScalarVector<Format extends GPUCrossfilterScalarFormat>(
  fixture: GraphFixture,
  id: string,
