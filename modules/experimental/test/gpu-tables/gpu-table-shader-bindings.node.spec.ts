...
it('GPUTableShaderBindings binds complete fixed-size-list rows without trailing padding', () => {
  ...
  expect(embeddings.valueLength, 'logical rows expose flattened scalar element counts').toBe(6);
  expect(embeddings.rowByteLength, 'row payload excludes physical padding').toBe(12);
  expect(shaderBindings.bufferLayout, 'storage-only inputs have no vertex attributes').toEqual([]);
  expect(
