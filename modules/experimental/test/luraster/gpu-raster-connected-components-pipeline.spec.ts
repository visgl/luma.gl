...
test('LuRaster connected components preserve exact 4/8 connectivity, sparse roots, and independent nodata', async testCase => {
...
test('LuRaster threshold, binary closing, and deterministic component roots re-encode as one GPU pipeline', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster insufficient component rounds clear all dependent labels, validity, and histogram bins', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
