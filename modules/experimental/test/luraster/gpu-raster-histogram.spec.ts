...
test('LuRasterHistogram excludes finite raw nodata and preserves offset-aligned source masks', async testCase => {
...
test('LuRasterHistogram retains exact signed and unsigned integer nodata sentinels', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRasterHistogram clears all-invalid outputs and reuses one compiled graph', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
