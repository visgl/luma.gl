...
test('LuRaster composes calibrated NDVI, masked extent, and histogram without intermediate readback', async testCase => {
...
test('LuRaster band math evaluates every calibrated operation, honors offsets, and reuses graphs', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster NDVI rejects nonfinite and epsilon-sized denominators without implicit clamping', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster pointwise contributors reject incompatible grids, ownership, output aliases, and options', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
