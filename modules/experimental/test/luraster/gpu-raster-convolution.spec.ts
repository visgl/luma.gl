...
test('LuRasterConvolution preserves signed impulse responses and rectangular ramp derivatives', async testCase => {
...
test('LuRasterGaussianBlur matches the full normalized outer-product CPU reference', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRasterBoxBlur preserves constant scenes and matches odd-size bordered ramp averages', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster separable smoothing propagates or renormalizes nodata without reviving invalid centers', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster smoothing reuses graph-planned scratch across chained reusable operators', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
