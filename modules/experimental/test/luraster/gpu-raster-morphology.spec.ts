...
test('LuRaster binary morphology removes islands, fills holes, and distinguishes square/Manhattan footprints', async testCase => {
...
test('LuRaster grayscale extrema preserve signed values and apply native integer calibration exactly once', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster morphology honors all borders, constant binary values, and propagate/ignore validity semantics', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster radius-zero composites are scratch-free calibrated/binary identity passes', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster chained opening/closing reuse scratch and preserve repeatable borrowed ownership', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
