...
test('LuRaster tiled halos match monolithic pointwise, smoothing, derivatives, and morphology', async testCase => {
...
test('LuRaster cumulative halos preserve ragged anisotropic-overview seams and masked boundaries', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
