...
test('LuRaster grouped measurements independently preserve geometry, calibrated intensity validity, and double-precision affine centroids', async testCase => {
...
test('LuRaster classification, connected roots, dense labels, and regional measurements compose into one GPU graph', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster grouped outputs fail closed on nonconvergence, truncation, and invalid selected capacity', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster regional pixel centers and overview affines preserve coordinate-system area semantics', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
