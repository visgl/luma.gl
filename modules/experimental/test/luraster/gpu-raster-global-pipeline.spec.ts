...
test('LuRaster global replay preserves calibrated monolithic statistics and Otsu under tile reordering', async testCase => {
...
test('LuRaster merge-only re-encoding retains global history and saturates sticky overflow safely', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
