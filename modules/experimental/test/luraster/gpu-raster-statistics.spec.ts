...
test('LuRasterStatistics publishes calibrated, nodata-aware count, sum, mean, and extent', async testCase => {
...
test('LuRasterStatistics clears all-invalid outputs and reuses one compiled reduction graph', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRasterStatistics consumes threshold masks and rejects calibration overflow', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
