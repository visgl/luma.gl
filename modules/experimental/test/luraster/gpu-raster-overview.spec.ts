...
test('LuRaster analytical overview skips invalid/nodata samples and preserves ragged weighted coverage', async testCase => {
...
test('LuRaster analytical overviews merge child sums/counts and reject dishonest coverage bounds', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster categorical overviews preserve exact unsigned/signed labels and deterministic nodata policies', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
