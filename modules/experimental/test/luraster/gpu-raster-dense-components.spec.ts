...
import {GPURasterDenseComponents} from '@luma.gl/experimental/luraster';
...
test('LuRaster dense representatives preserve real background and reject exact malformed unsigned roots', async testCase => {
...
test('LuRaster hierarchical dense scans clamp maximal checkerboards at zero, partial, and full capacities', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRaster dense graph replay clears convergence failures and resets count, overflow, validity, and roots', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
