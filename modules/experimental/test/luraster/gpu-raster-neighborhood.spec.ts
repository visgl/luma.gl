...
test('LuRasterNeighborhood matches edge/corner/one-pixel CPU oracles for every border mode', async testCase => {
...
test('LuRasterNeighborhood handles odd workgroups, nodata, calibrated samples, and strict/renormalized policies', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
...
test('LuRasterNeighborhood preserves raw signed/unsigned nodata and reusable borrowed buffers', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
