...
test('LuRaster analytical pyramids merge uneven masked coverage instead of averaging overview means', async testCase => {
...
test('LuRaster categorical overview policies preserve exact labels and expose valid alternatives', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
