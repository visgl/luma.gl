...
test('LuRaster tile halo gather preserves exact scalar formats, masks, raw nodata, and offset guards', async testCase => {
...
test('LuRaster tile halo pins diagonal/ragged WebGPU imports through core publication and fencing', async testCase => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    testCase.comment('WebGPU is not available');
    testCase.end();
