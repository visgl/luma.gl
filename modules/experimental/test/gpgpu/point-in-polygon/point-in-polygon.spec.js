...
const TEST_CASES = [
  ...
  },
  {
    name: 'mix',
    polygons: [
      [[-0.35, -0.35], [0.45, -0.35], [0.35, 0.45], [-0.5, 0.35], [-0.35, -0.35]]
    ],
    points: [
      [-0.35, -0.35],
      [0.25, -0.45],
...
test('gpgpu#point-in-polygon CPU vs GPU', t => {
  ...
  TEST_CASES.forEach(tc => {
    ...
    scales.forEach(scale => {
      const inPolygons = tc.polygons || [tc.polygon];
      // const polygon = tc.polygon.map(xy => [xy[0]*scale, xy[1]*scale]);
      const polygons = inPolygons; // .map(p => p.map(xy => [xy[0]*scale, xy[1]*scale]));
