...
} from '../../src/lugraph/lu-graph-breadth-first-search-internals';
...
test('LuGraphBreadthFirstSearch plans bounded three-dimensional seed and vertex dispatch', tapeTest => {
  tapeTest.deepEqual(getLuGraphBreadthFirstSearchDispatchLayout(0, 2), {x: 1, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphBreadthFirstSearchDispatchLayout(512, 2), {x: 2, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphBreadthFirstSearchDispatchLayout(513, 2), {x: 2, y: 2, z: 1});
  tapeTest.deepEqual(getLuGraphBreadthFirstSearchDispatchLayout(1025, 2), {x: 2, y: 2, z: 2});
  tapeTest.throws(() => getLuGraphBreadthFirstSearchDispatchLayout(2049, 2), /3D dispatch limit/);
  tapeTest.end();
});

for (const scenario of searchScenarios) {
  test(`LuGraphBreadthFirstSearch GPU traversal: ${scenario.name}`, async tapeTest => {
...
test('LuGraphBreadthFirstSearch rereads dynamic seeds, depth, and sources on every encoding', async tapeTest => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    tapeTest.comment('WebGPU is not available');
    tapeTest.end();
...
function executeSearch(fixture: SearchExecutionFixture): void {
  const commandEncoder = fixture.device.createCommandEncoder({id: 'lu-graph-breadth-first-test'});
