...
import {
  ...
  type LuGraphBreadthFirstSearchDirection,
  type LuGraphBreadthFirstSearchProps
...
describe('LuGraphBreadthFirstSearch public API and caller ownership', () => {
  test('publishes shortest-path traversal only through the optional luGraph subpath', () => {
    expect(typeof LuGraphBreadthFirstSearch).toBe('function');
    expect('LuGraphBreadthFirstSearch' in experimentalModule).toBe(false);
  });
  ...
  test('preserves topology, chunked seeds, outputs, and controls without executing GPU work', () => {
    ...
    const search = new LuGraphBreadthFirstSearch({...props, id: 'borrowed-search'});
  ...
  test('accepts empty graphs, empty chunked seeds, and optional masks', () => {
    ...
    const search = new LuGraphBreadthFirstSearch({...props, seeds, maxDepth: 0});
...
describe('LuGraphBreadthFirstSearch direction and depth contracts', () => {
  test.each([
    'outgoing',
    'incoming',
    'both'
    ...
    expect(new LuGraphBreadthFirstSearch({...props, direction}).direction).toBe(direction);
  });
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, direction})).toThrow(
  ...
  test.each([
    ...
    const fixture = createSearchFixture();
    const props = createSearchProps(fixture, {reverse: true});
    const direction = invalidDirection as LuGraphBreadthFirstSearchDirection;

    expect(() => new LuGraphBreadthFirstSearch({...props, direction})).toThrow(/direction/);
  });
  ...
  test.each([0, 1, 1024])('accepts a bounded maximum traversal depth: %i', maxDepth => {
    ...
    expect(new LuGraphBreadthFirstSearch({...props, maxDepth}).maxDepth).toBe(maxDepth);
  });
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, maxDepth})).toThrow(
      /maxDepth|depth|1024/
    );
  });
});

describe('LuGraphBreadthFirstSearch vector and control validation', () => {
  test.each([
    'distances',
    'predecessors',
    'mask'
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, [outputName]: wrongFormat})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, [outputName]: shortOutput})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, [outputName]: partitioned})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, predecessors})).toThrow(
  ...
  test('requires packed uint32 seed chunks and validates intentionally empty chunks', () => {
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, seeds: wrongFormat})).toThrow(
      /seeds|uint32|packed/
    );
    Object.defineProperty(props.seeds.data[1], 'format', {value: 'float32'});
    expect(() => new LuGraphBreadthFirstSearch(props)).toThrow(/seeds|uint32|packed|chunk/);
  });
  ...
  test.each([
    ...
    for (const length of [0, 2]) {
      ...
      expect(() => new LuGraphBreadthFirstSearch({...props, [controlName]: control})).toThrow(
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, [controlName]: wrongFormat})).toThrow(
  ...
  test('accepts uint32-aligned seed, output, and scalar views at non-256-byte offsets', () => {
    ...
    const search = new LuGraphBreadthFirstSearch({...props, seeds, distances, seedCount});
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, distances})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphBreadthFirstSearch({...props, [outputName]: output})).toThrow(
...
function getSearchInputVector(props: LuGraphBreadthFirstSearchProps, name: string): GPUVector {
