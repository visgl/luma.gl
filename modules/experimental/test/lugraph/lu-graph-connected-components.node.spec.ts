...
import {
  ...
  type LuGraphConnectedComponentsProps
...
describe('LuGraphConnectedComponents public contract and ownership', () => {
  test('keeps weak connected components isolated in the optional luGraph entry point', () => {
    expect(typeof LuGraphConnectedComponents).toBe('function');
    expect('LuGraphConnectedComponents' in experimentalModule).toBe(false);
  });
  ...
  test('preserves graph topology and caller outputs without allocations, submission, or readback', () => {
    ...
    const components = new LuGraphConnectedComponents({...props, id: 'borrowed-components'});
  ...
  test('accepts a directed graph without reverse adjacency and without optional convergence output', () => {
    ...
    const components = new LuGraphConnectedComponents(props);
...
describe('LuGraphConnectedComponents iteration and vector validation', () => {
  test.each([1, 32, 1024])('accepts a positive bounded iteration count: %i', iterations => {
    const fixture = createComponentsFixture();
    const props = createComponentsProps(fixture);

    expect(new LuGraphConnectedComponents({...props, iterations}).iterations).toBe(iterations);
  });
  ...
  test.each([
    ...
    expect(() => new LuGraphConnectedComponents({...props, iterations})).toThrow(
  ...
  test.each([5, 7])('requires exactly one output row per graph vertex: %i', length => {
    ...
    expect(() => new LuGraphConnectedComponents({...props, output})).toThrow(
  ...
  test.each([0, 2])('requires exactly one convergence status row: %i', length => {
    ...
    expect(() => new LuGraphConnectedComponents({...props, converged})).toThrow(
  ...
  test('requires packed uint32 convergence status with exactly one chunk', () => {
    ...
    expect(() => new LuGraphConnectedComponents({...props, converged: wrongFormat})).toThrow(
      /converged|uint32|packed/
    );
    expect(() => new LuGraphConnectedComponents({...props, converged: partitioned})).toThrow(
  ...
  test('accepts uint32-aligned component and status ranges at non-256-byte offsets', () => {
    ...
    const components = new LuGraphConnectedComponents({...props, output, converged});
  ...
  test('rejects convergence status aliasing component labels or topology allocations', () => {
    ...
    expect(() => new LuGraphConnectedComponents({...props, converged: outputAlias})).toThrow(
      /converged|distinct|physical|allocation/
    );
    expect(() => new LuGraphConnectedComponents({...props, converged: topologyAlias})).toThrow(
      /converged|distinct|physical|allocation/
    );
  });
...
): LuGraphConnectedComponentsProps {
