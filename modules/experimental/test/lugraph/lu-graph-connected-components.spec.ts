...
} from '../../src/lugraph/lu-graph-connected-components-internals';
...
test('LuGraphConnectedComponents plans bounded three-dimensional vertex dispatch', tapeTest => {
  tapeTest.deepEqual(getLuGraphConnectedComponentsDispatchLayout(0, 2), {x: 1, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphConnectedComponentsDispatchLayout(512, 2), {x: 2, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphConnectedComponentsDispatchLayout(513, 2), {x: 2, y: 2, z: 1});
  tapeTest.deepEqual(getLuGraphConnectedComponentsDispatchLayout(1025, 2), {x: 2, y: 2, z: 2});
  tapeTest.throws(() => getLuGraphConnectedComponentsDispatchLayout(2049, 2), /3D dispatch limit/);
  tapeTest.end();
});

for (const scenario of componentsScenarios) {
  test(`LuGraphConnectedComponents GPU labeling: ${scenario.name}`, async tapeTest => {
...
test('LuGraphConnectedComponents rebuilds weak labels after source updates without hidden execution', async tapeTest => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    tapeTest.comment('WebGPU is not available');
    tapeTest.end();
...
function executeComponents(fixture: ComponentsExecutionFixture): void {
  const commandEncoder = fixture.device.createCommandEncoder({id: 'lu-graph-components-test'});
