...
import {
  addLuGraphSpatialForceLayoutToGraphWithDispatchLimit,
  getLuGraphSpatialForceLayoutDispatchLayout
} from '../../src/lugraph/lu-graph-spatial-force-layout-internals';
...
type SpatialExecutionFixture = {
  ...
  spatial: LuGraphSpatialForceLayout;
...
test('LuGraphSpatialForceLayout plans bounded 3D vertex, grid, and centroid dispatch', tapeTest => {
  tapeTest.deepEqual(getLuGraphSpatialForceLayoutDispatchLayout(0, 2), {x: 1, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphSpatialForceLayoutDispatchLayout(512, 2), {x: 2, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphSpatialForceLayoutDispatchLayout(513, 2), {x: 2, y: 2, z: 1});
  tapeTest.deepEqual(getLuGraphSpatialForceLayoutDispatchLayout(1025, 2), {x: 2, y: 2, z: 2});
  tapeTest.throws(() => getLuGraphSpatialForceLayoutDispatchLayout(2049, 2), /3D dispatch limit/);
  tapeTest.end();
});

for (const scenario of spatialScenarios) {
  test(`LuGraphSpatialForceLayout GPU spatial physics: ${scenario.name}`, async tapeTest => {
...
test('LuGraphSpatialForceLayout repeatedly rebuilds spatial cells across warm starts and deterministic reset', async tapeTest => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    tapeTest.comment('WebGPU is not available');
    tapeTest.end();
...
function createExecutionFixture(
  ...
  const spatial = new LuGraphSpatialForceLayout({
    layout,
    gridSize,
...
function executeSpatialLayout(fixture: SpatialExecutionFixture): void {
  const commandEncoder = fixture.device.createCommandEncoder({
    id: 'lu-graph-spatial-force-layout-test'
  });
