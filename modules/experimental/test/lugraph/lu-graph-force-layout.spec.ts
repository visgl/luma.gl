...
import {
  addLuGraphForceLayoutToGraphWithDispatchLimit,
  getLuGraphForceLayoutDispatchLayout
} from '../../src/lugraph/lu-graph-force-layout-internals';
...
test('LuGraphForceLayout plans bounded three-dimensional exact repulsion dispatch', tapeTest => {
  tapeTest.deepEqual(getLuGraphForceLayoutDispatchLayout(0, 2), {x: 1, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphForceLayoutDispatchLayout(512, 2), {x: 2, y: 1, z: 1});
  tapeTest.deepEqual(getLuGraphForceLayoutDispatchLayout(513, 2), {x: 2, y: 2, z: 1});
  tapeTest.deepEqual(getLuGraphForceLayoutDispatchLayout(1025, 2), {x: 2, y: 2, z: 2});
  tapeTest.throws(() => getLuGraphForceLayoutDispatchLayout(2049, 2), /3D dispatch limit/);
  tapeTest.end();
});

for (const scenario of layoutScenarios) {
  test(`LuGraphForceLayout exact GPU physics: ${scenario.name}`, async tapeTest => {
...
test('LuGraphForceLayout progressively warm-starts and repeats deterministic seeded reset', async tapeTest => {
  const device = await getWebGPUTestDevice();
  if (!device) {
    tapeTest.comment('WebGPU is not available');
    tapeTest.end();
...
function executeLayout(fixture: LayoutExecutionFixture): void {
  const commandEncoder = fixture.device.createCommandEncoder({id: 'lu-graph-force-layout-test'});
