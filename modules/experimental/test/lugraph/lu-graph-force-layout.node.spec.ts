...
describe('LuGraphForceLayout optional API and render-ready caller-owned buffers', () => {
  test('exports exact force-directed layout only through the optional luGraph subpath', () => {
    expect(typeof LuGraphForceLayout).toBe('function');
    expect('LuGraphForceLayout' in experimentalModule).toBe(false);
  });
  ...
  test('preserves topology, positions, velocities, controls, and all default physics', () => {
    ...
    const layout = new LuGraphForceLayout({...props, id: 'borrowed-render-layout'});
  ...
  test('requires reverse directed adjacency and accepts undirected or weighted topology', () => {
    ...
    expect(() => new LuGraphForceLayout(directed)).toThrow(/directed|reverse|adjacency/);
    expect(new LuGraphForceLayout(undirected).topology.reverse).toBeUndefined();
    expect(new LuGraphForceLayout(weighted).topology.graph.edgeWeights).toBeDefined();
  });
  ...
  test('accepts empty render vectors with a caller-owned optional reset scalar', () => {
    ...
    const layout = new LuGraphForceLayout(props);
...
describe('LuGraphForceLayout bounded deterministic simulation parameters', () => {
  test.each([
    0, 1, 0xffffffff
  ])('accepts an unsigned deterministic initialization seed: %i', seed => {
    const fixture = createLayoutFixture();
    expect(new LuGraphForceLayout({...createLayoutProps(fixture), seed}).seed).toBe(seed);
  });
  ...
  test.each([
    ...
    const fixture = createLayoutFixture();
    expect(() => new LuGraphForceLayout({...createLayoutProps(fixture), seed})).toThrow(
      /seed|uint32/
    );
  });
  ...
  test.each([1, 4, 1024])('accepts a bounded iteration count per frame: %i', iterationsPerFrame => {
    const fixture = createLayoutFixture();
    expect(
      new LuGraphForceLayout({...createLayoutProps(fixture), iterationsPerFrame}).iterationsPerFrame
  ...
  test.each([
    ...
    const fixture = createLayoutFixture();
    expect(
      () => new LuGraphForceLayout({...createLayoutProps(fixture), iterationsPerFrame})
  ...
  test.each([
    ...
    const fixture = createLayoutFixture();
    expect(new LuGraphForceLayout({...createLayoutProps(fixture), [parameter]: 0})[parameter]).toBe(
      0
    );
    expect(new LuGraphForceLayout({...createLayoutProps(fixture), [parameter]: 2})[parameter]).toBe(
      2
    );
  });
  ...
  test.each([
    ...
    const fixture = createLayoutFixture();
    expect(
      () => new LuGraphForceLayout({...createLayoutProps(fixture), [parameter]: value})
  ...
  test.each([0, 0.5, 1])('accepts damping factors in the closed unit interval: %s', damping => {
    const fixture = createLayoutFixture();
    expect(new LuGraphForceLayout({...createLayoutProps(fixture), damping}).damping).toBe(damping);
  });
});

describe('LuGraphForceLayout packed vectors and physical allocation safety', () => {
  test.each([
    'positions',
    'velocities'
    ...
    expect(() => new LuGraphForceLayout({...props, [field]: output})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphForceLayout({...props, [field]: wrongFormat})).toThrow(
      new RegExp(`${field}|float32x2|packed`)
    );
    expect(() => new LuGraphForceLayout({...props, [field]: partitioned})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphForceLayout({...props, positions})).toThrow(
  ...
  test('accepts directly renderable float32x2 views at four-byte storage offsets', () => {
    ...
    const layout = new LuGraphForceLayout({...props, positions, velocities});
  ...
  test('requires render positions to support both STORAGE and VERTEX usages', () => {
    ...
    expect(() => new LuGraphForceLayout({...props, positions: withoutVertex})).toThrow(
      /positions|VERTEX|vertex|render/
    );
    expect(() => new LuGraphForceLayout({...props, positions: withoutStorage})).toThrow(
  ...
  test('requires writable velocities to support STORAGE usage', () => {
    ...
    expect(() => new LuGraphForceLayout({...props, velocities})).toThrow(
  ...
  test.each([5, 7])('requires a uint32 pinned flag per vertex: %i', length => {
    ...
    expect(() => new LuGraphForceLayout({...props, pinned})).toThrow(/pinned|row|vertexCount/);
  });
  ...
  test('rejects non-uint32 and partitioned pinned masks', () => {
    ...
    expect(
      () => new LuGraphForceLayout({...props, pinned: wrongFormat as GPUVector<'uint32'>})
    ).toThrow(/pinned|uint32|packed/);
    expect(() => new LuGraphForceLayout({...props, pinned: partitioned})).toThrow(
  ...
  test.each([0, 2])('requires exactly one uint32 reset scalar: %i', length => {
    ...
    expect(() => new LuGraphForceLayout({...props, reset})).toThrow(/reset|one|row|scalar/);
  });
  ...
  test('rejects float and partitioned deterministic reset controls', () => {
    ...
    expect(
      () => new LuGraphForceLayout({...props, reset: wrongFormat as GPUVector<'uint32'>})
    ).toThrow(/reset|uint32|packed/);
    expect(() => new LuGraphForceLayout({...props, reset: partitioned})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphForceLayout({...props, [field]: alias})).toThrow(
      /distinct|physical|allocation/
    );
  });
