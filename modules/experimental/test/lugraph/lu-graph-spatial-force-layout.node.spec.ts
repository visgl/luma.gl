...
import {
  ...
  type LuGraphSpatialForceLayoutProps
...
describe('LuGraphSpatialForceLayout composition and caller-owned grid resources', () => {
  test('exposes spatial acceleration only through the optional luGraph package entry', () => {
    expect(typeof LuGraphSpatialForceLayout).toBe('function');
    expect('LuGraphSpatialForceLayout' in experimentalModule).toBe(false);
  });
  ...
  test('retains the exact base layout and every grid buffer without hidden GPU operations', () => {
    ...
    const spatial = new LuGraphSpatialForceLayout({...props, id: 'borrowed-spatial-layout'});
  ...
  test('accepts empty graphs and zero-capacity caller-owned vertex-ID indexes', () => {
    ...
    const spatial = new LuGraphSpatialForceLayout(props);
...
describe('LuGraphSpatialForceLayout finite grid and honest approximation contracts', () => {
  test.each([
    0, 0.001, 0.6, 1, 100
  ])('accepts a nonnegative finite opening criterion: %s', theta => {
    const fixture = createSpatialFixture();
    expect(new LuGraphSpatialForceLayout({...createSpatialProps(fixture), theta}).theta).toBe(
      theta
    );
  });
  ...
  test.each([
    ...
    const fixture = createSpatialFixture();
    expect(() => new LuGraphSpatialForceLayout({...createSpatialProps(fixture), theta})).toThrow(
  ...
  test.each([
    ...
    const fixture = createSpatialFixture();
    expect(
      new LuGraphSpatialForceLayout({...createSpatialProps(fixture), nearCellRadius}).nearCellRadius
    ).toBe(nearCellRadius);
  });
  ...
  test.each([
    ...
    const fixture = createSpatialFixture();
    expect(
      () => new LuGraphSpatialForceLayout({...createSpatialProps(fixture), nearCellRadius})
...
describe('LuGraphSpatialForceLayout packed index outputs and physical allocation safety', () => {
  test.each([6, 8])('requires one exclusive offset per cell plus a trailing total: %i', length => {
    ...
    expect(() => new LuGraphSpatialForceLayout({...props, cellOffsets})).toThrow(
  ...
  test.each([5, 7])('requires one float32x2 center per row-major grid cell: %i', length => {
    ...
    expect(() => new LuGraphSpatialForceLayout({...props, cellCenters})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphSpatialForceLayout({...props, [name]: wrongFormat})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphSpatialForceLayout({...props, [name]: vector})).toThrow(
  ...
  test.each(['count', 'overflow'] as const)('requires exactly one uint32 %s status row', name => {
    ...
    for (const length of [0, 2]) {
      ...
      expect(() => new LuGraphSpatialForceLayout({...props, [name]: status})).toThrow(
  ...
  test('accepts four-byte offsets for packed scalar and float32x2 index vectors', () => {
    ...
    const spatial = new LuGraphSpatialForceLayout({...props, cellOffsets, cellCenters, count});
  ...
  test.each([
    ...
    expect(() => new LuGraphSpatialForceLayout({...props, vertexIds})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphSpatialForceLayout({...props, [name]: alias})).toThrow(
      /distinct|physical|allocation/
    );
  });
...
function createSpatialProps(
...
): LuGraphSpatialForceLayoutProps {
  ...
  const reset = options.reset
    ...
    : undefined;
  const layout = new LuGraphForceLayout({topology, positions, velocities, pinned, reset});
...
function getExistingVector(layout: LuGraphForceLayout, name: string): GPUVector {
