...
import {
  ...
  type LuGraphModularityOptimizationProps
...
describe('LuGraphModularityOptimization optional graph-analysis contract', () => {
  ...
  test('borrows weighted topology, seed, and outputs without allocating or synchronizing', () => {
    ...
    const operation = new LuGraphModularityOptimization({
      ...props,
  ...
  test('defaults to singleton initialization, unit resolution, 32 rounds, and zero gain floor', () => {
    const fixture = createOptimizationFixture();
    const operation = new LuGraphModularityOptimization(createOptimizationProps(fixture));

    expect(operation.id).toBe('lu-graph-modularity-optimization');
  ...
  test.each([0, 1, 32, 1024])('accepts a bounded local-moving budget: %s', iterations => {
    const fixture = createOptimizationFixture();
    expect(
      new LuGraphModularityOptimization({...createOptimizationProps(fixture), iterations})
        .iterations
    ).toBe(iterations);
  });
  ...
  test.each([0, 0.125, 1, 2])('accepts a finite nonnegative resolution: %s', resolution => {
    const fixture = createOptimizationFixture();
    expect(
      new LuGraphModularityOptimization({...createOptimizationProps(fixture), resolution})
        .resolution
    ).toBe(resolution);
  });
  ...
  test.each([0, 0.001, 1, 2])('accepts a nonnegative strict minimum gain: %s', minimumGain => {
    const fixture = createOptimizationFixture();
    expect(
      new LuGraphModularityOptimization({...createOptimizationProps(fixture), minimumGain})
        .minimumGain
    ).toBe(minimumGain);
  });
  ...
  test('requires reverse adjacency for exact directed modularity moves', () => {
    const fixture = createOptimizationFixture();
    expect(
      () => new LuGraphModularityOptimization(createOptimizationProps(fixture, {reverse: false}))
...
describe('LuGraphModularityOptimization packed GPU vectors and physical ownership', () => {
  test.each([
    'output',
    'initialCommunities'
    ...
    expect(() => new LuGraphModularityOptimization({...props, [vectorName]: floating})).toThrow(
      new RegExp(`${vectorName}|uint32|packed`)
    );
    expect(() => new LuGraphModularityOptimization({...props, [vectorName]: truncated})).toThrow(
      new RegExp(`${vectorName}|rows|length`)
    );
    expect(() => new LuGraphModularityOptimization({...props, [vectorName]: partitioned})).toThrow(
  ...
  test.each([0, 2])('requires exactly one float32 modularity score row: %s', length => {
    ...
    expect(() => new LuGraphModularityOptimization({...props, modularity})).toThrow(
  ...
  test.each([
    ...
    expect(
      () => new LuGraphModularityOptimization({...props, [scalarName]: invalidScalar})
  ...
  test.each([
    ...
    expect(() => new LuGraphModularityOptimization({...props, output})).toThrow(
  ...
  test.each([
    ...
    expect(() => new LuGraphModularityOptimization({...props, [destination]: aliased})).toThrow(
      /distinct|physical|allocation/
    );
  });
...
): LuGraphModularityOptimizationProps {
...
function getOptimizationInput(props: LuGraphModularityOptimizationProps, name: string): GPUVector {
