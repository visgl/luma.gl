  ...
  test('excludes the only dimension from distributions and materializes all-row rendering masks', () => {
    ...
    expect(addComputePassSpy.mock.calls.map(([pass]) => pass.id)).toEqual(
      expect.arrayContaining([
        'single-dimension-all-rows-all-not',
        'single-dimension-all-rows-all-compose',
        'single-dimension-all-visible-all-not',
        'single-dimension-all-visible-all-compose'
      ])
    );
  ...
  test('shares leave-one-out masks between linked views while retaining other dimensions', () => {
    ...
    expect(
      addComputePassSpy.mock.calls.filter(
        ([pass]) => pass.id === 'linked-dimensions-compose-without-first'
      )
    ).toHaveLength(1);
    expect(
      addComputePassSpy.mock.calls.filter(
        ([pass]) => pass.id === 'linked-dimensions-compose-without-second'
      )
    ).toHaveLength(1);
