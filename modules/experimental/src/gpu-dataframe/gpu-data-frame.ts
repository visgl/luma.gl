...
import {
  ...
  type GPUDataFrameDerivedColumnOptions
...
import type {GPUDataFrameHistogramOptions, GPUDataFrameHistogramQuery} from './gpu-histogram-query';
import type {
...
export type GPUDataFrameDictionaries<T extends GPUTypeMap = GPUTypeMap> = Partial<{
  [Name in keyof T & string]: GPUDataFrameDictionary;
}>;

/** Existing GPU table resources and optional explicit analytical sidecars. */
export type GPUDataFrameProps<T extends GPUTypeMap = GPUTypeMap> = {
 ...
 */
export class GPUDataFrame<T extends GPUTypeMap = GPUTypeMap> {
  ...
  readonly ownership: GPUDataFrameOwnership;

  private resourceLease: GPUDataFrameResourceLease;
  ...
  constructor({
    ...
    if (ownership !== 'borrowed' && ownership !== 'owned') {
      throw new Error('GPUDataFrame ownership must be borrowed or owned');
    }
  ...
  get sourceInfo(): readonly GPUDataFrameSourceInfo[] {
  ...
  column<Name extends keyof T & string>(columnName: Name): GPUDataFrameColumn<T[Name]> {
    this.assertAvailable();
    const column = this.table.gpuColumns[columnName];
    if (!column) {
      throw new Error(`GPUDataFrame column "${columnName}" does not exist`);
    }
    return column as GPUDataFrameColumn<T[Name]>;
  }
  ...
  filter<ReferencedColumns extends keyof T & string>(
    ...
    this.assertAvailable();
    return new GPUDataFrameQuery(this, [predicate], this.columnNames);
  }
  ...
  ): GPUDataFrameQuery<T & Record<Name, Format>, (keyof T & string) | Name, T> {
    this.assertAvailable();
    const query = new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames);
  ...
  groupBy<Key extends GPUDataFrameColumnNamesOfFormat<T, keyof T & string, 'uint32'>>(
    key: Key,
    options: GPUDataFrameGroupByOptions = {}
  ): GPUDataFrameGroupByQuery<T, keyof T & string, Key, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).groupBy(
      key,
      options
    );
  }
  ...
  aggregate<Definitions extends GPUDataFrameGlobalAggregationDefinitions<T>>(
    definitions: Definitions
  ): GPUDataFrameAggregationQuery<T, keyof T & string, Definitions, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).aggregate(
      definitions
    );
  }

  /** Plans fixed-domain or irregular-edge histogram binning entirely on the CPU. */
  histogram<Column extends GPUDataFrameScalarColumnNames<T, keyof T & string>>(
    column: Column,
    options: GPUDataFrameHistogramOptions
  ): GPUDataFrameHistogramQuery<T, keyof T & string, Column, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).histogram(
      column,
      options
    );
  }
  ...
  sortBy<Column extends GPUDataFrameScalarColumnNames<T, keyof T & string>>(
    column: Column,
    options: GPUDataFrameSortOptions = {}
  ): GPUDataFrameSortQuery<T, keyof T & string, Column, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).sortBy(
      column,
      options
    );
  }
  ...
  topK<Column extends GPUDataFrameScalarColumnNames<T, keyof T & string>>(
    column: Column,
    limit: number,
    options: GPUDataFrameSortOptions = {}
  ): GPUDataFrameSortQuery<T, keyof T & string, Column, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).topK(
      column,
      limit,
      options
    );
  ...
  sortByGlobal<Column extends GPUDataFrameScalarColumnNames<T, keyof T & string>>(
    column: Column,
    options: GPUDataFrameSortOptions = {}
  ): GPUDataFrameGlobalSortQuery<T, keyof T & string, Column, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).sortByGlobal(
      column,
      options
    );
  }
  ...
  topKGlobal<Column extends GPUDataFrameScalarColumnNames<T, keyof T & string>>(
    column: Column,
    limit: number,
    options: GPUDataFrameSortOptions = {}
  ): GPUDataFrameGlobalSortQuery<T, keyof T & string, Column, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).topKGlobal(
      column,
      limit,
      options
    );
  ...
  innerJoin<
    ...
    LeftKey extends GPUDataFrameColumnNamesOfFormat<T, keyof T & string, 'uint32'>,
  ...
  ): GPUDataFrameJoinQuery<T, keyof T & string, Right, LeftKey, RightKey, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).innerJoin(
      right,
      options
    );
  }
  ...
  >(
    ...
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).leftJoin(
      right,
      options
    );
  }
  ...
  >(
    ...
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).semiJoin(
      right,
      options
    );
  }
  ...
  >(
    ...
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).antiJoin(
      right,
      options
    );
  }
  ...
  ): GPUDataFrameLookupQuery<T, keyof T & string, Right, LeftKey, RightKey, T> {
    this.assertAvailable();
    return new GPUDataFrameQuery<T, keyof T & string, T>(this, [], this.columnNames).lookup(
      right,
      options
    );
  }
  ...
  select<ColumnName extends keyof T & string>(
  ...
  ): GPUDataFrame<Pick<T, ColumnName>> {
    this.assertAvailable();
    ...
    const projection = new GPUDataFrame<Pick<T, ColumnName>>({
  ...
  private assertAvailable(): void {
    if (this.destroyed) {
      throw new Error('GPUDataFrame has been destroyed');
    }
  }
}
...
function assertValidDataFrameValidity<T extends GPUTypeMap>(
  ...
  for (const [columnName, vector] of Object.entries(validity)) {
    if (!table.schema.fields.some(field => field.name === columnName)) {
      throw new Error(`GPUDataFrame validity column "${columnName}" does not exist`);
    }
    if (!table.gpuVectors[columnName]) {
      throw new Error(`GPUDataFrame validity column "${columnName}" must contain GPU vector data`);
    }
    if (!vector || !(vector instanceof GPUVector) || vector.format !== 'uint32') {
      throw new Error(`GPUDataFrame validity column "${columnName}" requires a uint32 GPUVector`);
    }
    if (vector.length !== table.numRows) {
      throw new Error(
        `GPUDataFrame validity column "${columnName}" must match the table row count`
      );
    }
    if (vector.data.length !== table.batches.length) {
      throw new Error(
        `GPUDataFrame validity column "${columnName}" must match source batch chunks`
      );
    }
    for (const [batchIndex, batch] of table.batches.entries()) {
      if (vector.data[batchIndex]?.length !== batch.numRows) {
        throw new Error(
          `GPUDataFrame validity column "${columnName}" must match source batch ${batchIndex}`
        );
      }
    }
  }
...
function assertValidDataFrameDictionaries<T extends GPUTypeMap>(
  ...
  for (const columnName of Object.keys(dictionaries)) {
    if (!table.schema.fields.some(field => field.name === columnName)) {
      throw new Error(`GPUDataFrame dictionary column "${columnName}" does not exist`);
    }
  }
}
...
function assertSelectedDataFrameColumns<T extends GPUTypeMap>(
  ...
  for (const columnName of columnNames) {
    ...
    if (selectedNames.has(columnName)) {
      throw new Error(`GPUDataFrame column "${columnName}" cannot be selected more than once`);
    }
...
function getRequiredDataFrameField(fields: GPUField[], columnName: string): GPUField {
  ...
  if (!field) {
    throw new Error(`GPUDataFrame source schema is missing column "${columnName}"`);
  }
  return field;
}
