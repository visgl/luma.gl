...
import {GPUBatchSort} from '../gpu-core/gpu-batch-sort';
import {
...
export class CompiledGPUDataFrameSort<
...
> extends CompiledGPUDataFrameQuery<T> {
  ...
  readonly direction: GPUDataFrameNormalizedSortOptions['direction'];
  /** Placement of explicitly null source rows, outside the numeric/NaN groups. */
  readonly nulls: GPUDataFrameNormalizedSortOptions['nulls'];
  /** Placement of NaN source rows within the non-null group. */
  readonly nans: GPUDataFrameNormalizedSortOptions['nans'];
  /** Stable sort algorithm requested independently for every source batch. */
  readonly algorithm: GPUDataFrameNormalizedSortOptions['algorithm'];
...
export function compileGPUDataFrameSort<Source extends GPUTypeMap, Selection extends GPUTypeMap>(
...
): CompiledGPUDataFrameSort<Selection> {
  validateGPUAnalyticsSource(source, [column]);
  validateGPUSortSourceOffsets(source);
  ...
  >(source, predicates, selectedColumns, graph, derivedColumns, {
    ...
    prepare: context => addGPUBatchSortToGraph(context, column, options)
  });
}

/** Ensures stable source-row identifiers remain exactly representable by uint32 output vectors. */
function validateGPUSortSourceOffsets<Source extends GPUTypeMap>(
  source: GPUDataFrame<Source>
): void {
  let sourceOffset = 0;
  for (const batch of source.batches) {
    ...
    if (
      ...
      throw new Error('GPUDataFrame sort source-row identities must fit uint32');
    }
...
function addGPUBatchSortToGraph<Selection extends GPUTypeMap>(
  context: GPUDataFrameQueryExtensionContext<Selection>,
  column: keyof Selection & string,
  options: GPUDataFrameNormalizedSortOptions
): GPUDataFrameQueryExtensionResult<Selection, CompiledGPUDataFrameSort<Selection>> {
  ...
  if (field?.nullable && !validityVector && input.length > 0) {
    throw new Error(`GPUDataFrame nullable sort column "${column}" requires GPU validity`);
  }
  ...
  for (const [batchIndex, values] of input.data.entries()) {
    ...
    addGPUSortEncodeKeysPass(graph, `${id}-encode-batch-${batchIndex}`, {
      input: values,
  ...
  for (const [batchIndex, batch] of context.table.batches.entries()) {
    if (batch.numRows > 0) {
      addGPUSortPublishPermutationPass(graph, `${id}-publish-batch-${batchIndex}`, {
  ...
  return {
    ...
    createCompiled: props => new CompiledGPUDataFrameSort(props, column, options)
  };
}

/** Encodes signed/floating order into the complete unsigned domain and preserves stable ties. */
function addGPUSortEncodeKeysPass(
  graph: GPUCommandGraph<GPUDataFrameQueryParameters>,
  id: string,
  chunk: GPUSortChunk
): void {
  const shaderType = getGPUAnalyticsShaderType(chunk.input.format);
  ...
  const nanExpression =
    chunk.input.format === 'float32'
      ...
      : 'false';
  const key = getGPUSortNumericKeyExpression(chunk.input.format);
  const source = /* wgsl */ `
...
fn main(
  ...
  ${getGPUAnalyticsInvocationIndexSource(graph, chunk.input.length)}
...
function getGPUSortNumericKeyExpression(format: GPUAnalyticsScalarFormat): string {
  switch (format) {
    case 'uint32':
      return 'value';
    case 'sint32':
...
function addGPUSortEncodeClassesPass(
  graph: GPUCommandGraph<GPUDataFrameQueryParameters>,
  id: string,
  chunk: GPUSortChunk,
  options: GPUDataFrameNormalizedSortOptions
...
function addGPUSortPublishPermutationPass(
  graph: GPUCommandGraph<GPUDataFrameQueryParameters>,
  id: string,
  props: {
...
fn main(
  ...
  ${getGPUAnalyticsInvocationIndexSource(graph, props.sortedIndices.length)}
  if (index < ELEMENT_COUNT) {
