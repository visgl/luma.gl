...
import type {GPUDataFrameColumnNamesOfFormat} from './gpu-group-by-query';
import {
  compileGPUDataFrameJoin,
  compileGPUDataFrameLookup,
  type CompiledGPUDataFrameJoin,
  type CompiledGPUDataFrameLookup
} from './gpu-join-compiler';
...
export type GPUDataFrameJoinType = 'inner' | 'left' | 'semi' | 'anti';

/** Unique-right join columns, per-batch output capacity, and bounded hash-index controls. */
export type GPUDataFrameJoinOptions<
...
export type GPUDataFrameLookupOptions<
  LeftKey extends string = string,
  RightKey extends string = string
> = Readonly<{
  /** Selected unsigned key column on the left dataframe. */
...
export type GPUDataFrameNormalizedJoinOptions<
  LeftKey extends string = string,
  RightKey extends string = string
> = Readonly<{
  leftOn: LeftKey;
...
export class GPUDataFrameJoinQuery<
...
> {
  ...
  readonly right: GPUDataFrame<Right>;
  /** Immutable, fully validated key names and fixed GPU index/publication bounds. */
  readonly options: GPUDataFrameNormalizedJoinOptions<LeftKey, RightKey>;
  ...
  constructor(
    ...
    joinType: GPUDataFrameJoinType = 'inner'
  ) {
    this.query = query;
    this.right = right;
    this.options = normalizeGPUDataFrameJoinOptions(query, right, options, true);
  ...
  ): CompiledGPUDataFrameJoin<Pick<Logical, SelectedColumns>, Right> {
    return compileGPUDataFrameJoin<Source, Pick<Logical, SelectedColumns>, Right>(
      this.query.source,
...
export class GPUDataFrameLookupQuery<
  Logical extends GPUTypeMap,
  SelectedColumns extends keyof Logical & string,
  Right extends GPUTypeMap,
  LeftKey extends GPUDataFrameColumnNamesOfFormat<Logical, SelectedColumns, 'uint32'>,
...
> {
  ...
  constructor(
    ...
    this.query = query;
    this.right = right;
    this.options = normalizeGPUDataFrameJoinOptions(query, right, options, false);
    Object.freeze(this);
  }
  ...
  compile(
    graph: GPUCommandGraph<GPUDataFrameQueryParameters>
  ): CompiledGPUDataFrameLookup<Pick<Logical, SelectedColumns>, Right> {
    return compileGPUDataFrameLookup<Source, Pick<Logical, SelectedColumns>, Right>(
      this.query.source,
      this.query.predicates,
      this.query.selectedColumns,
      this.query.derivedColumns,
...
function normalizeGPUDataFrameJoinOptions<
  Logical extends GPUTypeMap,
  SelectedColumns extends keyof Logical & string,
  Right extends GPUTypeMap,
  LeftKey extends GPUDataFrameColumnNamesOfFormat<Logical, SelectedColumns, 'uint32'>,
...
>(
  ...
  right: GPUDataFrame<Right>,
  options:
    | GPUDataFrameJoinOptions<LeftKey, RightKey>
    | GPUDataFrameLookupOptions<LeftKey, RightKey>,
  allowCapacity: boolean
): GPUDataFrameNormalizedJoinOptions<LeftKey, RightKey> {
  if (!options || typeof options !== 'object') {
    throw new Error('GPUDataFrame joins require explicit leftOn and rightOn key columns');
  }
  if (!right || typeof right !== 'object') {
    throw new Error('GPUDataFrame joins require an existing right dataframe');
  }
  if (!query.selectedColumns.includes(options.leftOn)) {
    throw new Error(`GPUDataFrame left join key "${options.leftOn}" is not selected`);
  }
  if (getGPUDataFrameAnalyticColumnFormat(query, options.leftOn) !== 'uint32') {
    throw new Error(`GPUDataFrame left join key "${options.leftOn}" must be uint32`);
  }
  if (query.source.table.gpuConstants[options.leftOn]) {
    throw new Error(`GPUDataFrame left join key "${options.leftOn}" cannot be constant`);
  }
  if (!right.columnNames.includes(options.rightOn)) {
    throw new Error(`GPUDataFrame right join key "${options.rightOn}" does not exist`);
  }
  ...
  if (rightFormat !== 'uint32') {
    throw new Error(`GPUDataFrame right join key "${options.rightOn}" must be uint32`);
  }
  if (right.table.gpuConstants[options.rightOn]) {
    throw new Error(`GPUDataFrame right join key "${options.rightOn}" cannot be constant`);
  }
  assertCompatibleGPUDataFrameJoinDictionaries(query, right, options.leftOn, options.rightOn);
  if (!Number.isSafeInteger(right.numRows) || right.numRows < 0 || right.numRows > MAXIMUM_UINT32) {
    throw new Error('GPUDataFrame right join row counts must fit uint32');
  }

  const indexCapacity = options.indexCapacity ?? getDefaultGPUDataFrameJoinCapacity(right.numRows);
  if (
    ...
    throw new Error('GPUDataFrame joins require a positive power-of-two uint32 index capacity');
  }
  ...
  if (
    ...
    throw new Error('GPUDataFrame joins require a safely bounded uint32 probe count');
  }

  if ('capacity' in options) {
    const capacity = options.capacity;
    if (!allowCapacity) {
      throw new Error('GPUDataFrame lookups do not accept a compacted output capacity');
    }
    if (
      capacity !== undefined &&
      (!Number.isSafeInteger(capacity) || capacity < 0 || capacity > MAXIMUM_UINT32)
    ) {
      throw new Error('GPUDataFrame joins require a nonnegative uint32 output capacity');
    }
  }
...
function assertCompatibleGPUDataFrameJoinDictionaries<
  Logical extends GPUTypeMap,
  SelectedColumns extends keyof Logical & string,
  Right extends GPUTypeMap,
  Source extends GPUTypeMap
>(
  ...
  const leftDictionary = (
    query.source.dictionaries as Readonly<Record<string, GPUDataFrameDictionary | undefined>>
  )[leftOn];
  ...
  if (!leftDictionary || !rightDictionary) {
    throw new Error('GPUDataFrame join dictionaries must exist on both key columns');
  }
  ...
  if (
    ...
    throw new Error('GPUDataFrame join dictionaries must use identical labels and ordering');
  }
}

/** Chooses a bounded power-of-two index with a maximum default half-full load factor. */
function getDefaultGPUDataFrameJoinCapacity(rightRowCount: number): number {
  const requiredCapacity = Math.max(1, rightRowCount * 2);
  let capacity = 1;
  while (capacity < requiredCapacity) {
    capacity *= 2;
    if (capacity > MAXIMUM_HASH_CAPACITY) {
      throw new Error('GPUDataFrame default join index exceeds uint32 hash capacity');
    }
  }
  return capacity;
}
