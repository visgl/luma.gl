...
type EncodeGPUCommandGraphNodeRange<Parameters> = (
 ...
 */
export class GPUCommandGraphExecution<Parameters = void> {
  ...
  private readonly encodeNodeRange: EncodeGPUCommandGraphNodeRange<Parameters>;
  private nextStepIndex = 0;
  private currentPublishedProgress = 0;
  ...
  encodeNext(
    commandEncoder: CommandEncoder,
    options: GPUCommandGraphEncodeOptions<Parameters>
  ): GPUCommandGraphExecutionStep {
    if (this.completed) {
      throw new Error('GPUCommandGraph execution has already completed');
    }
    const plannedStep = this.plan.steps[this.nextStepIndex];
    const encoding = this.encodeNodeRange(
      commandEncoder,
      options,
      plannedStep.firstNodeIndex,
      plannedStep.nextNodeIndex
    );
    this.nextStepIndex++;
    if (plannedStep.publishable) this.currentPublishedProgress = this.progress;
    return {
      ...plannedStep,
      encoding,
...
type GPUCommandGraphExecutionCost = {
  nodeCount: number;
  commandCount: number;
  maximumInvocationCount: number;
  readByteLength: number;
...
export function getGPUCommandGraphExecutionPlan(
  preflight: Pick<GPUCommandGraphPreflightReport, 'nodes' | 'annotatedNodeCount'>,
  budget: GPUCommandGraphExecutionBudget,
  options: GPUCommandGraphExecutionOptions = {}
): GPUCommandGraphExecutionPlan {
  ...
  if (!['final', 'progressive'].includes(publicationPolicy)) {
    throw new Error(
      `GPUCommandGraph execution publication policy "${publicationPolicy}" is invalid`
    );
  }
  const nodes = preflight.nodes;
  const steps: Readonly<GPUCommandGraphExecutionPlanStep>[] = [];
  let nextNodeIndex = 0;
  while (nextNodeIndex < nodes.length) {
    ...
    while (nextNodeIndex < nodes.length) {
      ...
      cost = nextCost;
      nextNodeIndex++;
      if (publicationPolicy === 'progressive' && nodes[nextNodeIndex - 1].publication) break;
      if (reachesGPUCommandGraphExecutionBudget(cost, budget)) break;
    }
    const publications =
      publicationPolicy === 'progressive'
        ...
        : Object.freeze([] as Readonly<GPUCommandGraphNodePublication>[]);
  ...
  const totalCost = nodes.reduce(
    (cost, node) => addGPUCommandGraphExecutionCost(cost, node),
    getEmptyGPUCommandGraphExecutionCost()
  );
...
function getEmptyGPUCommandGraphExecutionCost(): GPUCommandGraphExecutionCost {
  return {
    nodeCount: 0,
    commandCount: 0,
...
function addGPUCommandGraphExecutionCost(
  cost: GPUCommandGraphExecutionCost,
  node: GPUCommandGraphNodePreflight
): GPUCommandGraphExecutionCost {
  return {
...
function exceedsGPUCommandGraphExecutionBudget(
  cost: GPUCommandGraphExecutionCost,
  budget: GPUCommandGraphExecutionBudget
): boolean {
  return (
    cost.maximumInvocationCount > budget.maximumInvocationCount ||
...
function reachesGPUCommandGraphExecutionBudget(
  cost: GPUCommandGraphExecutionCost,
  budget: GPUCommandGraphExecutionBudget
): boolean {
  return (
...
function validateGPUCommandGraphExecutionBudget(budget: GPUCommandGraphExecutionBudget): void {
  const limits = [
    ...
    get(target, property) {
      if (property === 'dispatch') {
        return () => {
          if (dispatchCount > 0) {
            throw new Error(
              `GPUCommandGraph GPU-conditional compute node "${nodeId}" must encode exactly one dispatch`
            );
          }
          dispatchCount++;
      ...
      if (property === 'dispatchIndirect') {
        return () => {
          throw new Error(
            `GPUCommandGraph GPU-conditional compute node "${nodeId}" must use dispatch(); the graph supplies its indirect command`
          );
        };
      }
...
export class GPUCommandGraphEncoding {
  ...
  constructor(
    ...
    computePassCount = nodes.filter(
      node => node.stats.type === 'compute' && node.stats.condition?.outcome !== 'skipped'
    ).length
  ) {
    this.nodes = nodes;
...
export class GPUCommandGraph<Parameters = void> {
  ...
  private addNode(node: GPUCommandGraphNode<Parameters>): void {
    ...
    if (node.condition) {
      ...
      if (node.condition.source === 'cpu') {
        if (typeof node.condition.evaluate !== 'function') {
          throw new Error(
            `GPUCommandGraph node "${node.id}" CPU condition requires an evaluate function`
          );
        }
      } else {
        ...
        if (node.type !== 'compute' || gpuCondition.mode !== 'indirect') {
          throw new Error(
            `GPUCommandGraph node "${node.id}" GPU conditions require an indirect compute node`
          );
        }
        if (!gpuCondition.buffer) {
          throw new Error(
            `GPUCommandGraph node "${node.id}" GPU condition requires an indirect command buffer`
          );
        }
        ...
        if (!Number.isSafeInteger(byteOffset) || byteOffset < 0 || byteOffset % 4 !== 0) {
          throw new Error(
            `GPUCommandGraph node "${node.id}" GPU condition byteOffset must be a nonnegative multiple of 4`
          );
        }
        if (byteOffset + 3 * UINT32_BYTE_LENGTH > gpuCondition.buffer.byteLength) {
          throw new Error(
            `GPUCommandGraph node "${node.id}" GPU condition indirect command exceeds buffer "${gpuCondition.buffer.id}"`
          );
        }
        if (
          !(node.resources ?? []).some(
    ...
    if (node.publication) {
      if (node.condition) {
        throw new Error(
          `GPUCommandGraph node "${node.id}" cannot combine conditional execution with a publication boundary`
        );
      }
...
export class CompiledGPUCommandGraph<Parameters = void> {
  ...
  createExecution(
    budget: GPUCommandGraphExecutionBudget,
    options: GPUCommandGraphExecutionOptions = {}
  ): GPUCommandGraphExecution<Parameters> {
    return new GPUCommandGraphExecution({
      ...
      encodeNodeRange: (commandEncoder, options, firstNodeIndex, nextNodeIndex) =>
        this.encodeNodeRange(commandEncoder, options, firstNodeIndex, nextNodeIndex)
    });
  }
      ...
      for (let nodeIndex = firstNodeIndex; nodeIndex < nextNodeIndex; nodeIndex++) {
        ...
        if (node.condition?.source === 'cpu') {
          ...
          try {
            ...
            throw new Error(
              `GPUCommandGraph CPU condition "${node.condition.id}" failed for node "${node.id}": ${message}`
            );
          }
          if (!shouldExecute) {
            encodedNodes.push({
          ...
          case 'compute': {
            ...
            try {
              ...
              const conditionalPass = gpuCondition
                ? makeGPUIndirectConditionalComputePass(
                    computePass,
                    getBuffer(gpuCondition.buffer),
                    gpuCondition.byteOffset ?? 0,
                    node.id
                  )
                : undefined;
            ...
            ...(node.condition
              ? {
                  condition: {
                    ...
                    outcome:
                      node.condition.source === 'gpu'
                        ? ('gpu-resolved' as const)
                        : ('executed' as const)
                  }
                }
              : {})
