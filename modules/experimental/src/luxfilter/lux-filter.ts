...
} from '../gpu-primitives/gpu-group-aggregation';
import {
  GPUHistogram,
  type GPUHistogramDomain,
  type GPUHistogramEdges
} from '../gpu-primitives/gpu-histogram';
import {GPUMask} from '../gpu-primitives/gpu-mask';
import {GPUVisibilityWorkflow} from '../gpu-primitives/gpu-visibility-workflow';
import {createTransientView} from '../gpu-primitives/graph-data-view-utils';
import {
  LuxFilterSelection,
  type LuxFilterDimension,
  type LuxFilterMask,
  type LuxFilterScalarFormat,
  type LuxFilterScalarInput
} from './gpu-selection';

/** Shared options for a linked, GPU-resident dashboard view. */
export type LuxFilterViewOptions = {
  /** Stable view identifier used by graph nodes and {@link LuxFilter.getViewMask}. */
  id: string;
...
export type LuxFilterHistogramView<Format extends LuxFilterScalarFormat = LuxFilterScalarFormat> =
  LuxFilterViewOptions & {
    ...
    input: LuxFilterScalarInput<Format>;
...
export type LuxFilterGroupView = LuxFilterViewOptions & {
...
export type LuxFilterVisibilityView = LuxFilterViewOptions & {
  ...
  outputMask?: LuxFilterMask;
  /** Optional explicit stable source identifiers. */
  sourceIds?: LuxFilterMask;
...
export type LuxFilterMaskView = LuxFilterViewOptions & {
...
export type LuxFilterProps = {
  ...
  dimensions: readonly LuxFilterDimension[];
  /** Linked histogram, group, visibility, and mask consumers. */
  views?: readonly LuxFilterView[];
 ...
 */
export class LuxFilter<Parameters = void> {
  ...
  readonly dimensions: readonly LuxFilterDimension[];
  /** Linked dashboard consumers. */
  readonly views: readonly LuxFilterView[];
  /** GPU-resident intersection of every registered dimension. */
  readonly mask: LuxFilterMask;

  private readonly selections = new Map<string, LuxFilterSelection>();
  private readonly viewMasks = new Map<string, LuxFilterMask | undefined>();
  private readonly excludedDimensionMasks = new Map<string, LuxFilterMask | undefined>();
  private addedToGraph = false;
  private destroyed = false;

  /** Registers source-aligned selections and reserves GPU control and mask storage. */
  constructor(graph: GPUCommandGraph<Parameters>, props: LuxFilterProps) {
    this.id = props.id ?? 'lux-filter';
    this.graph = graph;
    ...
    try {
      for (const dimension of this.dimensions) {
        ...
        const selection = new LuxFilterSelection(graph, dimension, {
  ...
  getDimensionMask(dimensionId: string): LuxFilterMask {
   ...
   */
  getViewMask(viewId: string): LuxFilterMask | undefined {
    this.assertAvailable();
  ...
  private getSelection(dimensionId: string): LuxFilterSelection {
    this.assertAvailable();
  ...
  private getEffectiveMask(view: LuxFilterView): LuxFilterMask | undefined {
...
function assertMatchingInputs(
  first: LuxFilterScalarInput,
  second: LuxFilterScalarInput,
  name: string
): void {
...
function validateViews(
  views: readonly LuxFilterView[],
  selections: ReadonlyMap<string, LuxFilterSelection>,
  controllerId: string
): void {
