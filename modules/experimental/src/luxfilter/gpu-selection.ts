...
export type LuxFilterScalarFormat = 'float32' | 'sint32' | 'uint32';

/** One scalar chunk or an ordered vector whose original chunks remain intact. */
export type LuxFilterScalarInput<T extends LuxFilterScalarFormat = LuxFilterScalarFormat> =
  | GraphDataView<T>
  | GraphVectorView<T>;

/** Source-aligned selection flags: zero rejects and one accepts a row. */
export type LuxFilterMask = GraphDataView<'uint32'> | GraphVectorView<'uint32'>;

/** One scalar dimension with an optional caller-owned output mask. */
export type LuxFilterRangeDimension = {
...
export type LuxFilterBoundsDimension = {
  /** Stable dimension identifier used in generated graph resources and node IDs. */
  id: string;
  /** Select rows within an inclusive two-dimensional rectangle. */
  kind: 'bounds';
  /** Packed horizontal source values. */
  x: LuxFilterScalarInput;
  /** Packed vertical source values with the same chunk topology as `x`. */
  y: LuxFilterScalarInput;
  /** Optional packed source-aligned destination for canonical selection flags. */
  mask?: LuxFilterMask;
};

/** Interactive scalar or two-dimensional selection registered with a LuxFilter controller. */
export type LuxFilterDimension = LuxFilterRangeDimension | LuxFilterBoundsDimension;
 ...
 */
export class LuxFilterSelection {
  ...
  readonly dimension: LuxFilterDimension;
  ...
  constructor(
    ...
    dimension: LuxFilterDimension,
    props: {id?: string} = {}
  ) {
    this.graph = graph;
    this.dimension = dimension;
    this.id = props.id ?? `lux-filter-${dimension.id}`;
...
function addSelectionPass<Parameters>(
  ...
  props: {
    id: string;
    primaryInput: GraphDataView<LuxFilterScalarFormat>;
    secondaryInput?: GraphDataView<LuxFilterScalarFormat>;
...
function getScalarChunks<T extends LuxFilterScalarFormat>(
  input: LuxFilterScalarInput<T>
...
function getScalarShaderType(format: LuxFilterScalarFormat): 'f32' | 'i32' | 'u32' {
  switch (format) {
    case 'float32':
      return 'f32';
    case 'sint32':
...
function getStateScalarExpression(format: LuxFilterScalarFormat, index: number): string {
