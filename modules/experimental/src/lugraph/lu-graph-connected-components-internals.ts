...
import type {LuGraphConnectedComponents} from './lu-graph-connected-components';
...
export function addLuGraphConnectedComponentsToGraphWithDispatchLimit<Parameters>(
...
function addInitializationPass<Parameters>(
  ...
  const convergenceInitialization = state.converged
    ...
    : '';
  const dispatchLayout = getLuGraphConnectedComponentsDispatchLayout(
...
export function getLuGraphConnectedComponentsDispatchLayout(
  elementCount: number,
  maxComputeWorkgroupsPerDimension: number
