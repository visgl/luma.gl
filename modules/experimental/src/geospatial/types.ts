...
/** Packed local two-dimensional positions. */
 ...
 *
 * Each `uint32x4` row stores `[xLowWord, xHighWord, yLowWord, yHighWord]` on the
 * little-endian platforms supported by WebGPU.
 */
...
/** One scalar result per row, preserving source chunk topology. */
export type GPUScalarRows = GraphDataView<'float32'> | GraphVectorView<'float32'>;

/** One double-single scalar result per row, preserving source chunk topology. */
