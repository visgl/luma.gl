...
These paths keep application imports stable while the private scene package delegates shader and
...
- [Complete ANARI API reference](https://luma.gl/docs/api-reference/scene)
- [ANARI materials, image maps, and lighting](https://luma.gl/docs/api-reference/scene/anari-materials-and-lights)
- [ANARI animation and optional glTF integration](https://luma.gl/docs/api-reference/scene/anari-animation)
- [ANARI C API and THREE.js mapping](https://luma.gl/docs/api-reference/scene/anari-api-mapping)
...
The interactive showcase lives at `examples/showcase/scene`. Run it with
`yarn workspace luma.gl-examples-showcase-scene start`; add `?backend=webgl` to force the WebGL2
...
The optional, experimental `@luma.gl/scene/schemas` entry point exports Zod schemas and the
generated draft-07 JSON Schema without adding Zod to imports of the core ANARI rendering API:

```ts
import {ANARISceneSchema, ANARI_SCENE_JSON_SCHEMA} from '@luma.gl/scene/schemas';
...
The optional `@luma.gl/scene/gltf` entry point adapts glTF-owned node hierarchies and decoded clips
...
The format loader lives under `examples/showcase/scene/usd-loader` and follows the loaders.gl
