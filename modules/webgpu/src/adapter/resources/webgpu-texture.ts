...
export class WebGPUTexture extends Texture {
  ...
  readBuffer(options: TextureReadOptions = {}, buffer?: Buffer): Buffer {
    ...
    const layout = this.getTextureDataLayout(options);

    const {bytesPerRow, rowsPerImage, imageCount} = layout;
    const byteLength = bytesPerRow * rowsPerImage * imageCount;
...
>>>>>>> feat: Add Buffer.mapAndWrite(), mapAndRead() (#2372)
  ...
  writeBuffer(buffer: Buffer, options: TextureWriteOptions = {}) {
    /*
    const {
      x = 0,
      y = 0,
