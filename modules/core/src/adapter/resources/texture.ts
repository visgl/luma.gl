...
export type ReadTextureOptions = {
  x?: number;
  y?: number;
  z?: number;
  width?: number;
...
export abstract class Texture extends Resource<TextureProps> {
   ...
   */
  getMemoryLayout(options_: ReadTextureOptions = {}): TextureMemoryLayout {
    const options = this._normalizeReadTextureOptions(options_)
   ...
   */
  abstract readBuffer(options?: ReadTextureOptions, buffer?: Buffer): Buffer;
   ...
   * @note The application can call Texture.getMemoryLayout() to compute the layout.
   */
  abstract readDataAsync(options?: ReadTextureOptions): Promise<ArrayBuffer>;
   ...
   * @note The application can call Texture.getMemoryLayout() to compute the layout.
   */
  abstract writeBuffer(buffer: GPUBuffer, options?: WriteTextureOptions): void;
   ...
   * @note The application can call Texture.getMemoryLayout() to compute the layout.
   */
  abstract writeData(data: ArrayBuffer | ArrayBufferView, options?: WriteTextureOptions): void;

  // IMPLEMENTATION SPECIFIC

  /** Return the implementation specific alignment for a texture format */
  abstract _getRowByteAlignment(format: TextureFormat, width: number): number;

  /**
   * WebGL can read data synchronously.
   * @note While it is convenient, the performance penalty is very significant
   */
  readDataSyncWebGL(options?: ReadTextureOptions): ArrayBuffer | ArrayBufferView {
  ...
  _normalizeReadTextureOptions(
    options_: ReadTextureOptions
  ): Required<ReadTextureOptions> {
    const {width, height} = this;
    const options = {...Texture.defaultReadTextureOptions, width, height, ...options_};
    // WebGL will error if we try to copy outside the bounds of the texture
    options.width = Math.min(options.width, this.width - options.x);
    options.height = Math.min(options.height, this.height - options.y);
    return options;
  ...
  _normalizeWriteTextureOptions(
    options_: WriteTextureOptions
  ): Required<WriteTextureOptions> {
    const {width, height} = this;
    const options = {...Texture.defaultReadTextureOptions, width, height, ...options_};
    // WebGL will error if we try to copy outside the bounds of the texture
    options.width = Math.min(options.width, this.width - options.x);
  ...
  protected static defaultReadTextureOptions: Required<ReadTextureOptions> = {
    x: 0,
    y: 0,
    z: 0,
    width: undefined!,
