...
export default class Program extends Resource {
    ...
    vertexArray.bindForDraw(vertexCount, instanceCount, () => {
      ...
      if (transformFeedback) {
        const primitiveMode = getPrimitiveDrawMode({
          isIndexed, isInstanced,
          drawMode, vertexCount,
  ...
  _multiDraw(parameters) {
    const {
      ...
      drawMode, vertexCount,
      indexType, offset,
      firsts, firstsOffset = 0,
      counts, countsOffset = 0,
      offsets, offsetsOffset = 0,
      instanceCounts, instanceCountOffset = 0,
      drawCount = firsts.length
    } = parameters;

    const isMultidraw = firsts || firstsOffset || counts || countsOffset || offsets ||
    ...
    if (isIndexed && isInstanced) {
      multiDrawInstancedExt.multiDrawElementsInstancedWEBGL(
        drawMode, vertexCount, indexType, offset,
        instanceCounts, instanceCountOffset, drawCount);
    } else if (isInstanced) {
      multiDrawInstancedExt.multiDrawArraysInstancedWEBGL(
        drawMode, firsts, firstsOffset, counts, countsOffset,
        instanceCounts, instanceCountOffset, drawCount);
    } else if (isIndexed) {
      multiDrawExt.multiDrawElementsWEBGL(
        drawMode, vertexCount, indexType, offset, drawCount);
    } else {
      multiDrawExt.multiDrawArraysWEBGL(
        drawMode, firsts, firstsOffset, counts, countsOffset, drawCount);
    }
