...
import { getDataType, getTypedArrayConstructor, getVertexFormatFromAttribute } from '@luma.gl/core';
const TEST_CASES = [
...
test('shadertypes#getVertexFormatFromAttribute', t => {
    for (const { typedArray, size, normalized, result, error } of TEST_CASES) {
        if (result) {
            ...
            t.deepEqual(vertexFormat, result, `Typed array: '${typedArray.constructor.name}, size: ${size}' => ${result}`);
        }
        else {
            t.throws(() => {
...
test('shadertypes#getDataType', t => {
    for (const { typedArray } of ARRAY_TEST_CASES) {
        ...
        t.deepEqual(typedArray, result, `TypedArray '${typedArray.name}, => ${dataType} => ${result.name}`);
    }
    t.end();
});
