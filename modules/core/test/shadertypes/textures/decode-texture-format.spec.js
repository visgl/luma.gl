...
test('shadertype#textureFormatDecoder.getInfo', t => {
    for (const tc of TEST_CASES) {
        ...
        t.deepEqual(decoded, { format: tc.format, ...tc.result }, `textureFormatDecoder.getInfo('${tc.format}') => ${JSON.stringify(decoded.dataType)}`);
    }
    t.end();
});
// prettier-ignore
...
test('shadertype#textureFormatDecoder.getCapabilities', t => {
    for (const tc of TEST_CASES_CAPABILITIES) {
        ...
        t.deepEqual(decoded, { format: tc.format, ...tc.result }, `getVertexFormatInfo('${tc.format}') => ${JSON.stringify(decoded)}`);
    }
    t.end();
});
/**
 ...
 */
const bytesPerPixelFromFormat = (format) => textureFormatDecoder.getInfo(format).bytesPerPixel;
