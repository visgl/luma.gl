...
import { Buffer, Texture, textureFormatDecoder, _getTextureFormatTable, TextureView, getTypedArrayConstructor } from '@luma.gl/core';
...
test('Texture#createView returns a TextureView', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.equal(view.texture.width, tex.width, `${device.type}: view.texture.width matches texture.width`);
        t.equal(view.texture.height, tex.height, `${device.type}: view.texture.height matches texture.height`);
        tex.destroy();
    }
    t.end();
});
...
const TEXTURE_UPLOAD_METHODS = ['writeData', 'copyImageData', 'writeBuffer'];
function createRgbaUploadData(options) {
    const { width, height, depthOrArrayLayers = 1, bytesPerRow = width * 4, rowsPerImage = height, byteOffset = 0, pixel } = options;
    ...
    for (let layer = 0; layer < depthOrArrayLayers; layer++) {
        for (let row = 0; row < height; row++) {
            for (let column = 0; column < width; column++) {
                const offset = byteOffset + layer * rowsPerImage * bytesPerRow + row * bytesPerRow + column * 4;
...
function createLayeredRgbaUploadData(options) {
    const { width, height, pixels, bytesPerRow = width * 4, rowsPerImage = height, byteOffset = 0 } = options;
    ...
    for (let layerIndex = 0; layerIndex < pixels.length; layerIndex++) {
        ...
        data.set(layerData.subarray(byteOffset, byteOffset + rowsPerImage * bytesPerRow), layerByteOffset);
    }
    return data;
}
function repeatRgbaPixel(pixel, texelCount) {
...
function normalizeTextureReadOptionsForTest(texture, options) {
    ...
    const mipDepthOrArrayLayers = texture.dimension === '3d' ? Math.max(1, texture.depth >> mipLevel) : texture.depth;
    return {
        x,
        y,
        z,
...
function compactTextureBytes(texture, arrayBuffer, options) {
    ...
    for (let layer = 0; layer < depthOrArrayLayers; layer++) {
        for (let row = 0; row < height; row++) {
            const sourceOffset = layer * layout.rowsPerImage * layout.bytesPerRow + row * layout.bytesPerRow;
...
function readTexturePixelsSyncWebGL(texture, options) {
...
function uploadTexture(device, texture, method, data, options) {
    switch (method) {
        case 'writeData':
...
test('Texture#writeData & readDataAsync round-trip', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(result, RGBA8_DATA, `${device.type}: writeData + readDataAsync returns same pixels`);
        tex.destroy();
    }
    t.end();
});
test('Texture color read APIs support subresources and mip levels', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        twoDimensionalTexture.writeData(createRgbaUploadData({
            width: 2,
            height: 2,
            bytesPerRow: 256,
            rowsPerImage: 2,
        ...
        t.deepEquals(await readTexturePixels(twoDimensionalTexture, { width: 2, height: 2 }), repeatRgbaPixel([1, 2, 3, 255], 4), `${device.type}: readDataAsync reads 2d base level texels`);
        t.deepEquals(await readTextureBufferPixels(twoDimensionalTexture, { width: 2, height: 2 }), repeatRgbaPixel([1, 2, 3, 255], 4), `${device.type}: readBuffer matches 2d base level texels`);
        ...
        mipTexture.writeData(createRgbaUploadData({
            width: 2,
            height: 2,
            bytesPerRow: 256,
            rowsPerImage: 2,
        ...
        t.deepEquals(await readTexturePixels(mipTexture, { mipLevel: 1, width: 2, height: 2 }), repeatRgbaPixel([4, 5, 6, 255], 4), `${device.type}: readDataAsync reads nonzero mip texels`);
        mipTexture.destroy();
        ...
        cubeTexture.writeData(createRgbaUploadData({
            width: 1,
            height: 1,
            bytesPerRow: 256,
            pixel: [7, 8, 9, 255]
        }), { width: 1, height: 1, z: 5, bytesPerRow: 256 });
        t.deepEquals(await readTexturePixels(cubeTexture, { width: 1, height: 1, z: 5 }), new Uint8Array([7, 8, 9, 255]), `${device.type}: readDataAsync reads cube face texels`);
        cubeTexture.destroy();
        ...
        arrayTexture.writeData(createRgbaUploadData({
            width: 1,
            height: 1,
            bytesPerRow: 256,
            pixel: [10, 11, 12, 255]
        ...
        t.deepEquals(await readTexturePixels(arrayTexture, { width: 1, height: 1, z: 1, depthOrArrayLayers: 1 }), new Uint8Array([10, 11, 12, 255]), `${device.type}: readDataAsync reads array layer texels`);
        ...
        threeDimensionalTexture.writeData(createRgbaUploadData({
            width: 1,
            height: 1,
            bytesPerRow: 256,
            pixel: [13, 14, 15, 255]
        ...
        t.deepEquals(await readTexturePixels(threeDimensionalTexture, {
            width: 1,
            height: 1,
            z: 1,
            depthOrArrayLayers: 1
        }), new Uint8Array([13, 14, 15, 255]), `${device.type}: readDataAsync reads 3d slice texels`);
...
test('Texture color read APIs pack multi-layer and multi-slice reads consistently', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        arrayTexture.writeData(createLayeredRgbaUploadData({
            width: 1,
            height: 1,
            bytesPerRow: 256,
            rowsPerImage: 1,
        ...
        t.deepEquals(await readTexturePixels(arrayTexture, { width: 1, height: 1, depthOrArrayLayers: 2 }), new Uint8Array([1, 10, 20, 255, 2, 11, 21, 255]), `${device.type}: multi-layer reads pack consecutive array layers`);
        arrayTexture.destroy();
        const threeDimensionalTexture = device.createTexture({
            dimension: '3d',
            width: 1,
        ...
        threeDimensionalTexture.writeData(createLayeredRgbaUploadData({
            width: 1,
            height: 1,
            bytesPerRow: 256,
            rowsPerImage: 1,
        ...
        }), new Uint8Array([3, 12, 22, 255, 4, 13, 23, 255]), `${device.type}: multi-slice reads pack consecutive 3d slices`);
        threeDimensionalTexture.destroy();
    }
    t.end();
});
test('Texture color read APIs keep readBuffer and readDataAsync in parity', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        texture.writeData(createLayeredRgbaUploadData({
            width: 1,
            height: 1,
            bytesPerRow: 256,
            rowsPerImage: 1,
        ...
        t.deepEquals(await readTextureBufferPixels(texture, options), await readTexturePixels(texture, options), `${device.type}: readBuffer and readDataAsync return the same texel bytes`);
        texture.destroy();
    }
    t.end();
});
test('Texture#readDataSyncWebGL matches async color reads on WebGL subresources', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(readTexturePixelsSyncWebGL(mipTexture, { mipLevel: 1, width: 2, height: 2 }), await readTexturePixels(mipTexture, { mipLevel: 1, width: 2, height: 2 }), 'webgl: readDataSyncWebGL matches readDataAsync for lower mip reads');
        mipTexture.destroy();
        const arrayTexture = device.createTexture({
            dimension: '2d-array',
            width: 1,
        ...
        t.deepEquals(readTexturePixelsSyncWebGL(arrayTexture, { width: 1, height: 1, depthOrArrayLayers: 2 }), await readTexturePixels(arrayTexture, { width: 1, height: 1, depthOrArrayLayers: 2 }), 'webgl: readDataSyncWebGL matches readDataAsync for multi-layer reads');
        arrayTexture.destroy();
    }
    t.end();
});
test('Texture upload methods round-trip through full-level uploads', async (t) => {
    for (const device of await getTestDevices()) {
        for (const method of TEXTURE_UPLOAD_METHODS) {
            ...
            const uploadBuffer = uploadTexture(device, texture, method, expected, createTextureWriteOptions({ width: 64, height: 1 }));
            const result = await readTexturePixels(texture, { width: 64, height: 1 });
            t.deepEquals(result, expected, `${device.type}: ${method} round-trips tightly packed full-level uploads`);
...
test('Texture upload methods support lower mip uploads', async (t) => {
    for (const device of await getTestDevices()) {
        for (const method of TEXTURE_UPLOAD_METHODS) {
            ...
            t.deepEquals(result, repeatRgbaPixel([90, 80, 70, 255], 4), `${device.type}: ${method} uploads lower mip subresources using mip-sized extents`);
            uploadBuffer?.destroy();
            texture.destroy();
        }
    }
...
test('Texture upload methods honor explicit byteOffset and bytesPerRow', async (t) => {
    for (const device of await getTestDevices()) {
        for (const method of TEXTURE_UPLOAD_METHODS) {
            ...
            t.deepEquals(result, new Uint8Array([11, 22, 33, 255, 11, 22, 33, 255, 11, 22, 33, 255, 11, 22, 33, 255]), `${device.type}: ${method} honors byteOffset and padded rows`);
            uploadBuffer?.destroy();
            texture.destroy();
        }
    }
...
test('Texture upload methods interpret byteOffset as bytes for typed array uploads', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        for (const method of TEXTURE_UPLOAD_METHODS) {
            ...
            t.deepEquals(pixel, [20, 0, 0, 65535], `${device.type}: ${method} converts byteOffset to typed-array element offset`);
            uploadBuffer?.destroy();
            texture.destroy();
        }
    }
...
test('Texture upload methods target cube faces, array layers, and 3d slices consistently', async (t) => {
    for (const device of await getTestDevices()) {
        for (const method of TEXTURE_UPLOAD_METHODS) {
            ...
            t.deepEquals(cubePixels, new Uint8Array([7, 8, 9, 255]), `${device.type}: ${method} targets the requested cube face`);
            ...
            const arrayUploadBuffer = uploadTexture(device, arrayTexture, method, arrayData, arrayOptions);
            ...
            t.deepEquals(arrayPixels, new Uint8Array([21, 22, 23, 255]), `${device.type}: ${method} targets the requested array layer`);
            ...
            const threeDimensionalUploadBuffer = uploadTexture(device, threeDimensionalTexture, method, threeDimensionalData, threeDimensionalOptions);
            ...
            t.deepEquals(threeDimensionalPixels, new Uint8Array([31, 32, 33, 255]), `${device.type}: ${method} targets the requested 3d slice`);
...
test('Texture#copyExternalImage uploads image data on both backends', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(result, new Uint8Array([1, 2, 3, 255, 5, 6, 7, 255]), `${device.type}: copyExternalImage uploads pixel data`);
        texture.destroy();
    }
    t.end();
});
test('Texture#copyImageData is a compatibility wrapper over writeData', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(copyImageDataPixels, writeDataPixels, `${device.type}: copyImageData matches writeData for equivalent uploads`);
...
test('Texture#writeData rejects invalid row layout', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.throws(() => texture.writeData(new Uint8Array(8), {
            width: 2,
            height: 2,
            bytesPerRow: 4,
            rowsPerImage: 2
        }), /bytesPerRow/, `${device.type}: writeData rejects bytesPerRow smaller than the write width`);
        texture.destroy();
    }
    t.end();
});
test('Texture#writeData & readDataAsync round-trip for all formats and dimensions', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        const formats = Object.keys(formatTable);
        for (const format of formats) {
            ...
            for (const dimension of ['2d', '3d']) {
                ...
                if (dimension === '3d' && device.type === 'webgl')
                    continue;
                ...
                try {
                    ...
                    const match = ArrayType === Float32Array ? almostEqual(output, input) : deepEqual(output, input);
                    if (!match) {
...
test.skip('Texture#copyImageData & readDataAsync round-trip', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(result, RGBA8_DATA, `${device.type}: copyImageData + readDataAsync returns same pixels`);
        tex.destroy();
    }
    t.end();
});
test.skip('Texture#writeBuffer & readDataAsync round-trip', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(result, RGBA8_DATA, `${device.type}: writeBuffer + readDataAsync returns same pixels`);
        upload.destroy();
        tex.destroy();
    }
    t.end();
...
test.skip('Texture#readBuffer & Buffer.readAsync round-trip', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.deepEquals(result, RGBA8_DATA, `${device.type}: readBuffer + Buffer.readAsync returns same pixels`);
        tex.destroy();
    }
    t.end();
});
...
test('Texture color read APIs reject unsupported formats and aspects', async (t) => {
    ...
    for (const device of await getTestDevices()) {
        ...
        t.throws(() => colorTexture.readBuffer({ aspect: 'depth-only' }), /aspect 'all'/, `${device.type}: color reads reject non-all aspects`);
        colorTexture.destroy();
        const depthTexture = device.createTexture({ width: 1, height: 1, format: 'depth16unorm' });
        t.throws(() => depthTexture.readBuffer({}), /depth formats/, `${device.type}: color reads reject depth formats`);
        ...
        if (device.isTextureFormatSupported('stencil8')) {
            try {
                ...
                t.throws(() => stencilTexture.readBuffer({}), /stencil formats/, `${device.type}: color reads reject stencil formats`);
                stencilTexture.destroy();
            }
            catch (error) {
                t.comment(`${device.type}: skipping stencil read guard test (${error.message})`);
            }
        }
        ...
        t.throws(() => depthStencilTexture.readBuffer({}), /depth-stencil formats/, `${device.type}: color reads reject depth-stencil formats`);
        depthStencilTexture.destroy();
        const compressedFormat = compressedFormats.find(format => device.isTextureFormatSupported(format));
        if (compressedFormat) {
            ...
            t.throws(() => compressedTexture.readBuffer({}), /compressed formats/, `${device.type}: color reads reject compressed formats`);
            compressedTexture.destroy();
        }
        else {
            t.comment(`${device.type}: skipping compressed read guard test (no supported compressed format)`);
        }
    }
    t.end();
});
test('Texture#readDataAsync reuses the cached WebGL read framebuffer', async (t) => {
    ...
    const firstFramebuffer = texture._framebuffer;
    await texture.readDataAsync({ width: 1, height: 1, z: 1, depthOrArrayLayers: 1 });
    const secondFramebuffer = texture._framebuffer;
    t.ok(firstFramebuffer, 'webgl: first read creates a cached read framebuffer');
    t.equal(secondFramebuffer, firstFramebuffer, 'webgl: subsequent reads reuse the cached read framebuffer');
    texture.destroy();
    t.end();
});
// //////////////////////////////
test('Device#isTextureFormatSupported()', async (t) => {
    ...
    for (const device of await getTestDevices()) {
        ...
        t.ok(unSupportedFormats.every(format => expected.includes(format)), `${device.type}: unsupported formats ${unSupportedFormats.join(',') in [expected.join(',')]}`);
    }
    t.end();
});
test('Device#isTextureFormatFilterable()', async (t) => {
    ...
    for (const device of await getTestDevices()) {
        ...
        unSupportedFormats.sort();
        const expected = UNSUPPORTED_FORMATS[device.type].sort();
        t.ok(unSupportedFormats.every(format => expected.includes(format)), `${device.type}: Unfilterable formats ${unSupportedFormats.join(',') in [expected.join(',')]}`);
    }
    t.end();
});
test('Device#isTextureFormatRenderable()', async (t) => {
    ...
    for (const device of await getTestDevices()) {
        ...
        unSupportedFormats.sort();
        const expected = UNSUPPORTED_FORMATS[device.type].sort();
        t.ok(unSupportedFormats.every(format => expected.includes(format)), `${device.type}: Unrenderable formats ${unSupportedFormats.join(',') in [expected.join(',')]}`);
    }
    t.end();
});
...
test('Texture#depth/stencil formats', async (t) => {
    ...
    for (const device of await getTestDevices()) {
        for (const key of DEPTH_STENCIL_FORMATS) {
            ...
            t.notOk(device.isTextureFormatFilterable(format), `${device.type} ${format} is not filterable`);
...
// };
function testFormatCreation(t, device, withData = false) {
    for (const [formatName] of Object.entries(_getTextureFormatTable)) {
        ...
        if (canGenerateMipmaps) {
            try {
                ...
                t.equals(texture.props.format, format, `Texture(${device.type},${format}) created with mipmaps=${mipmaps}`);
                texture.destroy();
            }
            catch (error) {
...
test('Texture#copyExternalImage', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        if (device.info.type === 'webgl') {
            ...
            t.deepEquals(webglDevice.readPixelsToArrayWebGL(texture), new Uint8Array(8), `${device.info.type} Pixels are initially empty`);
        }
        if (typeof document !== 'undefined') {
            ...
            if (device.info.type === 'webgl') {
                t.deepEquals(device.readPixelsToArrayWebGL(texture), new Uint8Array([255, 0, 0, 255, 255, 0, 0, 255]), `${device.info.type} Pixels were set correctly (full image)`);
            }
            // Subimage copy
            ...
            if (device.info.type === 'webgl') {
                t.deepEquals(device.readPixelsToArrayWebGL(texture), new Uint8Array([255, 0, 0, 255, 0, 0, 0, 255]), `${device.info.type} Pixels were set correctly (sub image)`);
            }
            ...
            texture.copyExternalImage({ image: canvas, x: 1 });
            if (device.info.type === 'webgl') {
                t.deepEquals(device.readPixelsToArrayWebGL(texture), new Uint8Array([255, 0, 0, 255, 0, 255, 0, 255]), `${device.info.type} Pixels were set correctly (sub image small canvas)`);
            }
        }
        if (device.info.type !== 'webgl') {
...
test.skip('WebGL#TextureCube construct/delete', async (t) => {
    t.throws(
    // @ts-expect-error
    () => new TextureCube(), /.*WebGLRenderingContext.*/, 'TextureCube throws on missing gl context');
...
test.skip('WebGL#Texture3D construct/delete', async (t) => {
    t.throws(() => webglDevice.createTexture({ dimension: '3d', width: 1, height: 1 }), 'Texture3D throws on missing gl context');
