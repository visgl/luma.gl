...
test('Framebuffer#clone overrides size', async (t) => {
    for (const device of await getTestDevices()) {
        ...
        t.equal(cloned.colorAttachments[0].texture.width, 4, `${device.type}: cloned color attachment width overridden`);
        t.equal(cloned.colorAttachments[0].texture.height, 4, `${device.type}: cloned color attachment height overridden`);
        t.notEqual(cloned.colorAttachments[0].texture, framebuffer.colorAttachments[0].texture, `${device.type}: cloned color attachment is new texture`);
...
test('WebGLFramebuffer create and resize attachments', async (t) => {
    for (const testDevice of await getTestDevices()) {
        for (const tc of TEST_CASES) {
            ...
            if (tc.pass) {
                t.doesNotThrow(testFramebufferOpts, `${testDevice.id}.createFramebuffer() success as expected for "${tc.title}"`);
            }
            else {
                t.throws(testFramebufferOpts, `${testDevice.id}.createFramebuffer() failure as expected for "${tc.title}"`);
            }
        }
    }
    t.end();
    ...
    for (const testDevice of await getTestDevices()) {
        ...
        if (testDevice.type === 'webgl') {
            try {
                ...
                t.comment(`beginRenderPass() failed ${error.message}`);
            }
            ...
            t.deepEqual(pixels, 
            // prettier-ignore
            [255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255], 'Framebuffer pixel colors are set correctly');
        }
        framebuffer.delete();
    }
    t.end();
