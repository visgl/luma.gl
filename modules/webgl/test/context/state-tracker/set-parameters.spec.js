...
test('WebGL#composite setter', async (t) => {
    ...
    for (const key of compositeStateKeys) {
        const value = getGLParameters(webglDevice.gl, [key])[key];
        t.deepEqual(value, GL_PARAMETER_DEFAULTS[key], `got expected default value ${stringifyTypedArray(value)}`);
    }
    ...
    t.deepEqual(value, GL_PARAMETER_DEFAULTS[GL.STENCIL_VALUE_MASK], `got expected updated defuault value ${stringifyTypedArray(value)}`);
    t.end();
});
test('WebGL#setGLParameters per-face stencil state', async (t) => {
    ...
    t.deepEqual(stencilState[GL.STENCIL_FUNC], GL.GREATER, `got expected stencil func ${stringifyTypedArray(stencilState[GL.STENCIL_FUNC])}`);
    t.deepEqual(stencilState[GL.STENCIL_REF], 1, `got expected stencil reference ${stringifyTypedArray(stencilState[GL.STENCIL_REF])}`);
    t.deepEqual(stencilState[GL.STENCIL_VALUE_MASK], 0x0f, `got expected stencil mask ${stringifyTypedArray(stencilState[GL.STENCIL_VALUE_MASK])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_FUNC], GL.LESS, `got expected back stencil func ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_FUNC])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_REF], 2, `got expected back stencil reference ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_REF])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_VALUE_MASK], 0xf0, `got expected back stencil mask ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_VALUE_MASK])}`);
    t.deepEqual(stencilState[GL.STENCIL_FAIL], GL.KEEP, `got expected stencil fail op ${stringifyTypedArray(stencilState[GL.STENCIL_FAIL])}`);
    t.deepEqual(stencilState[GL.STENCIL_PASS_DEPTH_FAIL], GL.REPLACE, `got expected stencil depth fail op ${stringifyTypedArray(stencilState[GL.STENCIL_PASS_DEPTH_FAIL])}`);
    t.deepEqual(stencilState[GL.STENCIL_PASS_DEPTH_PASS], GL.INCR, `got expected stencil depth pass op ${stringifyTypedArray(stencilState[GL.STENCIL_PASS_DEPTH_PASS])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_FAIL], GL.DECR, `got expected back stencil fail op ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_FAIL])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_PASS_DEPTH_FAIL], GL.INVERT, `got expected back stencil depth fail op ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_PASS_DEPTH_FAIL])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_PASS_DEPTH_PASS], GL.ZERO, `got expected back stencil depth pass op ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_PASS_DEPTH_PASS])}`);
    t.deepEqual(stencilState[GL.STENCIL_WRITEMASK], 0x0f0f0f0f, `got expected stencil write mask ${stringifyTypedArray(stencilState[GL.STENCIL_WRITEMASK])}`);
    t.deepEqual(stencilState[GL.STENCIL_BACK_WRITEMASK], 0x00ff00ff, `got expected back stencil write mask ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_WRITEMASK])}`);
    t.end();
});
test('WebGLState#get all parameters', async (t) => {
    ...
    for (const key in ENUM_STYLE_SETTINGS_SET1_PRIMITIVE) {
        const value = getGLParameters(webglDevice.gl, [key])[key];
        t.deepEqual(value, ENUM_STYLE_SETTINGS_SET1_PRIMITIVE[key], `got expected value ${stringifyTypedArray(value)} after setGLParameters for ${webglDevice.getGLKey(key)}`);
    }
    const copy = getGLParameters(webglDevice.gl);
    for (const key in ENUM_STYLE_SETTINGS_SET1_PRIMITIVE) {
        const value = copy[key];
        t.deepEqual(value, ENUM_STYLE_SETTINGS_SET1_PRIMITIVE[key], `got expected value ${stringifyTypedArray(value)} after getGLParameters for ${webglDevice.getGLKey(key)}`);
    }
    t.end();
});
test('WebGLRenderPass#setParameters stencil reference', async (t) => {
    ...
    try {
        ...
        t.deepEqual(stencilState[GL.STENCIL_REF], 5, `got expected stencil reference ${stringifyTypedArray(stencilState[GL.STENCIL_REF])}`);
        t.deepEqual(stencilState[GL.STENCIL_BACK_REF], 5, `got expected back stencil reference ${stringifyTypedArray(stencilState[GL.STENCIL_BACK_REF])}`);
...
test('WebGL#reset', async (t) => {
    ...
    for (const key in ENUM_STYLE_SETTINGS_SET1_PRIMITIVE) {
        const value = getGLParameters(webglDevice.gl, [key])[key];
        t.deepEqual(value, GL_PARAMETER_DEFAULTS[key], `got expected value ${stringifyTypedArray(value)} after resetGLParameters for ${webglDevice.getGLKey(key)}`);
    }
    t.end();
});
