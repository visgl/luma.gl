...
function isTypedArray(v) {
    return ArrayBuffer.isView(v) && !(v instanceof DataView) ? v : null;
}
export function stringifyTypedArray(v) {
...
test('WebGLState#getGLParameters (WebGL)', async (t) => {
    ...
    for (const setting in GL_PARAMETERS) {
        ...
        t.ok(value !== undefined, `${webglDevice.getGLKey(setting)}: got a value ${stringifyTypedArray(value)}`);
    }
    t.end();
});
// TODO - restore asap
test.skip('WebGLState#setGLParameters (Mixing enum and function style keys)', async (t) => {
    ...
    for (const key in ENUM_STYLE_SETTINGS_SET1) {
        ...
        t.deepEqual(value, ENUM_STYLE_SETTINGS_SET1[key], `got expected value ${stringifyTypedArray(value)} for key: ${webglDevice.getGLKey(key)}`);
    }
    t.end();
});
// TODO - restore asap
test('WebGLState#setGLParameters (Argument expansion for ***SeparateFunc setters))', async (t) => {
    ...
    for (const key in expectedValues) {
        ...
        t.deepEqual(value, expectedValues[key], `got expected value ${stringifyTypedArray(value)} for key: ${webglDevice.getGLKey(key)}`);
    }
    t.end();
});
test('WebGLState#withGLParameters', async (t) => {
    ...
    t.throws(() => withGLParameters(webglDevice.gl, {
        clearColor: [0, 1, 0, 1],
        [GL.BLEND]: true,
        nocatch: false
    }, () => {
...
test('WebGLState#withGLParameters: recursive', async (t) => {
    ...
    t.deepEqual(blendFuncSrcRGB, GL.ONE_MINUS_SRC_ALPHA, `got expected value ${stringifyTypedArray(blendFuncSrcRGB)}`);
    t.deepEqual(blendEquation, GL.FUNC_ADD, `got expected value ${stringifyTypedArray(blendEquation)}`);
    withGLParameters(webglDevice.gl, {
        clearColor: [0, 1, 0, 1],
        [GL.BLEND]: true
    }, () => {
        ...
        t.deepEqual(clearColor, [0, 1, 0, 1], `got expected value ${stringifyTypedArray(clearColor)}`);
        t.deepEqual(blendState, true, `got expected value ${stringifyTypedArray(blendState)}`);
        // Verify un-changed state
        t.deepEqual(blendFuncSrcRGB, GL.ONE_MINUS_SRC_ALPHA, `got expected un changed value ${stringifyTypedArray(blendFuncSrcRGB)}`);
        t.deepEqual(blendEquation, GL.FUNC_ADD, `got expected un changed value ${stringifyTypedArray(blendEquation)}`);
        withGLParameters(webglDevice.gl, {
            ...
            t.deepEqual(blendState, true, `got expected value ${stringifyTypedArray(blendState)}`);
            // Verify changed state
            t.deepEqual(blendFuncSrcRGB, GL.ZERO, `got expected changed value ${stringifyTypedArray(blendFuncSrcRGB)}`);
            t.deepEqual(blendEquation, GL.FUNC_SUBTRACT, `got expected changed value ${stringifyTypedArray(blendEquation)}`);
        });
        parameters = getGLParameters(webglDevice.gl);
        blendFuncSrcRGB = parameters[GL.BLEND_SRC_RGB];
        blendEquation = parameters[GL.BLEND_EQUATION_RGB];
    ...
    t.deepEqual(clearColor, [0, 0, 0, 0], `got expected value ${stringifyTypedArray(clearColor)}`);
    t.deepEqual(blendState, false, `got expected value ${stringifyTypedArray(blendState)}`);
    t.deepEqual(blendFuncSrcRGB, GL.ONE_MINUS_SRC_ALPHA, `got expected initial value ${stringifyTypedArray(blendFuncSrcRGB)}`);
    t.deepEqual(blendEquation, GL.FUNC_ADD, `got expected initial value ${stringifyTypedArray(blendEquation)}`);
    t.end();
});
// EXT_blend_minmax
test('WebGLState#BlendEquationMinMax', async (t) => {
    ...
    for (const index in parametersArray) {
        ...
        for (const state in expected) {
            ...
            t.equal(value, expected[state], `WebGL : expected value, ${webglDevice.getGLKey(value)} received for ${webglDevice.getGLKey(state)}`);
        }
    }
    t.end();
});
...
test('WebGLState#withGLParameters framebuffer', async (t) => {
    ...
    withGLParameters(webglDevice.gl, { framebuffer: framebufferOne }, () => {
        ...
        fbHandle = getGLParameters(webglDevice.gl, GL.FRAMEBUFFER_BINDING);
        t.deepEqual(fbHandle, framebufferOne.handle, 'Inner withGLParameters should restore draw framebuffer binding');
    });
    fbHandle = getGLParameters(webglDevice.gl, GL.FRAMEBUFFER_BINDING);
