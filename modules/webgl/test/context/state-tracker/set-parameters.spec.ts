...
it('WebGL#composite setter', async () => {
  const webglDevice = await getWebGLTestDevice();
  ...
  for (const key of compositeStateKeys) {
    ...
    expect(value, `got expected default value ${stringifyTypedArray(value)}`).toEqual(
  ...
  expect(value, `got expected updated defuault value ${stringifyTypedArray(value)}`).toEqual(
...
it('WebGL#setGLParameters per-face stencil state', async () => {
  const webglDevice = await getWebGLTestDevice();
...
it('WebGLState#get all parameters', async () => {
  const webglDevice = await getWebGLTestDevice();
  ...
  for (const key in ENUM_STYLE_SETTINGS_SET1_PRIMITIVE) {
    ...
    ).toEqual(ENUM_STYLE_SETTINGS_SET1_PRIMITIVE[key]);
  }
...
it('WebGLRenderPass#setParameters stencil reference', async () => {
  const webglDevice = await getWebGLTestDevice();
...
it('WebGLState#setGLParameters framebuffer', async () => {
  const webglDevice = await getWebGLTestDevice();
...
it('WebGLState#setGLParameters read-framebuffer (WebGL2 only)', async () => {
  const webglDevice = await getWebGLTestDevice();
