...
import { getGLParameters, setGLParameters, resetGLParameters, WebGLStateTracker } from '@luma.gl/webgl';
...
test('WebGLStateTracker#push & pop', async (t) => {
    ...
    for (const key in GL_PARAMETER_DEFAULTS) {
        ...
        t.deepEqual(value, GL_PARAMETER_DEFAULTS[key], `default: got expected value ${stringifyTypedArray(value)} for key: ${key}`);
    }
    device.pushState();
    ...
    for (const key in ENUM_STYLE_SETTINGS_SET1) {
        const value = parameters[key];
        t.deepEqual(value, ENUM_STYLE_SETTINGS_SET1[key], `first set: got expected set value ${stringifyTypedArray(value)} for key: ${key}`);
    }
    device.pushState();
    // Set custom values and verify
    ...
    for (const [key, value] of Object.entries(ENUM_STYLE_SETTINGS_SET2)) {
        t.deepEqual(value, ENUM_STYLE_SETTINGS_SET2[key], `second set: got expected value ${stringifyTypedArray(value)} for key: ${key}`);
    }
    ...
    for (const key in ENUM_STYLE_SETTINGS_SET1) {
        const value = parameters[key];
        t.deepEqual(value, ENUM_STYLE_SETTINGS_SET1[key], `first pop: got expected value ${stringifyTypedArray(value)} for key: ${key}`);
    }
    device.popState();
    parameters = getGLParameters(gl);
    for (const key in GL_PARAMETER_DEFAULTS) {
        const value = parameters[key];
        t.deepEqual(value, GL_PARAMETER_DEFAULTS[key], `second pop: got expected value ${stringifyTypedArray(value)} for key: ${key}`);
    }
    t.end();
});
test('WebGLStateTracker#gl API', async (t) => {
    ...
    for (const key in GL_PARAMETER_DEFAULTS) {
        const value = parameters[key];
        t.deepEqual(value, GL_PARAMETER_DEFAULTS[key], `got expected value ${stringifyTypedArray(value)}`);
    }
    device.pushState();
    ...
    for (const key in ENUM_STYLE_SETTINGS_SET1) {
        ...
        if (!isNaN(Number(GL_PARAMETER_SETTERS[key]))) {
            ...
            t.deepEqual(value, ENUM_STYLE_SETTINGS_SET1[key], `got expected value ${stringifyTypedArray(value)}`);
        }
    }
    device.popState();
    parameters = getGLParameters(gl);
...
test('WebGLStateTracker#tracks metadata on gl.lumaState', async (t) => {
    ...
    t.equal(gl.lumaState, state, 'tracker is stored on gl.lumaState');
    // @ts-expect-error
    t.equal(gl.state, undefined, 'legacy gl.state metadata slot is not used');
    t.equal(
    // @ts-expect-error
    typeof (gl.lumaState?.cache || null), 'object', 'lumaState has cache object');
    t.end();
});
