...
async function createOffscreenWebGLDevice() {
...
test('WebGLPresentationContext delegates framebuffer sizing and present()', async (t) => {
    ...
    try {
        ...
        const defaultCanvas = defaultCanvasContext.canvas;
        ...
        destinationCanvas.getContext = ((contextId, options) => {
...
test('WebGLPresentationContext supports sequential presentation contexts', async (t) => {
    ...
    try {
        const defaultCanvas = device.getDefaultCanvasContext().canvas;
...
test('WebGLPresentationContext fails without a default canvas context', async (t) => {
    ...
    try {
        t.throws(() => device.createPresentationContext({ canvas: destinationCanvas }), /Device has no default CanvasContext/, 'constructor requires a default canvas context');
    }
    finally {
...
test('WebGLPresentationContext fails when default canvas is not offscreen', async (t) => {
    ...
    const destinationCanvas = document.createElement('canvas');
    t.throws(() => device.createPresentationContext({ canvas: destinationCanvas }), /requires the default CanvasContext canvas to be an OffscreenCanvas/, 'constructor requires an offscreen default canvas context');
    t.end();
});
test('WebGLPresentationContext fails when destination canvas has no 2d context', async (t) => {
    ...
    try {
        ...
        t.throws(() => device.createPresentationContext({ canvas: destinationCanvas }), /Failed to create 2d presentation context/, 'constructor requires a destination 2d context');
    }
    finally {
        device.destroy();
    }
...
test('WebGPUPresentationContext destroy() releases its depth attachment', async (t) => {
    ...
    const depthStencilAttachment = presentationContext.depthStencilAttachment;
    ...
    t.equal(presentationContext.depthStencilAttachment, null, 'destroy clears the cached depth attachment reference');
    t.end();
});
test('PresentationContext is unsupported on NullDevice', async (t) => {
    const nullDevice = await getNullTestDevice();
    t.throws(() => nullDevice.createPresentationContext({ width: 1, height: 1 }), /not supported/, 'NullDevice rejects presentation contexts');
    t.end();
});
