...
test('WebGL#TransformFeedback#setBuffers', async (t) => {
    ...
    t.deepEqual(transformFeedback.buffers, {
...
test('WebGL#TransformFeedback#capture', async (t) => {
    ...
    t.deepEqual(Array.from(outArray), Array.from(inArray).map(x => x * 2), 'transform feedback output');
    t.end();
});
/**
 ...
 */
function createModel(device, buffer, vertexCount) {
