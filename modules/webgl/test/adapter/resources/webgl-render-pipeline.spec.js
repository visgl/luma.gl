...
test('WEBGLRenderPipeline#uniformBlockBinding applies block indices in the correct argument position', async (t) => {
    ...
    for (const blockName of ['BlockA', 'BlockB', 'BlockC']) {
        ...
        t.equal(reportedBinding, expectedBindingPointByName[blockName], `${blockName} is bound to point ${expectedBindingPointByName[blockName]}`);
    }
    renderPass.end();
    vertexArray.destroy();
    bufferA.destroy();
