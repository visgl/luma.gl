...
export class WEBGLTexture extends Texture {
    ...
    _readColorTextureLayers(options, memoryLayout, readLayer) {
        ...
        const prevReadBuffer = this.gl.getParameter(GL.READ_BUFFER);
        const prevHandle = this.gl.bindFramebuffer(GL.FRAMEBUFFER, framebuffer.handle);
        try {
    ...
    _attachReadSubresource(framebuffer, mipLevel, layer) {
        ...
        switch (this.dimension) {
            case '2d':
                this.gl.framebufferTexture2D(GL.FRAMEBUFFER, GL.COLOR_ATTACHMENT0, GL.TEXTURE_2D, this.handle, mipLevel);
                break;
            case 'cube':
                this.gl.framebufferTexture2D(GL.FRAMEBUFFER, GL.COLOR_ATTACHMENT0, getWebGLCubeFaceTarget(this.glTarget, this.dimension, layer), this.handle, mipLevel);
                break;
            case '2d-array':
            case '3d':
                this.gl.framebufferTextureLayer(GL.FRAMEBUFFER, GL.COLOR_ATTACHMENT0, this.handle, mipLevel, layer);
                break;
            default:
