...
export class WEBGLTexture extends Texture {
  ...
  writeData(data: ArrayBuffer | ArrayBufferView, options_: TextureWriteOptions = {}): void {
    ...
    const memoryLayout = this.getTextureDataLayout(options);
    const {bytesPerRow, rowsPerImage} = memoryLayout;

    const byteAlignment = this.getBestByteAlignment(this.format, width);
  ...
  override readDataSyncWebGL(options_: TextureReadOptions = {}): ArrayBuffer {
    ...
    const dataLayout = this.getTextureDataLayout(options);
    const byteLength = dataLayout.bytesPerRow * dataLayout.imageCount * dataLayout.imageCount;
    ``;
    // const formatInfo = getTextureFormatInfo(format);
    // Allocate pixel array if not already available, using supplied type
    const shaderType = convertGLDataTypeToDataType(this.glType);

    const TypedArrayConstructor = getTypedArrayConstructor(shaderType);
    // const components = glFormatToComponents(this.glFormat);
    // TODO - check for composite type (components = 1).

    // TODO - does the type of the array matter?
    const targetArray = new TypedArrayConstructor(byteLength);
