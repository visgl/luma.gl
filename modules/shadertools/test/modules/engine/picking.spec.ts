...
const TEST_DATA = {
  vertexColorData: new Float32Array([0, 0, 0, 255, 100, 150, 50, 50, 50, 251, 103, 153,
  // is picked only when threshold is 5
  150, 100, 255, 254.5, 100, 150,
  // is picked with default threshold (1)
  100, 150, 255, 255, 255, 255, 255, 100, 149.5 // // is picked with default threshold (1)
  ])
};
const TEST_CASES = [{
...
test('picking#getUniforms', async () => {
  expect(getShaderModuleUniforms(picking, {}, {}), 'Empty input').toEqual({});
  expect(picking.getUniforms({
    isActive: true,
    highlightedObjectColor: undefined,
    highlightColor: [255, 0, 0]
  }), 'Undefined input (no change to highlighted object)').toEqual({
    isActive: true,
    isAttribute: false,
  ...
  expect(picking.getUniforms({
    ...
    highlightedObjectColor: null,
    highlightColor: [255, 0, 0]
  }), 'Null input (clear highlighted object)').toEqual({
    isActive: true,
    isAttribute: false,
  ...
  }), 'Picked input (set highlighted object)').toEqual({
  ...
  }), 'Override useFloatColors').toEqual({
    useFloatColors: false,
    isHighlightActive: true,
    highlightedObjectColor: [0, 0, 1],
    highlightColor: [0.4, 0, 0, 0.2]
...
test('picking#highlightedObjectColor', async () => {
...
test('picking#picking_setPickingColor', async () => {
  const device = await getWebGLTestDevice();
  if (!BufferTransform.isSupported(device)) {
    return;
  }
  ...
  transform.run();
  const outData = await readTransformOutput(transform, 'isPicked', vertexCount);
  expect(Array.from(outData), 'Zero picking color is marked invalid and all non-zero colors are pickable').toEqual([0, 1, 1, 1, 1, 1, 1, 1, 1]);
  transform.destroy();
});
function createPickingAlphaTransform(device, vertexColorData: Float32Array, vertexCount: number): BufferTransform {
  const vertexColor = device.createBuffer({
    data: vertexColorData
  });
  const isPicked = device.createBuffer({
    byteLength: vertexCount * Float32Array.BYTES_PER_ELEMENT
  });
...
async function readTransformOutput(transform: BufferTransform, varyingName: string, vertexCount: number): Promise<Float32Array> {
