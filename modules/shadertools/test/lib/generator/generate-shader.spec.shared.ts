...
`
  ...
  options: {
    shaderLanguage: 'glsl',
    uniforms: 'unscoped-interface-blocks'
  },
  result: `\
uniform Test {
...
`
  ...
  options: {
    shaderLanguage: 'glsl',
    uniforms: 'scoped-interface-blocks'
  },
  result: `\
uniform Test {
  mat4 uProjectMatrix;
...
export function registerGenerateShaderTests(): void {
  test('shadertools#generateGLSLForModule', () => {
    ...
    expect(() => generateShaderForModule({
      name: 'broken',
      uniformTypes: {
        light: [{
          color: 'vec3<f32>'
      ...
      shaderLanguage: 'wgsl'
    }), 'WGSL generation rejects composite uniform types').toThrow(/Composite uniform types/);
  });
}
