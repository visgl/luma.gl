...
test.skip('transpileGLSLShader', async (t) => {
    for (const tc of TRANSPILATION_TEST_CASES) {
        ...
        () => transpileGLSLShader(GLSL_300, stage), `${title} unknown glsl version`);
        // let assembleResult;
...
test('transpileGLSLShader#minified shaders', async (t) => {
    ...
    for (const tc of TRANSPILATION_TEST_CASES) {
        ...
        compareStrings(t, assembleResult, minifyShader(GLSL_300_TRANSPILED), `minified 3.00 => 3.00: ${title}`);
    }
    t.end();
});
