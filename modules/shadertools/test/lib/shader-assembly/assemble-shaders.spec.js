...
import { assembleGLSLShaderPair, picking, fp64, pbrMaterial } from '@luma.gl/shadertools';
...
test('assembleGLSLShaderPair#version_directive', async (t) => {
    ...
    t.equal(assembleResult.vs.indexOf('#version 300'), 0, 'version directive should be first statement');
    t.equal(assembleResult.fs.indexOf('#version 300'), 0, 'version directive should be first statement');
    t.end();
});
...
test('assembleGLSLShaderPair#shaderhooks', async (t) => {
    ...
    // Verify version directive remains as first line.
    t.ok(assembleResult.vs.indexOf('LUMAGL_pickColor') > -1, 'hook function injected into vertex shader');
    t.ok(assembleResult.fs.indexOf('LUMAGL_fragmentColor') > -1, 'hook function injected into fragment shader');
    t.ok(assembleResult.fs.indexOf('if (color.a == 0.0) discard;') > -1, 'hook header injected into fragment shader');
    t.ok(assembleResult.vs.indexOf('picking_setPickingColor(color.rgb)') === -1, 'injection code not included in vertex shader without module');
    t.ok(assembleResult.fs.indexOf('color = picking_filterColor(color)') === -1, 'injection code not included in fragment shader without module');
    t.ok(assembleResult.fs.indexOf('fragmentColor = picking_filterColor(fragmentColor)') === -1, 'regex injection code not included in fragment shader without module');
    assembleResult = assembleGLSLShaderPair({
        platformInfo: getInfo(webglDevice),
        vs: VS_GLSL_300,
        fs: FS_GLSL_300,
    ...
    t.ok(assembleResult.vs.indexOf('LUMAGL_pickColor') > -1, 'hook function injected into vertex shader');
    t.ok(assembleResult.fs.indexOf('LUMAGL_fragmentColor') > -1, 'hook function injected into fragment shader');
    t.ok(assembleResult.vs.indexOf('picking_setPickingColor(color.rgb)') > -1, 'injection code included in vertex shader with module');
    t.ok(assembleResult.fs.indexOf('color = picking_filterColor(color)') > -1, 'injection code included in fragment shader with module');
    t.ok(assembleResult.fs.indexOf('color.a *= 1.2;') >
        assembleResult.fs.indexOf('color = picking_filterColor(color)'), 'hook footer injected after injection code');
    t.ok(assembleResult.fs.indexOf('fragmentColor = picking_filterColor(fragmentColor)') > -1, 'regex injection code included in fragment shader with module');
    assembleResult = assembleGLSLShaderPair({
        platformInfo: getInfo(webglDevice),
        vs: VS_GLSL_300,
        fs: FS_GLSL_300,
    ...
    t.ok(assembleResult.vs.indexOf('color *= 0.1') > -1, 'argument injection code included in shader hook');
    t.ok(assembleResult.fs.indexOf('color += 0.1') > -1, 'argument injection code included in shader hook');
    t.ok(assembleResult.fs.indexOf('color += 0.1') <
        assembleResult.fs.indexOf('color = picking_filterColor(color)'), 'argument injection code injected in the correct order');
    assembleResult = assembleGLSLShaderPair({
        platformInfo: getInfo(webglDevice),
        vs: VS_GLSL_300,
        fs: FS_GLSL_300,
    ...
    t.ok(assembleResult.fs.indexOf('color.r = 1.0') > -1, 'module injection code included in shader hook');
    t.ok(assembleResult.fs.indexOf('color.r = 1.0') <
        assembleResult.fs.indexOf('color = picking_filterColor(color)'), 'module injection code injected in the correct order');
    assembleResult = assembleGLSLShaderPair({
        platformInfo: getInfo(webglDevice),
        vs: VS_GLSL_300,
        fs: FS_GLSL_300,
    ...
    t.ok(assembleResult.fs.indexOf('fragmentColor -= 0.1;') > -1, 'regex injection code included in shader hook');
    t.end();
});
test('assembleGLSLShaderPair#injection order', async (t) => {
    ...
    t.ok(compileAndLinkShaders(t, webglDevice, assembleResult), 'Hook functions have access to injected variables.');
    assembleResult = assembleGLSLShaderPair({
        platformInfo: getInfo(webglDevice),
        vs: VS_GLSL_300_MODULES,
        fs: FS_GLSL_300_MODULES,
    ...
    t.ok(compileAndLinkShaders(t, webglDevice, assembleResult), 'Hook functions have access to injected variables through modules.');
    t.end();
});
...
test.skip('assembleGLSLShaderPair#transpilation', async (t) => {
    ...
    if (isBrowser() && extension) {
        ...
        t.ok(compileAndLinkShaders(t, webglDevice, assembleResult), 'Deck shaders transpile 300 to 100 valid program');
    }
    assembleResult = assembleGLSLShaderPair({
        platformInfo: getInfo(webglDevice),
        vs: VS_GLSL_300_GLTF,
    ...
    t.ok(compileAndLinkShaders(t, webglDevice, assembleResult), 'assemble GLSL300 + PBR assemble, WebGL2');
    t.end();
});
// HELPERS
function compileAndLinkShaders(t, device, assembleResult) {
    const gl = device.gl;
