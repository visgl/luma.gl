...
test('injectShader#injectShader', async (t) => {
    ...
    t.ok(fuzzySubstring(injectResult, VS_GLSL_RESOLVED_DECL), 'declarations correctly injected in vertex shader');
    t.ok(fuzzySubstring(injectResult, VS_GLSL_RESOLVED_MAIN), 'main correctly injected in vertex shader');
    ...
    t.ok(fuzzySubstring(injectResult, FS_GLSL_RESOLVED_DECL), 'declarations correctly injected in vertex shader');
    t.ok(fuzzySubstring(injectResult, FS_GLSL_RESOLVED_MAIN), 'main correctly injected in vertex shader');
    t.ok(/#endif\s*$/.test(injectResult), 'standard stubs injected');
    t.end();
});
test('injectShader#assembleGLSLShaderPair', async (t) => {
    ...
    t.ok(fuzzySubstring(assembleResult.vs, VS_GLSL_RESOLVED_DECL), 'declarations correctly assembled in vertex shader');
    t.ok(fuzzySubstring(assembleResult.vs, VS_GLSL_RESOLVED_MAIN), 'main correctly assembled in vertex shader');
    t.ok(fuzzySubstring(assembleResult.fs, FS_GLSL_RESOLVED_DECL), 'declarations correctly assembled in vertex shader');
    t.ok(fuzzySubstring(assembleResult.fs, FS_GLSL_RESOLVED_MAIN), 'main correctly assembled in vertex shader');
    t.end();
});
