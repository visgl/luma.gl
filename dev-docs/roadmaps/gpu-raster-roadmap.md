...
- **Status:** Active; foundation, tiled analysis, dense components, region metrics, and complete cross-tile segmentation implemented
- **Experimental API:** `@luma.gl/experimental/gpu-raster`
...
GPURaster is an independently implemented WebGPU raster-analysis toolkit inspired by the
...
buffers. GPURaster contributors declare explicit compute, copy, and optional render passes against
...
Start with [GPURaster Concepts and Execution Model](/docs/api-reference/experimental/gpu-raster/concepts)
for the user-facing vocabulary, data-flow model, operation selection, and ownership rules. The
[GPURaster API reference](/docs/api-reference/experimental/gpu-raster) documents shipped classes and
...
1. Add a side-effect-free `./gpu-raster` subpath to the existing experimental package.
...
9. Keep browser decoding, GeoTIFF/COG access, worker pools, deck.gl rendering, Arrow adaptation,
   and coordinate reprojection outside the GPURaster runtime.
...
- Promising complete cuCIM feature parity. NDVI, marching squares, and possible watershed support
  are independent GPURaster capabilities, not verified cuCIM public-API equivalents.
...
optional `@luma.gl/experimental/lufft` subpath rather than inside `./gpu-raster`. Preserve the
...
| LuFFT tranche | Outcome                                                                                 | GPURaster dependency |
| ------------- | --------------------------------------------------------------------------------------- | ------------------- |
...
GPURaster tranche 7.4 consumes LuFFT L1–L3 when spectral convolution is justified. LuFFT and
ocean migration do not block the GPURaster package or raster-contract pull requests.
...
[GPURaster API reference](/docs/api-reference/experimental/gpu-raster) for authoritative signatures.
...
independent resources unless an explicit GPURaster bridge contributor connects them. The runtime
...
| 0.2     | `./gpu-raster` ESM/CommonJS/types subpath and package-boundary tests            | 0.1                                         | Small  |
...
**Entry:** Existing graph texture, geospatial, and GPUCrossfilter contracts are available.
...
**Work:** Add `modules/experimental/src/gpu-raster/index.ts` and the `./gpu-raster` conditional
`import`/`require`/`types` export. Add an isolated package-boundary node test. Generalize the
built-package verifier or add an analogous independently runnable GPURaster verifier.

**Exit evidence:** Built ESM, CommonJS, and declarations resolve from
`@luma.gl/experimental/gpu-raster`. Raster runtime symbols do not appear in the main experimental
barrel. Existing geospatial/GPUCrossfilter imports, `sideEffects: false`, and package privacy remain
unchanged.
...
dependency enters the GPURaster package.
...
**Work:** Add `modules/experimental/src/gpu-raster/README.md` and
`docs/api-reference/experimental/gpu-raster/` guides. Add the raster category to both experimental
...
```text
modules/experimental/src/gpu-raster/
  index.ts
  types.ts
  gpu-raster.ts
  raster-utils.ts
...
modules/experimental/test/gpu-raster/
  index.ts
  gpu-raster-package-boundary.node.spec.ts
...
- `package.json`: expose an explicit built-package verification command without changing
  unrelated scripts.
- `docs/api-reference/experimental/gpu-raster/`: public runtime documentation.
...
GPURaster implementation rules:
...
and rendering in GPURaster and the caller-owned GPU pipeline.
...
`gpu-raster/index.ts`, built ESM/CommonJS/declaration verification, root-isolation tests, and tiny
