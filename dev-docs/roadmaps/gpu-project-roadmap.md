...
| P.5a.1 — Lambert Conformal Conic | Add bounded forward/inverse LCC operations, explicit PROJJSON/pipeline lowering, and native/adaptive benchmark fixtures | Landed P.3/P.4 conventions | Supported standard-parallel variants, hemispheres, singularities, and inverse branches match an independent oracle; all input encodings and inline/materialized paths agree; default double-single accuracy is retained | Next | Medium |
| P.5a.2 — Albers Equal Area | Add bounded forward/inverse Albers operations and planner lowering, reusing the conic validation/benchmark conventions | P.5a.1 conventions | Standard-parallel variants, hemispheres, equal-area properties, invalid parameters, and inverse domains have independent oracle and hardware coverage; no custom consumer code or precision downgrade | Planned | Medium |
...
1. **Next: P.5a.1, then P.5a.2.** Keep each family independently reviewable, with planner lowering,
   domain/inverse rules, independent oracle and GPU tests, documentation, and benchmark fixtures.
   LCC before Albers is the proposed implementation order, not a measured demand ranking; consumer
   requirements can reverse it. Stack.gl supplies scope inspiration, not an implementation to port.
...
3. **Use evidence to choose P.8b/P.8c or P.6.** The current shader still scans patches unless an
