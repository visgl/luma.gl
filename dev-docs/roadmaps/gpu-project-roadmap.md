...
| P.2b — Program numerical metadata | Generalize operation dimension/domain metadata for CRS lowering and report composed precision/error requirements | P.2a and P.3 frontend requirements | Programs expose inspectable dimensions, stage domains, and error propagation; per-stage sampled tolerance is not mistaken for a composed guarantee | Planned | Medium |
