...
Treat `needsRedraw()` as an invalidation signal. State changes set it; reading it clears it. The application or `AnimationLoop` decides whether to request and render a frame. See [redraw detection](https://luma.gl/next/docs/api-guide/engine/redraw.md).
...
* Add [interaction](https://luma.gl/next/docs/api-guide/engine/interactivity.md) for orbit controls, GPU picking, and hover/selection feedback.
* Add a [scenegraph](https://luma.gl/next/docs/api-guide/engine/scenegraph.md) when parent/child transforms and traversal are genuinely useful.
* Add [animation](https://luma.gl/next/docs/api-guide/engine/animation.md) for clips, tracks, blending, and morphs.
* Add [compute helpers](https://luma.gl/next/docs/api-guide/engine/transforms.md) for a bounded compute or transform workflow; use GPU Core for a multi-stage scheduled dataflow.
...
| -------------------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| Copy a complete small task       | [Engine cookbook](https://luma.gl/next/docs/api-guide/engine/cookbook.md)    | Begin with the smallest complete lifecycle.                 |
| Pass module props to shaders     | [Shader inputs](https://luma.gl/next/docs/api-guide/engine/shader-inputs.md) | Keep typed module values separate from resource bindings.   |
| Avoid idle rendering             | [Redraw detection](https://luma.gl/next/docs/api-guide/engine/redraw.md)     | Invalidation is a signal; frame scheduling remains yours.   |
| Add camera or object interaction | [Interactivity](https://luma.gl/next/docs/api-guide/engine/interactivity.md) | Route events first, then derive controls or picking.        |
| Organize hierarchical transforms | [Scenegraphs](https://luma.gl/next/docs/api-guide/engine/scenegraph.md)      | Use hierarchy for relationships, not merely as a container. |
| Play and blend authored motion   | [Animation](https://luma.gl/next/docs/api-guide/engine/animation.md)         | Advance animation state before deciding to redraw.          |
| Run a bounded GPU transform      | [Compute helpers](https://luma.gl/next/docs/api-guide/engine/transforms.md)  | Select by backend and data shape.                           |
...
* Start from the [Engine cookbook](https://luma.gl/next/docs/api-guide/engine/cookbook.md).
* Use the [Engine API overview](https://luma.gl/next/docs/api-reference/engine.md) to find exact contracts.
* Drop to the [Core programming guide](https://luma.gl/next/docs/api-guide/gpu.md) for resource and command control.
* Move to [GPU Core](https://luma.gl/next/docs/api-reference/experimental/gpu-core.md) when the workload becomes a scheduled GPU pipeline rather than a collection of draws.
