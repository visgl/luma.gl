...
* `setNeedsRedraw(reason: string)` — mark the object as needing a redraw. The optional `reason` string is stored for debugging.
* `needsRedraw(): false | string` — returns `false` if no redraw is pending, or the `reason` string. Calling this method clears the flag.

## Redraw flag semantics[​](#redraw-flag-semantics "Direct link to Redraw flag semantics")
...
## Classes supporting `needsRedraw`[​](#classes-supporting-needsredraw "Direct link to classes-supporting-needsredraw")
