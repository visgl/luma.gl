...
Texture filtering solves texture minification and magnification aliasing. It is separate from polygon coverage antialiasing; see [Antialiasing and Multisampling](https://luma.gl/next/docs/api-guide/gpu/gpu-antialiasing.md) for how the techniques fit together. Sampling is a highly configuratble process that is specified using by binding a separate `Sampler` object for each texture.
...
Texture data can be copied between GPU and CPU and vice versa, via GPU Buffers. See [Issuing GPU Commands](https://luma.gl/next/docs/api-guide/gpu/gpu-commands.md) for when those copies should be recorded explicitly versus issued through resource-level convenience methods.
...
* [`VideoTexture`](https://luma.gl/next/docs/api-reference/engine/video-texture.md):
  <!-- -->
  * for live `HTMLVideoElement` or `VideoFrame` bindings that should update across draws; see [Working With Video Textures](https://luma.gl/next/docs/api-guide/gpu/video-textures.md)
* [HTML-in-Canvas](https://luma.gl/next/docs/api-guide/gpu/html-in-canvas.md):
  <!-- -->
...
For supported compressed texture formats, see [Texture Formats](https://luma.gl/next/docs/api-reference/core/texture-formats.md).
