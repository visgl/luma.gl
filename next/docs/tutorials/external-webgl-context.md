...
## Attach luma.gl to MapLibre[​](#attach-lumagl-to-maplibre "Direct link to Attach luma.gl to MapLibre")
...
## Handle map resizes[​](#handle-map-resizes "Direct link to Handle map resizes")
...
If you are matching an overlaid map canvas such as deck.gl `MapboxOverlay` or a MapLibre custom overlay that still sizes its drawing buffer with legacy `Math.round(css * devicePixelRatio)` logic, opt into compatibility sizing when creating the canvas context:
...
Keep the default `'exact'` mode when you want luma.gl to track the browser's precise device-pixel content box.

## See it in action[​](#see-it-in-action "Direct link to See it in action")

The revived [External WebGL Context example](https://luma.gl/next/examples/integrations/external-context) renders a luma.gl overlay driven by MapLibre's WebGL render loop and uses the `webgl2Adapter.attach` API to keep both frameworks in sync.
