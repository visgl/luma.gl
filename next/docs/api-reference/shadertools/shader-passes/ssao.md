...
The evaluation stage visits 12 nearby depth samples; two subsequent [Depth-Aware Blur](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/depth-aware-blur.md) passes preserve foreground/background boundaries. `resolutionScale: 0.5` reduces intermediate pixel count to approximately one quarter. SSAO does not maintain temporal history and composites onto the complete source color.

Choose [GTAO](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/gtao.md) when horizon integration, temporal stabilization, or ambient-only composition is required.

## Related Effects[​](#related-effects "Direct link to Related Effects")

* [GTAO](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/gtao.md) adds horizon-based visibility, temporal history, and ambient-only composition.
* [Depth-Aware Blur](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/depth-aware-blur.md) explains the bilateral denoising stages.
* [Outlines](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/outlines.md) can consume the same depth and normal attachments.
