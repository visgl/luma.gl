...
Two fullscreen passes are always used, and larger radii increase sampling work inside each pass. For very wide glow, the multiscale [Bloom](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/bloom.md) pipeline reduces work by filtering smaller pyramid levels instead of sampling a large full-resolution neighborhood. Unlike [Depth-Aware Blur](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/depth-aware-blur.md), Gaussian blur intentionally mixes across depth edges.

## Related Effects[​](#related-effects "Direct link to Related Effects")

* [Triangle Blur](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/triangle-blur.md) uses a triangular rather than Gaussian weight profile.
* [Depth-Aware Blur](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/depth-aware-blur.md) preserves depth discontinuities.
* [Bloom](https://luma.gl/next/docs/api-reference/shadertools/shader-passes/bloom.md) combines multiscale blur with highlight extraction and reconstruction.
