...
`GPUVector` represents one logical typed table column over one or more [`GPUData`](https://luma.gl/next/docs/api-reference/gpgpu/gpu-data.md) chunks. It does not own a buffer directly; every concrete buffer lives on a `GPUData` object in `GPUVector.data[]`.
