...
[`GPUData.getChild()`](https://luma.gl/next/docs/api-reference/gpgpu/gpu-data.md) returns a `GPUDataView` for one named field in an interleaved struct. Views can also be constructed directly or derived from an explicit `BufferLayout` attribute.
