...
* Move GPU data conversion to the GPU when possible.
...
## Data Inputs[​](#data-inputs "Direct link to Data Inputs")
...
## Direct Vectors[​](#direct-vectors "Direct link to Direct Vectors")
...
## GPU-Resident Inputs[​](#gpu-resident-inputs "Direct link to GPU-Resident Inputs")
...
## Picking[​](#picking "Direct link to Picking")
...
## GeoArrow[​](#geoarrow "Direct link to GeoArrow")
...
## Meshes And Point Clouds[​](#meshes-and-point-clouds "Direct link to Meshes And Point Clouds")
...
## Time Handling[​](#time-handling "Direct link to Time Handling")
...
Temporal conversion should normalize Arrow temporal values into relative GPU values while preserving enough metadata to recover the source meaning. Absolute `Date`, `Time`, and `Timestamp` values need an origin and unit; `Duration` values can use origin `0`.
...
## 64-Bit Precision[​](#64-bit-precision "Direct link to 64-Bit Precision")

Arrow inputs should be able to preserve 64-bit source data even when a layer converts a lower-level GPU representation for rendering.
...
Layer adapters should choose an explicit GPU conversion strategy instead of silently pretending every 64-bit value is an ordinary `Float32`. Useful strategies include direct `Float32` repacking when precision loss is acceptable, origin-relative `Float32` for local coordinates, high/low word-pair storage for high-precision positions, and bit-preserving integer storage for identifiers or DGGS cells.
...
## HDR Color Handling[​](#hdr-color-handling "Direct link to HDR Color Handling")
...
## Transitions / Animations[​](#transitions--animations "Direct link to Transitions / Animations")
