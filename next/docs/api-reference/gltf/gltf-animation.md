...
See the [engine animation guide](https://luma.gl/next/docs/api-guide/engine/animation.md) and [AnimationMixer API reference](https://luma.gl/next/docs/api-reference/engine/animation/animation-mixer.md) for pause, seek, reverse playback, once/repeat/ping-pong loops, weighted blending, and crossfading.

To share GPU models across independently posed actors, see [GPU-animated glTF crowds](https://luma.gl/next/docs/api-reference/gltf/gltf-animated-crowd.md).
...
Extras, structural material switches such as `alphaMode`, `doubleSided`, or `unlit`, animated `texCoord`, and `TEXCOORD_2+` are not supported. See [native glTF extensions](https://luma.gl/next/docs/api-reference/gltf/gltf-native-extensions.md) for the full target matrix and strict extension diagnostics.
