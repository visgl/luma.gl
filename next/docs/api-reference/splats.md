...
The renderer supports `none`, `global`, and `tile` depth-ordering modes alongside camera matrix, viewport, radius, opacity, and visibility controls. WebGPU uses GPU-ready splat buffers and WGSL; WebGL2 uses an attribute-backed GLSL fallback. Higher-order spherical harmonics and dedicated WebGPU picking are not part of the initial API. When globally sorted source batches are densely interleaved, the renderer bounds draw-call growth by grouping the rows into depth-ordered batch runs without changing or repacking their source buffers.
