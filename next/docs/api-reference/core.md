...
The curated pages explain workflows and portability. The [generated API index](https://luma.gl/next/docs/api-reference/generated/core.md) is the exact, source-linked TypeScript contract.

## When to use it[​](#when-to-use-it "Direct link to When to use it")

Use Core when you need direct control over GPU memory, formats, bindings, passes, or command submission. Start one layer higher with [Engine](https://luma.gl/next/docs/api-reference/engine.md) when a `Model` can manage those details. Move one layer higher still to [GPU Core](https://luma.gl/next/docs/api-reference/experimental/gpu-core.md) when several operations need dependency scheduling, transient storage, indirect work, or multi-frame execution.
...
1. [Learn the workflow](https://luma.gl/next/docs/api-guide/gpu.md)Build the mental model before choosing classes.
2. [Copy a focused recipe](https://luma.gl/next/docs/api-guide/gpu/cookbook.md)Start from a complete, small task.
3. [Check the complete API](https://luma.gl/next/docs/api-reference/generated/core.md)Confirm exact types, defaults, and ownership.

The [Core cookbook](https://luma.gl/next/docs/api-guide/gpu/cookbook.md) has compact recipes for initialization, upload, render, compute, readback, resize, and recovery from validation or device errors.
...
The [generated <!-- -->Core<!-- --> API index](https://luma.gl/next/docs/api-reference/generated/core.md) is the exhaustive, source-linked inventory of every public value and TypeScript export. The curated pages explain how the related families fit together.
...
* Move up to [Engine](https://luma.gl/next/docs/api-reference/engine.md) for managed models and redraw state.
* Move down to [Shadertools](https://luma.gl/next/docs/api-reference/shadertools.md) to author reusable shader behavior.
* Add [GPU Core](https://luma.gl/next/docs/api-reference/experimental/gpu-core.md) for multi-operation scheduling.
