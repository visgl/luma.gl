...
Use [`GPUHashIndex`](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-hash-index.md) when the entire right side already occupies one packed chunk. Use [`GPUBatchHashJoin`](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-batch-hash-join.md) when the left side also has several chunks; the right-index choice and left-join choice are independent.
