...
Use [`GPUGridBinning`](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-grid-binning.md) for population or occupancy alone. Use [`GPUGroupAggregation`](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-group-aggregation.md) when rows already carry categorical IDs rather than continuous positions, and use a spatial index when the application needs the individual objects in a queried region instead of one statistic per cell.
...
Counts remain independently available from [`GPUGridBinning`](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-grid-binning.md) when an application needs both population and value statistics. Irregular spatial bins, higher-dimensional aggregates, variance, and custom associative operations remain future work.
