...
| **Reads / writes**       | Uses each adapter’s normal graph resources and reads back exact stable IDs plus compact metrics.     |
| **Ownership**            | The harness owns benchmark fixtures; adapters retain their documented graph-resource ownership.      |
| **Output contract**      | Oracle-checked timings, memory, candidates, traversal work, and build/refit/query classifications.   |
| **Expected work**        | Explicit warmup and measured iterations for each selected strategy.                                  |
| **Chunks**               | Benchmark inputs and index layouts are reported rather than hidden.                                  |
| **Conditions / budgets** | Runs only when requested and synchronizes measurements deliberately.                                 |
...
**Common mistake**Do not compare query time without including build amortization and candidate counts.
