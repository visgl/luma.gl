...
`GPUTextureHistory` owns exactly two descriptor-identical, caller-configured textures and rotates their previous/current roles without copying texels. Its bindings plug directly into [`GPUCommandGraph.encode()`](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-command-graph.md#compiledgpucommandgraph), allowing one compiled graph to reuse retained radiance, temporal metadata, simulation state, or feedback buffers across frames.
...
| **Reads / writes**       | A graph reads the previous role and writes the current role; advance() swaps them.              |
| **Ownership**            | The history owns exactly two textures; compiled graphs borrow their bindings.                   |
| **Output contract**      | Descriptor-identical alternating textures with stable logical graph identifiers.                |
| **Expected work**        | No GPU commands or copies by itself; rotation is a CPU-side reference swap.                     |
| **Chunks**               | Not applicable to texture history.                                                              |
| **Conditions / budgets** | Conditioned writers must explicitly preserve or invalidate skipped history.                     |
...
**Common mistake**Do not sample and write the same physical texture in one temporal step.
...
See [physical texture overlap and writable aliases](https://luma.gl/next/docs/api-reference/experimental/gpu-core/gpu-command-graph.md#physical-texture-overlap-and-writable-aliases) for the complete graph contract.
