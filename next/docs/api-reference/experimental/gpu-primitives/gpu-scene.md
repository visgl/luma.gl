...
`groupId`, `geometryId`, and `commandSlot` default to `GPU_SCENE_INVALID_REFERENCE`. They are application-defined keys, not handles owned or destroyed by `GPUScene`. In particular, a command slot names where a later workflow may write; this tranche does not allocate or encode indirect draw commands. That explicit boundary avoids coupling scene storage to one renderer or table package.
