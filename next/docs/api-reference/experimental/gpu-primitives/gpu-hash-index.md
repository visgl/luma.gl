...
This first slice supports packed `uint32` keys and values and full rebuilds. Deletion and tombstones, preserved vector chunks, 64-bit keys, custom hash callbacks, partitioned tables, and join materialization remain future contracts. Starting with exact lookup keeps those additions grounded in real consumers rather than baking a general-purpose database into the primitive.
