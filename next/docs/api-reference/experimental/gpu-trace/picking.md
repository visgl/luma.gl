...
`getGPUTracePickingShader` generates a small WebGPU compute shader that resolves a timeline coordinate into the lowest matching visible canonical span row. It belongs to the optional [`@luma.gl/experimental/gpu-trace`](https://luma.gl/next/docs/api-reference/experimental/gpu-trace.md) submodule because its inputs understand execution-span timing, process/thread lane topology, hierarchy collapse, and trace-specific visibility policies.
...
| **Reads / writes**       | Reads span timing, effective lanes, and visibility; atomically writes one bounded picking record.                 |
| **Ownership**            | The helper supplies WGSL; the application owns the graph node, request/result storage, readback, and hover state. |
| **Output contract**      | One canonical span identity or an explicit no-hit sentinel.                                                       |
| **Expected work**        | One bounded candidate visit for the current picking request.                                                      |
| **Chunks**               | Results use canonical IDs independent of compact display position.                                                |
| **Conditions / budgets** | Encode on pointer invalidation and reject stale asynchronous generations.                                         |
...
**Common mistake**Do not confuse the compacted draw index with the canonical span ID.
...
[`GPUTraceInteraction`](https://luma.gl/next/docs/api-reference/experimental/gpu-trace/interaction.md) already produces exclusive-scanned thread offsets and a source-aligned final visibility mask. The picking shader consumes those buffers directly; it does not recompute hierarchy layout, traverse dependencies, or create its own filtering policy.
