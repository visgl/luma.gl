...
`createABufferResolveCompositeShaderPass({maxFragmentsPerPixel})` exposes the same one-slice fullscreen resolve used internally by `ABufferRenderer`. The renderer invokes it once per captured slice so storage buffers can be reused without allocating full-frame fragment storage.
...
4. Runs `createABufferResolveCompositeShaderPass()` to sort records back-to-front and composite premultiplied color over the previous slice result.
