...
`GPUInputSchema` is the runtime contract a model or renderer uses to declare the prepared varying [`GPUVector`](https://luma.gl/next/docs/api-reference/gpgpu/gpu-vector.md) and constant [`GPUConstant`](https://luma.gl/next/docs/api-reference/gpgpu/gpu-constant.md) inputs it accepts. Models expose the contract as a static `gpuInputSchema` property so renderers, adapters, validation helpers, and application code can inspect the same declaration.

It is distinct from [`GPUSchema`](https://luma.gl/next/docs/api-reference/experimental/gpu-tables/gpu-schema.md):
...
```
import type {GPUConstant, GPUVector, GPUVectorFormat} from '@luma.gl/gpgpu/gpu-data';
...
```
import type {GPUInputSchema} from '@luma.gl/experimental/gpu-tables';
...
```
import {validateGPUInputVectors} from '@luma.gl/experimental/gpu-tables';
...
For inputs that already live in a `GPUTable` and map directly to shader attributes or storage bindings, use [`GPUTableShaderBindings`](https://luma.gl/next/docs/api-reference/experimental/gpu-tables/gpu-table-shader-bindings.md) to resolve batch-preserving draw resources.
