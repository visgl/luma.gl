...
## Attribute Resolution[​](#attribute-resolution "Direct link to Attribute Resolution")

For varying inputs, `columnName` selects the table layout. `attributeName` selects one ordinary shader attribute, while `attributeNames` selects several shader-visible views for a composite input. The selected names must exist in the table `BufferLayout`. Shared physical layouts are bound once even when they expose several attributes, and inactive attribute views are omitted.
...
## Storage Resolution[​](#storage-resolution "Direct link to Storage Resolution")
...
## GPUTableModel Integration[​](#gputablemodel-integration "Direct link to GPUTableModel Integration")
