...
A `Framebuffer` holds textures that will be used as render targets for `RenderPipeline`s together with additional information on how the `RenderPipeline` should use the various attached textures:
...
The use of framebuffers is described in detail in the [Rendering Guide](https://luma.gl/next/docs/api-guide/gpu/gpu-rendering.md).
...
luma.gl does not yet expose a managed offscreen MSAA resolve path. For the current WebGL and WebGPU landscape, and the proposed color-only framebuffer API, see [Antialiasing and Multisampling](https://luma.gl/next/docs/api-guide/gpu/gpu-antialiasing.md) and [RFC #2741](https://github.com/visgl/luma.gl/issues/2741).

## Overview[​](#overview "Direct link to Overview")

### Framebuffer Attachment Values[​](#framebuffer-attachment-values "Direct link to Framebuffer Attachment Values")
...
## Framebuffer Attachments[​](#framebuffer-attachments "Direct link to Framebuffer Attachments")
...
### `FramebufferProps`[​](#framebufferprops "Direct link to framebufferprops")
...
* The `Framebuffer` class wraps the `WebGLFramebuffer` object, see e.g. [Framebuffer](https://www.khronos.org/opengl/wiki/Framebuffer) and [Framebuffer Object](https://www.khronos.org/opengl/wiki/Framebuffer_Object) in the OpenGL Wiki.
