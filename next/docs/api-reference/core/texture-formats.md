...
Support for some types of texture formats come in groups. As an alternative to checking texture support format-by-format, [`Device.features`](https://luma.gl/next/docs/api-reference/core/device-features.md) can be queried to determine if specific groups or classes of texture formats are supported.
