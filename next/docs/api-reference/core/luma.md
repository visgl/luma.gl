...
The [`luma`](https://luma.gl/next/docs/api-reference/core/luma.md) namespace provides luma.gl applications with the ability to create `Device` class instances against GPU `Adapters` that bring support for different GPU backends such as WebGPU and WebGL.

The returned [`Device`](https://luma.gl/next/docs/api-reference/core/device.md) instances provide luma.gl applications with a complete GPU API.

## Device Registration[​](#device-registration "Direct link to Device Registration")
...
It is possible to register more than one device to create an application that can work in both WebGL and WebGPU environments (choosing what to use depending on what the current browser supports).
...
## Registering Adapters[​](#registering-adapters "Direct link to Registering Adapters")
...
### `CreateDeviceProps`[​](#createdeviceprops "Direct link to createdeviceprops")

Properties for creating a new device. See [`DeviceProps`](https://luma.gl/next/docs/api-reference/core/device.md#deviceprops) for device specific options.
...
### `AttachDeviceProps`[​](#attachdeviceprops "Direct link to attachdeviceprops")

Properties for attaching an existing WebGL context or WebGPU device to a new luma Device. See [`DeviceProps`](https://luma.gl/next/docs/api-reference/core/device.md#deviceprops) for device specific options.
...
### `luma.createDevice()`[​](#lumacreatedevice "Direct link to lumacreatedevice")
...
* `...deviceProps`: See [`DeviceProps`](https://luma.gl/next/docs/api-reference/core/device.md#deviceprops) for device specific options.
...
### `luma.attachDevice()`[​](#lumaattachdevice "Direct link to lumaattachdevice")
...
### `luma.registerAdapters()`[​](#lumaregisteradapters "Direct link to lumaregisteradapters")
...
### `luma.enforceWebGL2()`[​](#lumaenforcewebgl2 "Direct link to lumaenforcewebgl2")
