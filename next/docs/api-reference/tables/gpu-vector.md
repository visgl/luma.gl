...
`GPUVector` represents one logical typed table column over one or more [`GPUData`](https://luma.gl/next/docs/api-reference/tables/gpu-data.md) chunks. It does not own a buffer directly; every concrete buffer lives on a `GPUData` object in `GPUVector.data[]`.
...
When the input starts as Apache Arrow, prefer [`makeGPUVectorFromArrow()`](https://luma.gl/next/docs/api-reference/arrow/supported-arrow-types.md) from `@luma.gl/arrow`.
