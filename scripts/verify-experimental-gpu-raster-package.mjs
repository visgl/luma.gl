...
const gpuRasterExport = packageJson.exports?.['./gpu-raster'];
...
assert.deepEqual(gpuRasterExport, {
  import: './dist/gpu-raster/index.js',
  require: './dist/gpu-raster/index.cjs',
  types: './dist/gpu-raster/index.d.ts'
});

const ecmaScriptModuleEntry = path.resolve(packageRoot, gpuRasterExport.import);
const commonJsEntry = path.resolve(packageRoot, gpuRasterExport.require);
const declarationEntry = path.resolve(packageRoot, gpuRasterExport.types);
...
const tileSourceImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-tile-source.ts'),
  'utf8'
);
const tileCacheImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-tile-cache.ts'),
  'utf8'
);
const tileHaloImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-tile-halo.ts'),
  'utf8'
);
const overviewImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-overview.ts'),
  'utf8'
);
const globalStatisticsImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-global-statistics.ts'),
  'utf8'
);
const connectedComponentsImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-connected-components.ts'),
  'utf8'
);
const denseComponentsImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-dense-components.ts'),
  'utf8'
);
const regionMeasurementsImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-region-measurements.ts'),
  'utf8'
);
const crossTileComponentsImplementation = readFileSync(
  path.join(packageRoot, 'src/gpu-raster/gpu-raster-cross-tile-components.ts'),
  'utf8'
);

assert.doesNotThrow(() => readFileSync(declarationEntry, 'utf8'), 'GPURaster declarations exist');
assert.doesNotMatch(
...
assert.deepEqual(
  ...
  'ESM and CommonJS expose the same optional GPURaster runtime symbols'
);
...
const temporaryDirectory = mkdtempSync(path.join(tmpdir(), 'luma-gpu-raster-package-'));
try {
...
console.log('Verified @luma.gl/experimental/gpu-raster ESM, CJS, and declaration imports.');
