...
## Atmosphere: Height Fog Versus Clustered Participating Media[​](#atmosphere-height-fog-versus-clustered-participating-media "Direct link to Atmosphere: Height Fog Versus Clustered Participating Media")
...
## Reflections: Environment Maps Versus Screen-Space Rays[​](#reflections-environment-maps-versus-screen-space-rays "Direct link to Reflections: Environment Maps Versus Screen-Space Rays")
...
[Effects: Visualization City](https://luma.gl/examples/experimental/advanced-effects) and [Deferred Rendering: Illumination Lab](https://luma.gl/examples/experimental/deferred-rendering) use the **same** exported `createSSRCompositeShaderPass()`. They are examples of one implementation in different render stacks, not competing copies of the reflection algorithm.
...
## Lighting: Baseline Deferred Versus Clustered Deferred[​](#lighting-baseline-deferred-versus-clustered-deferred "Direct link to Lighting: Baseline Deferred Versus Clustered Deferred")

Both lighting resolves consume the same `GBuffer` material attachments and emit HDR color into the normal ordered `previous` chain.
...
## Shadows: Light-Space Maps Versus Screen-Space Contacts[​](#shadows-light-space-maps-versus-screen-space-contacts "Direct link to Shadows: Light-Space Maps Versus Screen-Space Contacts")
...
## Transparency: Weighted Blending Versus A-Buffer[​](#transparency-weighted-blending-versus-a-buffer "Direct link to Transparency: Weighted Blending Versus A-Buffer")
...
For opaque-depth handling, sorted-alpha fallbacks, scene-color capture, and backend selection, see [Transparency](https://luma.gl/docs/api-guide/shaders/transparency.md). Refractive and reflective surface shading is separate from fragment ordering; see [Glass Effects](https://luma.gl/docs/api-guide/shaders/glass-effects.md) for composable material modules and their current quality limits.
...
For backend-specific constraints and combined ordering, see [Antialiasing and Multisampling](https://luma.gl/docs/api-guide/gpu/gpu-antialiasing.md).

## Compose by Contract, Not by Visual Name[​](#compose-by-contract-not-by-visual-name "Direct link to Compose by Contract, Not by Visual Name")
...
The application still owns geometry, light updates, cluster encoding, shadow-map rendering, camera matrices, and presentation. `GBuffer` standardizes surface attachments, while `ShaderPassRenderer` manages ordered color routing, intermediate targets, and temporal history.

For the underlying execution model, see [Shader Passes](https://luma.gl/docs/api-guide/shaders/shader-passes.md). For complete live stacks, compare [Effects: Visualization City](https://luma.gl/examples/experimental/advanced-effects) and [Deferred Rendering: Illumination Lab](https://luma.gl/examples/experimental/deferred-rendering).
