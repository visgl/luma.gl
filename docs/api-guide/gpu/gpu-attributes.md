...
Note that while **attributes** is a structured and performant mechanism to provide columnar data to shaders that works on both WebGPU and WebGL, they are rather rigid and have a number of limitations. In WebGPU a more significantly more flexible approach is to use [storage buffers](https://luma.gl/docs/api-guide/gpu/gpu-storage-buffers.md).

For a WGSL-focused comparison of attribute columns, interleaved rows, storage arrays, and storage structs, see [Tabular Data in WGSL](https://luma.gl/docs/api-guide/gpu/tabular-data-in-wgsl.md).
...
See the [VertexFormat reference](https://luma.gl/docs/api-reference/core/vertex-formats.md) for information about which formats are available..
