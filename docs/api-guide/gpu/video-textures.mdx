...
- `VideoTexture` copies frames during draw binding resolution. WebGL can generate copied video mipmaps there, but WebGPU `VideoTexture` rejects `mipmaps: true` because WebGPU mipmap generation must be recorded before the render pass. Upload into an ordinary `Texture` before `beginRenderPass()` when WebGPU video mipmaps are required.
