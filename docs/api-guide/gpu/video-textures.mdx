...
When a `VideoTexture` resolves against `texture_external`, luma.gl tries to acquire a native WebGPU [`GPUExternalTexture`](https://gpuweb.github.io/gpuweb/#gpu-external-texture). If that import is unavailable, `VideoTexture` falls back at the engine layer by copying the current frame into a compatible one-mip `Texture`.
