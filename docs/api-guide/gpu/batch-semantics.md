...
| `GPUScan` | Stateful prefix, same logical row order | Packed `uint32`; vector input/output/segment flags require identical chunk lengths, including empty chunks; atomic output/flags may have extra capacity | No writes for zero rows; carry is rebuilt per encoding |
