...
It is worth noting, especially when working directly with buffers, that WebGPU requires that `bytesPerRow` must be a multiple of `256` for linear texture data
