...
## Where Antialiasing Fits[​](#where-antialiasing-fits "Direct link to Where Antialiasing Fits")

Geometry, alpha cutouts, textures

→

Raster coverage<br />`MSAA` or supersampling
...
Sampleable depth, normals, velocity, shadows
...
## Try Common Techniques[​](#try-common-techniques "Direct link to Try Common Techniques")
...
Before adding an AA algorithm, make sure the drawing buffer matches the intended display resolution. A canvas rendered at CSS-pixel resolution on a high-DPI display will look soft or jagged even when other settings are correct. Use [`CanvasContext`](https://luma.gl/docs/api-reference/core/canvas-context.md) sizing controls to choose device pixels or an explicit render scale.
...
## WebGL Canvas Antialiasing[​](#webgl-canvas-antialiasing "Direct link to WebGL Canvas Antialiasing")
...
The request does not antialias textures attached to application-created framebuffers. The WebGL specification describes `antialias` as a best-effort drawing-buffer request, not a requirement. See the [WebGL context attributes specification](https://registry.khronos.org/webgl/specs/latest/1.0/).

## Explicit Multisampling[​](#explicit-multisampling "Direct link to Explicit Multisampling")
...
For the raw WebGL sequence, see the [WebGL2Samples `fbo_multisample` example](https://webglsamples.org/WebGL2Samples/samples/fbo_multisample.html). For WebGPU concepts and sample-count constraints, see the [WebGPU multisampling guide](https://webgpufundamentals.org/webgpu/lessons/webgpu-multisampling.html) and the [WebGPU specification](https://gpuweb.github.io/gpuweb/).

Proposed API, not implemented

[RFC #2741](https://github.com/visgl/luma.gl/issues/2741) proposes a framebuffer-level request for color-only offscreen MSAA:
...
Under that proposal, the color texture remains single-sampled and sampleable. WebGL would render into private multisampled renderbuffers and resolve into the supplied texture when `RenderPass.end()` is called. The first scope is color-only and rejects depth/stencil attachments.

## Depth, Stencil, and Shadows[​](#depth-stencil-and-shadows "Direct link to Depth, Stencil, and Shadows")
...
The current luma.gl effects use sampleable depth textures for these later passes. For example, [`ShaderPassRenderer`](https://luma.gl/docs/api-reference/engine/passes/shader-pass-renderer.md) accepts application-owned depth bindings for scene-aware effects, while the [Depth of Field example](https://luma.gl/examples/showcase/dof) renders a texture-backed depth attachment before sampling it.
...
## Alpha Cutouts and Transparency[​](#alpha-cutouts-and-transparency "Direct link to Alpha Cutouts and Transparency")

Alpha testing with `discard` creates a hard coverage edge. With multisampling enabled, `sampleAlphaToCoverageEnabled` can convert fragment alpha into a sample coverage mask, which is often useful for foliage, fences, and similar cutouts. It is not a replacement for correct transparency ordering or blending, and it does little without multiple samples.
...
## Postprocess and Temporal Antialiasing[​](#postprocess-and-temporal-antialiasing "Direct link to Postprocess and Temporal Antialiasing")
...
TAA accumulates samples over time. It is better at subpixel motion and shimmer, but it requires a jittered projection, history buffers, velocity, and depth rejection to avoid ghosting. luma.gl exports [`fxaa`](https://luma.gl/docs/api-guide/shaders/shader-passes.md) for WebGL and WebGPU shader-pass chains and `createTAACompositeShaderPass()` for the WebGPU-oriented advanced-effects path. See the [Advanced Effects example](https://luma.gl/examples/experimental/advanced-effects) for TAA combined with depth, velocity, SSAO, SSR, and motion blur.
...
## Texture Aliasing[​](#texture-aliasing "Direct link to Texture Aliasing")
...
See [GPU Textures](https://luma.gl/docs/api-guide/gpu/gpu-textures.md#mipmaps) and [`Sampler`](https://luma.gl/docs/api-reference/core/resources/sampler.md) for the concrete luma.gl texture and sampler settings.

## Practical Recipes[​](#practical-recipes "Direct link to Practical Recipes")
