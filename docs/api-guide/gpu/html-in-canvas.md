...
luma.gl surfaces the complete DOM-to-texture path as the `'html-in-canvas'` [`DeviceFeature`](https://luma.gl/docs/api-reference/core/device-features.md). Prefer checking the current `Device` instead of checking browser globals directly:
...
For normal image, canvas, or video uploads that do not need DOM layout rasterization, use [`Texture.copyExternalImage()`](https://luma.gl/docs/api-reference/core/resources/texture.md#copyexternalimage) instead.
