...
<GpuGuideDocsTabs group="rendering" active="antialiasing" />
...
The comparison below keeps the before side as the single-sample baseline and applies the selected
technique on the after side. Drag the divider to inspect the transition. It intentionally uses
techniques available through the portable luma.gl
...
`createTAACompositeShaderPass()` for the WebGPU-oriented advanced-effects path. See the
