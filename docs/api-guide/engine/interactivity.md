---
title: Interaction, picking, and highlighting
description: Route browser input into camera controls or GPU picking, then use the selected identity to drive ordinary render-state highlighting.
---
...
<EngineDocsTabs group="interaction" active="interaction-guide" />
...
Keeping those steps separate prevents controls, picking, and highlighting from becoming one
backend-specific event handler.
...
- Highlighting remains a normal render concern. Store the hovered or selected identity, pass it
  to the shader, and render that object differently.

Use browser events or an application framework to decide which canvas receives input. For more
general gesture recognition, [mjolnir.js](https://uber-web.github.io/mjolnir.js/) supports
portable touch and pointer gestures.

## Camera interaction

`OrbitControls` attaches orbit and wheel behavior to one HTML canvas. Update it before deriving
the view matrix, then invalidate the view only when its state changes.
...
The controller is independent of the graphics backend. The matrices derived from it can feed a
WebGPU or WebGL 2 model.
...
[`PickingManager`](/docs/api-reference/engine/picking-manager) supports two representations:

- `mode: 'color'` is the portable default.
- `mode: 'auto'` prefers integer index picking when the device supports a renderable `rg32sint`
  target and otherwise uses color picking.

The picked index should remain stable even if visible instances are compacted. If a renderer
uses batch-local indices, carry the batch identity as well or translate back to a canonical
application identity before publishing the result.

## Immediate highlighting

Picking and highlighting need not use the same render pass. The picking pass discovers an
identity; the visible fragment shader compares each object’s identity with the current hover or
selection value.
...
This produces immediate feedback without rebuilding geometry or changing the picking target.
For batched data, compare the canonical identity whenever possible rather than the transient
...
WebGL 2 readback can synchronously stall the pipeline. WebGPU permits better staging, but hover
still benefits from coalescing pointer movement and allowing only one readback in flight.
luma.gl does not currently provide a general CPU ray-intersection system.

## Decisions and tradeoffs

- Pick on meaningful pointer changes, not every animation callback.
- Prefer immediate shader highlighting for hover; reserve geometry or material changes for
  persistent selection state.
- Keep page scrolling outside an embedded canvas until the user explicitly activates its
  controls.
- Use a rectangle or lasso selection tool to define a query region, then let the renderer or
  data layer decide how objects inside that region are selected.
...
- [`OrbitControls`](/docs/api-reference/engine/orbit-controls)
- [`PickingManager`](/docs/api-reference/engine/picking-manager)
- [Redraw on demand](/docs/api-guide/engine/redraw)
- [Shadertools picking module](/docs/api-reference/shadertools/shader-modules/picking)
