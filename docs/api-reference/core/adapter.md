...
An `Adapter` is a factory that creates [`Device`](https://luma.gl/docs/api-reference/core/device.md) instances for a specific backend (e.g. WebGPU or WebGL). Each GPU backend exports a singleton adapter instance that is used to create devices for that GPU backend.

Adapters can be used directly to create and attach devices, but they are usually imported and used via the [`luma`](https://luma.gl/docs/api-reference/core/luma.md) API through methods like \[`luma.createDevice`].
