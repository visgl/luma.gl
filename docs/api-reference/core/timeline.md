...
Attach an animation object (can be any object with a `setTime` method) to the timeline, optionally attached to a specific channel referenced by `channelHandle`.
