...
`RenderPassProps` extends [`ResourceProps`](https://luma.gl/docs/api-reference/core/resources/resource.md#resourceprops) and accepts the following fields.
...
For indexed draws, `indexCount` takes precedence. When it is omitted, the active backend may use `vertexCount` as the indexed draw count for compatibility.
...
Replays previously recorded `RenderBundle` objects from a [`RenderBundleEncoder`](https://luma.gl/docs/api-reference/core/resources/render-bundle-encoder.md) in this render pass.
