...
While the idea behind textures is simple in principle (a grid of pixels stored on GPU memory), GPU Textures are surprisingly complex objects. It can be helpful to read the [API Guide section on textures](https://luma.gl/docs/api-guide/gpu/gpu-textures.md) to make sure you have a full picture.

For upload and readback strategy, also see [GPU Commands](https://luma.gl/docs/api-guide/gpu/gpu-commands.md).
...
### `TextureProps`[​](#textureprops "Direct link to textureprops")

`TextureProps` extends [`ResourceProps`](https://luma.gl/docs/api-reference/core/resources/resource.md#resourceprops) and accepts the following fields.
...
| `data?`      | `ExternalImage \| TypedArray \| null`                                         | `null`                                                 | Initial pixel data. This path is deprecated in favor of [`DynamicTexture`](https://luma.gl/docs/api-reference/engine/dynamic-texture.md) and only supports direct image uploads.                   |
...
| `format?`    | [`TextureFormat`](https://luma.gl/docs/api-reference/core/texture-formats.md) | `'rgba8unorm'`                                         | The GPU format that determines channel ordering, bit depth and compression.                                                                                                                        |
...
| `samples?`   | `number`                                                                      | backend default                                        | WebGPU sample count for a true multisampled render attachment. WebGL textures remain single-sampled; see [Antialiasing and Multisampling](https://luma.gl/docs/api-guide/gpu/gpu-antialiasing.md). |
...
`Texture.samples` does not provide a portable “render, resolve, then sample” workflow today. WebGL ignores it when allocating textures, and WebGPU multisampled textures need a compatible pipeline and explicit resolve path. See [Antialiasing and Multisampling](https://luma.gl/docs/api-guide/gpu/gpu-antialiasing.md) and [RFC #2741](https://github.com/visgl/luma.gl/issues/2741).
...
## TextureDimension[​](#texturedimension "Direct link to TextureDimension")
...
[GitHub](https://github.com/visgl/luma.gl/tree/master/examples/api/texture-3d)Info
...
## TextureData[​](#texturedata "Direct link to TextureData")
...
## CubeFace[​](#cubeface "Direct link to CubeFace")
...
### `Texture.isExternalImage()`[​](#textureisexternalimage "Direct link to textureisexternalimage")

Check whether a value is a valid external image object.

### `Texture.getExternalImageSize()`[​](#texturegetexternalimagesize "Direct link to texturegetexternalimagesize")

Deduces the size (width and height) of an "external image" object.

### `Texture.isTextureLevelData()`[​](#textureistextureleveldata "Direct link to textureistextureleveldata")

Check whether a value is valid as data for setting a texture level.

### `Texture.getTextureDataSize()`[​](#texturegettexturedatasize "Direct link to texturegettexturedatasize")

Calculates the size of texture data for a composite texture data object

### `Texture.getMipLevelCount()`[​](#texturegetmiplevelcount "Direct link to texturegetmiplevelcount")
...
### `Texture.getCubeFaceDepth()`[​](#texturegetcubefacedepth "Direct link to texturegetcubefacedepth")
...
### `constructor(props: TextureProps)`[​](#constructorprops-textureprops "Direct link to constructorprops-textureprops")
...
### `generateMipmap() : Texture2D`[​](#generatemipmap--texture2d "Direct link to generatemipmap--texture2d")
...
Use `readBuffer()` with an explicit destination buffer, or engine [`DynamicTexture`](https://luma.gl/docs/api-reference/engine/dynamic-texture.md) when you want convenience readback that allocates temporary buffers for you.
