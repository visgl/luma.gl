...
See also [Using GPU Buffers](https://luma.gl/docs/api-guide/gpu/gpu-buffers.md) and [GPU Commands](https://luma.gl/docs/api-guide/gpu/gpu-commands.md) for when buffer operations should use direct resource methods versus explicit command encoding.
...
### `BufferProps`[​](#bufferprops "Direct link to bufferprops")
...
### `BufferMapCallback`[​](#buffermapcallback "Direct link to buffermapcallback")
...
### `constructor(props: BufferProps)`[​](#constructorprops-bufferprops "Direct link to constructorprops-bufferprops")
...
### `write(): void`[​](#write-void "Direct link to write-void")
...
### `readAsync(): Promise<ArrayBuffer>`[​](#readasync-promisearraybuffer "Direct link to readasync-promisearraybuffer")
...
### `mapAndReadAsync(): Promise<void>`[​](#mapandreadasync-promisevoid "Direct link to mapandreadasync-promisevoid")
...
### `readSyncWebGL(): ArrayBuffer`[​](#readsyncwebgl-arraybuffer "Direct link to readsyncwebgl-arraybuffer")

* `byteOffset` - the first byte to read from the GPU buffer.
* `byteLength` - the number of bytes to read from the GPU buffer

Synchronous reads, while convenient, have significant performance penalty as they force a GPU sync. Synchronous reads are not available on WebGPU.
