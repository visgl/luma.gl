...
Compiling shaders is usually cheaper than creating complete pipelines, but applications that repeatedly request the same shader source should still prefer [`ShaderFactory`](https://luma.gl/docs/api-reference/core/shader-factory.md) over calling `device.createShader()` directly.
