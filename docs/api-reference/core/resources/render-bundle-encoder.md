...
[GitHub](https://github.com/visgl/luma.gl/tree/master/examples/api/render-bundles)Info
...
### Recording and replaying[​](#recording-and-replaying "Direct link to Recording and replaying")
...
`drawIndirect(buffer, byteOffset)` and `drawIndexedIndirect(buffer, byteOffset)` can be recorded in a bundle. The command and binding sequence stays immutable while compute or uploads change the contents of the indirect buffer before each replay. This is the recommended pattern for stable draw groups whose GPU-generated instance counts change every frame.

### Updating resources and splitting bundles[​](#updating-resources-and-splitting-bundles "Direct link to Updating resources and splitting bundles")
...
### `RenderBundleEncoderProps`[​](#renderbundleencoderprops "Direct link to renderbundleencoderprops")

`RenderBundleEncoderProps` extends [`ResourceProps`](https://luma.gl/docs/api-reference/core/resources/resource.md#resourceprops). It includes only resource metadata and the attachment compatibility fields required to record a bundle; render-pass setup properties are not accepted.
...
### `renderBundleEncoder.finish(): RenderBundle`[​](#renderbundleencoderfinish-renderbundle "Direct link to renderbundleencoderfinish-renderbundle")

Completes recording and returns an immutable reusable bundle. The bundle inherits the encoder's `id` and `userData`.

## See also[​](#see-also "Direct link to See also")

### `device.createRenderBundleEncoder(props?: RenderBundleEncoderProps): RenderBundleEncoder`[​](#devicecreaterenderbundleencoderprops-renderbundleencoderprops-renderbundleencoder "Direct link to devicecreaterenderbundleencoderprops-renderbundleencoderprops-renderbundleencoder")

Creates an encoder for reusable WebGPU render commands.

### `renderPass.executeBundles(bundles: Iterable<RenderBundle>): void`[​](#renderpassexecutebundlesbundles-iterablerenderbundle-void "Direct link to renderpassexecutebundlesbundles-iterablerenderbundle-void")
