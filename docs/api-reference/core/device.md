...
| `resourceInstrumentation?: ResourceInstrumentation \| null` | `null`                                        | Observes resource lifecycle and allocation events. Use `resourceStatsInstrumentation` from `@luma.gl/core/diagnostics` for the legacy resource counts and memory statistics. |
