...
| `requiredFeatures?: WebGPUDeviceFeature[]`            | `[]`                                         | WebGPU-only optional features that must be enabled. Creation fails when an adapter lacks one. WebGL rejects a non-empty list. |
...
`device.features` exposes native WebGPU optional features and luma.gl's WebGL extension-backed
features. Current WebGPU names include `texture-formats-tier1`, `texture-formats-tier2`,
`primitive-index`, `texture-component-swizzle`, and `subgroup-size-control`. WebGL-only feature
names end in `-webgl`, including `clip-control-webgl`, `polygon-offset-clamp-webgl`,
`texture-mirror-clamp-to-edge-webgl`, `draw-buffers-indexed-webgl`, `multi-draw-webgl`,
`stencil-texturing-webgl`, `shader-sample-variables-webgl`, and
`shader-multisample-interpolation-webgl`.

`dual-source-blending` is portable: it is reported for native WebGPU support and for WebGL's
`WEBGL_blend_func_extended` extension. `depth-clip-control` continues to represent WebGPU depth
clip control and WebGL `EXT_depth_clamp`; WebGL `EXT_clip_control` is separately reported as
`clip-control-webgl`.

`device.getTextureFormatCapabilities(format)` reports `create`, `render`, `filter`, `blend`, and
the compatibility `store` flag, plus explicit `storageRead`, `storageWrite`, and
`storageReadWrite` capabilities. WebGL reports storage texture access as unsupported; WebGPU
results are gated by the enabled optional texture-format features.
