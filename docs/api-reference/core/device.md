...
| `featureLevel?: 'core' \| 'max'`                     | `'core'`                                      | WebGPU feature/limit profile to request. `'core'` is the portable default; `'max'` requests every supported adapter feature and limit. WebGL and null devices ignore this prop. |
...
| `featureLevel?`          | `'core' \| 'max'` | Effective WebGPU feature level. |
