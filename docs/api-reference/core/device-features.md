...
WebGPU device features must be requested during device creation, while WGSL language features are discovered dynamically and are not device descriptor entries. See [Optional WebGPU and WGSL features](https://luma.gl/docs/api-reference/webgpu/optional-features.md) for the canonical capability-selection pattern.
