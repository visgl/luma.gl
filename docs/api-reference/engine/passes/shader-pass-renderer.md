...
Internally it uses [`ClipSpace`](https://luma.gl/docs/api-reference/engine/clip-space.md), [`BackgroundTextureModel`](https://luma.gl/docs/api-reference/engine/background-texture-model.md), and [`SwapFramebuffers`](https://luma.gl/docs/api-reference/engine/compute/swap.md) to manage the pass chain.

For the descriptor types, see [`ShaderPass`](https://luma.gl/docs/api-reference/shadertools/shader-pass.md). For the authoring model, see [Shader Passes](https://luma.gl/docs/api-guide/shaders/shader-passes.md).
...
## Routing Model[​](#routing-model "Direct link to Routing Model")
...
## Runtime Inputs[​](#runtime-inputs "Direct link to Runtime Inputs")
...
This makes it practical to keep one renderer alive while swapping in frame-specific resources such as a freshly rendered depth texture or a `VideoTexture`.

When you call `shaderInputs.setProps({[passName]: {...}})`, any texture bindings in that object are stored as defaults for that specific shader pass. `ShaderPassRenderer` resolves those defaults per pass, then layers `options.bindings` on top for per-frame overrides.
...
### `ShaderPassRendererProps`[​](#shaderpassrendererprops "Direct link to shaderpassrendererprops")
...
`colorFormat` controls the shared `previous` ping-pong textures and defaults to `device.preferredColorFormat`. Use `rgba16float` when HDR-producing passes must retain values above 1.0 until a later tone-mapping pass.

`flipY` controls texture sampling in the fullscreen copies and every shader subpass. It defaults to `true` on WebGPU so framebuffer attachments retain a consistent top-left screen orientation through intermediate render targets. Set it explicitly when integrating textures with a different origin convention.
...
### `constructor(device: Device, props: ShaderPassRendererProps)`[​](#constructordevice-device-props-shaderpassrendererprops "Direct link to constructordevice-device-props-shaderpassrendererprops")
...
Set `sampler: {minFilter: 'linear', magFilter: 'linear'}` on downsampled color targets when they will be upsampled later. Linear sampling prevents block-shaped bloom halos while leaving existing nearest-sampled passes unchanged.

Resizing invalidates history targets whose allocation size changes.

### `resetHistory(): void`[​](#resethistory-void "Direct link to resethistory-void")

Invalidates every history target. The next render initializes each target from its declared `initialize` value without reallocating it.

### `renderToScreen(options): boolean`[​](#rendertoscreenoptions-boolean "Direct link to rendertoscreenoptions-boolean")
...
### `renderToTexture(options): Texture | null`[​](#rendertotextureoptions-texture--null "Direct link to rendertotextureoptions-texture--null")
...
### `encodeToScreen(commandEncoder, options): boolean`[​](#encodetoscreencommandencoder-options-boolean "Direct link to encodetoscreencommandencoder-options-boolean")
...
This method only records commands. The caller remains responsible for finishing and submitting the encoder. The encoder must belong to the renderer's device, and no render or compute pass may be active when this method is called. Returns `false` when the source texture is not ready yet.

### `encodeToTexture(commandEncoder, options): Texture | null`[​](#encodetotexturecommandencoder-options-texture--null "Direct link to encodetotexturecommandencoder-options-texture--null")

Records the pass chain into a caller-owned command encoder and returns the renderer-owned output texture.
...
This method does not finish or submit the encoder. The encoder must belong to the renderer's device, and no render or compute pass may be active. The returned texture is valid after the recorded command buffer has been submitted.
