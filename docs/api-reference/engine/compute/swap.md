...
`Swap` is a small double-buffering helper for pairs of GPU resources. It is used by higher-level engine utilities such as [`ShaderPassRenderer`](https://luma.gl/docs/api-reference/engine/passes/shader-pass-renderer.md) and is also exported directly for application code.
