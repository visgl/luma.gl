...
`Model` is the main engine-level rendering class in luma.gl. It assembles shaders, manages geometry and bindings, reuses immutable cached pipelines, and applies its dynamic draw state to a [`RenderPass`](https://luma.gl/docs/api-reference/core/resources/render-pass.md).
...
`ModelProps` also includes the standard [`RenderPipelineProps`](https://luma.gl/docs/api-reference/core/resources/render-pipeline.md), except that `bindings`, `vs`, and `fs` are specialized for engine usage.
...
### `setIndexCount(indexCount: number | undefined): void`[​](#setindexcountindexcount-number--undefined-void "Direct link to setindexcountindexcount-number--undefined-void")
...
* `Model` integrates with [`ShaderInputs`](https://luma.gl/docs/api-reference/engine/shader-inputs.md), [`PipelineFactory`](https://luma.gl/docs/api-reference/core/pipeline-factory.md), and [`ShaderFactory`](https://luma.gl/docs/api-reference/core/shader-factory.md) by default.
