...
Use [`Texture`](https://luma.gl/docs/api-reference/core/resources/texture.md) for one uploaded image or when the shader needs ordinary texture features such as mipmaps, repeat addressing, render-target usage, or storage usage. Use `VideoTexture` when a `Model` or `Material` should follow a live video source across draws.

For the copied-versus-external texture tradeoff, see [Working With Video Textures](https://luma.gl/docs/api-guide/gpu/video-textures.md).
...
* [`DynamicTexture`](https://luma.gl/docs/api-reference/engine/dynamic-texture.md) wraps asynchronous or replaceable ordinary texture data.
* [`ExternalTexture`](https://luma.gl/docs/api-reference/core/resources/external-texture.md) is the low-level concrete one-shot WebGPU external binding.
* Experimental [`WebXRCameraTexture`](https://luma.gl/docs/api-reference/experimental/webxr/webxr-camera-texture.md) handles WebXR Raw Camera Access without making WebXR part of `VideoTexture`.
