...
Outlines use one render pass, allocate no history, and require scene attachments to remain aligned with the current color image. Apply them before image-space warps, or transform the auxiliary attachments by the same amount. For a purely color-derived illustrated treatment, compare [Ink](https://luma.gl/docs/api-reference/shadertools/shader-passes/ink.md) or [Edge Work](https://luma.gl/docs/api-reference/shadertools/shader-passes/edge-work.md).

## Related Effects[​](#related-effects "Direct link to Related Effects")

* [Ink](https://luma.gl/docs/api-reference/shadertools/shader-passes/ink.md) derives graphic contours directly from image color.
* [Edge Work](https://luma.gl/docs/api-reference/shadertools/shader-passes/edge-work.md) extracts image-frequency differences without scene attachments.
* [SSAO](https://luma.gl/docs/api-reference/shadertools/shader-passes/ssao.md) can reuse the same depth and normal buffers.
