...
`ShaderPlugin` groups reusable shader assembly contributions that can be attached to [`Model`](https://luma.gl/docs/api-reference/engine/model.md) and [`Computation`](https://luma.gl/docs/api-reference/engine/compute/computation.md). Prefer plugins as the application-facing optional composition layer when a behavior needs modules, defines, shader-facing vertex inputs, named injections, portable cross-stage varyings, or backend-specific shader source.

For the authoring model, see [Writing Customizable Shaders](https://luma.gl/docs/api-guide/shaders/writing-customizable-shaders.md).
...
## Injection Targets[​](#injection-targets "Direct link to Injection Targets")
...
## Vertex Inputs[​](#vertex-inputs "Direct link to Vertex Inputs")
...
## `filterShaderPlugin`[​](#filtershaderplugin "Direct link to filtershaderplugin")
...
## `clipShaderPlugin`[​](#clipshaderplugin "Direct link to clipshaderplugin")
