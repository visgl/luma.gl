...
The [Points example](https://luma.gl/examples/arrow/arrow-points) demonstrates `FixedSizeList<Float32, 2 | 3 | 4>` rows, including M-coordinate or timestamp animation and hover identity labels. The [Lines example](https://luma.gl/examples/arrow/arrow-lines) demonstrates variable-length path rows.
...
| `Struct`                | 🟡              | ❌       | Supported indirectly when the application selects supported child columns by field path.                                                                                          | Child fields can become separate shader columns after selection or flattening.                                               | Nested style or object columns.                                                       |
