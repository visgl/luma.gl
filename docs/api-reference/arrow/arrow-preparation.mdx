...
`makeArrowGPUData()`, `makeGPUVectorFromArrow()`, `makeArrowGPURecordBatch()`, and
...
Streaming append helpers preserve batch and buffer boundaries. Each appended
Arrow data chunk is uploaded into a new `GPUData` with its own buffer, and
`GPUTable` aggregate vectors expose the logical column through `GPUVector.data[]`
