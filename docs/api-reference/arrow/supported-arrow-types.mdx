...
identity labels. The [Paths example](/examples/arrow/arrow-path-model) demonstrates
DenseUnion LineString rows by normalizing them one-for-one into prepared path rows before
the path models consume them.
