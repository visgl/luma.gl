...
| Arrow data type | GPU<br/>upload | nullable | Upload notes | Shader-facing<br/>use | Higher-level<br/>model support |
| --- | :---: | :---: | --- | --- | --- |
| `FixedSizeList<numeric,`<br/>`1 \| 2 \| 3 \| 4>` | ✅ | ✅ | Fixed-width GPU rows preserve parent and child validity for Arrow readback; optional `validityColumns` publishes explicit GPU row masks.<br/>`convertArrowColors()` normalizes `Uint8`, `Float16`, and `Float32` RGB/RGBA rows into RGBA8 while preserving chunks and row validity. `FixedSizeList<Float16, 4>` remains the compact scene-linear/HDR form when no normalized conversion is requested. | Vector attributes or storage rows; consuming models must explicitly honor validity, and 3-component 8-bit and 16-bit integer formats are WebGL-only unless padded or normalized through the color adapter. | Positions, point rows, normalized colors, scene-linear/HDR colors, pixel offsets, clip rectangles, mesh attributes, and path style rows. |
