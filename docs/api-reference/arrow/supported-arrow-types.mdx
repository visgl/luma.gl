...
rows, including M-coordinate or timestamp animation and hover identity labels. The
[Lines example](/examples/arrow/arrow-lines) demonstrates variable-length path rows.
...
`List<FixedSizeList<Float32, 4>>` and `List<Timestamp>`
