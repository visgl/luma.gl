...
It is separate from the ordinary `@luma.gl/scene` entry point, so applications that only render
...
[ANARI animation and glTF integration](/docs/api-reference/scene/anari-animation) for retained
