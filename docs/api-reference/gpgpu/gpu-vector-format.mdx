...
flattened element. Producers provide chunk-local row boundaries through
[`GPUData.valueOffsets`](/docs/api-reference/gpgpu/gpu-data#variable-length-and-segmented-data).
Closed-path flags, text glyph maps, and similar domain-specific topology remain
producer-owned metadata.

Variable-length non-vertex values use `value-list<format>`. For example,
`value-list<uint8>` stores flattened UTF-8 bytes with the same chunk-local
`valueOffsets` contract.
