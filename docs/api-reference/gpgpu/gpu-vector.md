...
[GPUConstant](https://luma.gl/docs/api-reference/gpgpu/gpu-constant.md)[GPUVector](https://luma.gl/docs/api-reference/gpgpu/gpu-vector.md)[GPUData](https://luma.gl/docs/api-reference/gpgpu/gpu-data.md)[GPUDataView](https://luma.gl/docs/api-reference/gpgpu/gpu-data-view.md)[GPUVectorFormat](https://luma.gl/docs/api-reference/gpgpu/gpu-vector-format.md)

From v9.4Experimental API

`GPUVector` represents one logical typed table column over one or more [`GPUData`](https://luma.gl/docs/api-reference/gpgpu/gpu-data.md) chunks. It does not own a buffer directly; every concrete buffer lives on a `GPUData` object in `GPUVector.data[]`.
...
### `addData(data): this`[​](#adddatadata-this "Direct link to adddatadata-this")

Adds an existing `GPUData` chunk to this logical vector. The chunk must have the same `format`, `byteStride`, and `rowByteLength`.

### `appendDataChunk(data, appendedByteLength): this`[​](#appenddatachunkdata-appendedbytelength-this "Direct link to appenddatachunkdata-appendedbytelength-this")

Adds one adapter-created `GPUData` chunk to an appendable logical vector.

### `resetLastBatch(): this`[​](#resetlastbatch-this "Direct link to resetlastbatch-this")

Clears appendable logical rows and destroys appended `GPUData` buffers.

### `transferBufferOwnership(target): void`[​](#transferbufferownershiptarget-void "Direct link to transferbufferownershiptarget-void")
