...
`GPUData` describes one GPU buffer plus typed row metadata. It is the lowest-level storage object in `@luma.gl/gpgpu/gpu-data`: each `GPUData` owns or borrows its own `Buffer` or `DynamicBuffer`, and higher-level table objects refer to buffers through `GPUData`.

[`GPUDataView`](https://luma.gl/docs/api-reference/gpgpu/gpu-data-view.md) is the non-owning physical counterpart: one fixed-width format, value count, byte offset, and byte stride over any buffer-like resource.
