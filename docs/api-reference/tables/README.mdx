...
types such as `GPUSchema`, `GPUField`, `GPUTypeMap`, `GPUVectorFormat`,
`GPUDataStructFormat`, and `VertexList`. Physical struct formats let one
`GPUData` expose named zero-copy child views over interleaved rows. Models can publish `GPUInputSchema` declarations for the
