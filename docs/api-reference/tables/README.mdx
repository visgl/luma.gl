---
title: Experimental GPU data and tables
description: Model typed GPU data, batch-preserving tables, schemas, bindings, ownership, and lifecycle through the 9.4 experimental subpaths.
---
...
Typed GPU data primitives and batch-preserving table helpers for rendering, transforms, and compute.
...
- [GPUDataView](/docs/api-reference/tables/gpu-data-view)
...
The experimental `@luma.gl/gpgpu/gpu-data` subpath owns reusable primitive GPU objects such as
`GPUData`, `GPUDataView`, `GPUVector`, and `GPUConstant`, plus memory-format types such as
`GPUVectorFormat`, `GPUDataStructFormat`, and `VertexList`. The private experimental
`@luma.gl/experimental/gpu-tables` subpath owns `GPURecordBatch`, `GPUTable`, `GPUSchema`,
`GPUField`, `GPUTypeMap`, table bindings, table computations, and generic table planners.
...
These subpaths are experimental in 9.4 and are installed with their containing packages:
