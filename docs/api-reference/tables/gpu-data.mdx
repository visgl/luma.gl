...
```ts
import {GPUData, makeGPUDataStructFormat} from '@luma.gl/tables';
...
const data = new GPUData({buffer, format, length});
...
`makeGPUDataStructFormat(fields, options?)` describes several named fixed-width
fields stored in each `GPUData` row. The declaration is physical: it describes
the bytes in the buffer and does not declare a parallel shader struct type.
...
| `format` | `GPUDataFormat` | `undefined` | Canonical memory-layout descriptor: a `GPUVectorFormat` string or a physical `GPUDataStructFormat`. |
