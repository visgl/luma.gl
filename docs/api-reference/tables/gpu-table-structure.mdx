...
| [`GPUVector`](/docs/api-reference/tables/gpu-vector) | One logical typed column over one or more GPU data chunks. | `name`, `type`, `length`, `stride`, `byteStride`, `data[]`, optional direct `buffer` |
| [`GPUData`](/docs/api-reference/tables/gpu-data) | One typed GPU memory range. | `buffer`, `type`, `length`, `byteOffset`, `byteStride`, `rowByteLength`, `ownsBuffer` |

The table is intentionally batch-preserving. When an Arrow table has multiple
record batches, `makeArrowGPUTable()` creates one `GPURecordBatch` per source
...
| `Data` | Like `GPUData`, it is one typed chunk/range. | `GPUData` points at a `DynamicBuffer`, not a CPU typed-array view. It can only be read back through adapter-specific paths. |
| `Schema` and `Field` | Both models retain logical type metadata. | vis.gl field metadata describes GPU conventions such as matrix shape/layout and temporal origins. |
...
a `GPUData` chunk that describes a GPU buffer range.
...
- `GPUData` is the storage-owning layer. It destroys its `DynamicBuffer` only
  when `ownsBuffer` is true.
- `GPUVector` owns or borrows ordered `GPUData[]` chunks. It does not own raw
  buffers directly unless it was constructed around an owned concrete buffer or
  appendable `DynamicBuffer`.
...
## Direct Buffers and Aggregate Vectors

`GPUVector.buffer` is only available when the vector has one concrete backing
surface. Multi-batch aggregate vectors may contain many `GPUData` chunks, so
batch-aware code should use `data[]` or dispatch/draw one `GPURecordBatch` at a
time.
