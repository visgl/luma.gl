...
* A set of functions (e.g. [`isWebGL2`](/#/documentation/api-reference/is-webgl-2), [`getFeatures`](/#/documentation/api-reference/get-features) and [`hasFeatures`](/#/documentation/api-reference/has-features), described in this document) that enable you to check if the application is currently running on an environment that supports a certain feature (regardless of whether it is supported through e.g. WebGL2 or a WebGL1 extension).
