...
<GPUTablesDocsTabs active="schema" />
...
[`GPUTable`](/docs/api-reference/experimental/gpu-tables/gpu-table) and
...
with
[`isGPUVectorFormatCompatibleWithShaderType()`](/docs/api-reference/gpgpu/gpu-vector-format).
...
- [`GPUData`](/docs/api-reference/gpgpu/gpu-data) owns or borrows one buffer.
- [`GPUVector`](/docs/api-reference/gpgpu/gpu-vector) owns or borrows ordered
  `GPUData` chunks.
- [`GPURecordBatch`](/docs/api-reference/experimental/gpu-tables/gpu-record-batch) owns one
  batch-local `GPUData` chunk per selected column.
- [`GPUTable`](/docs/api-reference/experimental/gpu-tables/gpu-table) owns preserved batches.

Keeping schema as plain data lets gpgpu, generated-geometry, and application-specific adapters
create schemas without depending on a shared class
hierarchy.

## Source Interop

Source adapters can create `GPUSchema` objects while retaining their source type information as
adapter or readback metadata. Tables and record batches expose `GPUSchema` as the canonical GPU
schema.
