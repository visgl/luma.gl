...
<GPUTablesDocsTabs active="lifecycle" />
...
`@luma.gl/experimental/gpu-tables`: ownership, batching, packing, table-backed rendering,
transform, compute, and buffer planning.

## Overview

`GPUData`, `GPUVector`, `GPUConstant`, `GPURecordBatch`, `GPUTable`, and `GPUSchema`
are GPU-side representations for typed table columns and batches. The generic GPU objects do not
retain references to adapter-owned sources after extracting the buffer data and metadata they
need.
...
- Table-level `gpuVectors` aggregate those batch-local chunks through
  `GPUVector.data[]`.
- Table-level `gpuConstants` retain one immutable CPU payload and never appear in
  batch `gpuData` or the physical `bufferLayout`.
...
This means a multi-batch source stays multi-batch after GPU upload. If the application prefers
fewer draw units, it packs explicitly:
...
See [GPUSchema](/docs/api-reference/experimental/gpu-tables/gpu-schema) and
[GPUVectorFormat](/docs/api-reference/gpgpu/gpu-vector-format) for the type
reference. A model's accepted prepared vector inputs are declared separately
with
...
| --- | --- | --- |
| `GPUData` | One GPU buffer plus row/stride metadata and buffer ownership. | [GPUData](/docs/api-reference/gpgpu/gpu-data) |
| `GPUVector` | One logical column over ordered `GPUData[]` chunks. | [GPUVector](/docs/api-reference/gpgpu/gpu-vector) |
| `GPUConstant` | One table-wide fixed-width logical column value. | [GPUConstant](/docs/api-reference/gpgpu/gpu-constant) |
| `GPURecordBatch` | One immutable preserved batch with batch-local data chunks and layout metadata. | [GPURecordBatch](/docs/api-reference/experimental/gpu-tables/gpu-record-batch) |
| `GPUTable` | Table-level owner that preserves batches, exposes aggregate vectors, and controls packing/detach/destruction. | [GPUTable](/docs/api-reference/experimental/gpu-tables/gpu-table) |
| `GPUSchema` | Selected-column metadata with `GPUVectorFormat` fields. | [GPUSchema](/docs/api-reference/experimental/gpu-tables/gpu-schema) |
| `GPUInputSchema` | Runtime declaration of required and optional `GPUVector` model inputs. | [GPUInputSchema](/docs/api-reference/experimental/gpu-tables/gpu-input-schema) |

The important invariant is that varying buffers are reached through `GPUData`, not
...
When streaming source data, create one immutable GPU batch per source batch and add it to the
table:
...
Each prepared batch preserves previous batches. To reduce draw or dispatch units, call
...
When `table.gpuVectors.indices` exists, `GPUTableModel` treats it as the
reserved indexed-draw column rather than an attribute. The vector must use
`vertex-list<uint32>`, contain exactly one directly bindable `GPUData` chunk per
batch, and use a buffer created with `Buffer.INDEX`. `GPUTableModel` binds that
buffer with `Model.setIndexBuffer()` and sets the indexed draw count from
`indices.valueLength`.

For preserved indexed batches, call `drawBatches(renderPass)`. Each batch keeps
its own local index payload, so `drawBatches()` rebinds the batch-local index
buffer and count before drawing, then restores the table-level state afterward.
All batches in an indexed table must include `indices`; `packBatches()` rejects
indexed tables until packing can rebase those local indices correctly.
...
- [GPUVector](/docs/api-reference/gpgpu/gpu-vector)
- [GPUData](/docs/api-reference/gpgpu/gpu-data)
- [GPUSchema](/docs/api-reference/experimental/gpu-tables/gpu-schema)
- [GPUInputSchema](/docs/api-reference/experimental/gpu-tables/gpu-input-schema)
- [GPUVectorFormat](/docs/api-reference/gpgpu/gpu-vector-format)
- [GPUTableBufferPlanner](/docs/api-reference/experimental/gpu-tables/gpu-table-buffer-planner)
