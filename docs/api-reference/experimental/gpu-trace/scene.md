...
Source views such as `source.startTimes`, `source.processIds`, `source.parents`, and `source.outgoingNeighbors` can feed independent filtering, layout, and graph traversal passes in the same graph. [`GPUTraceInteraction`](https://luma.gl/docs/api-reference/experimental/gpu-trace/interaction.md) composes those views into a reusable hierarchy, dependency-focus, visibility, and indirect-draw workflow.

When a timeline coordinate needs to recover the original source row, compose the interaction outputs with [`getGPUTracePickingShader`](https://luma.gl/docs/api-reference/experimental/gpu-trace/picking.md). Its visibility-aware shader uses these same canonical records and scanned thread offsets without adding picking state to the scene.
