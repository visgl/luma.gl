...
The [Advanced Effects example](https://luma.gl/examples/experimental/advanced-effects) uses `GBuffer` with three extra channels for unshadowed color, directional direct light, and shadow debugging.

The [Deferred Illumination Lab](https://luma.gl/examples/experimental/deferred-rendering) uses two extra channels, `baseColorMetallic` and `emissiveOcclusion`, then resolves them with 64-capacity storage-buffer point lighting:
...
* [Shader Passes](https://luma.gl/docs/api-guide/shaders/shader-passes.md) explains the composable render-stack model and effect ordering.
* [Deferred Lighting](https://luma.gl/docs/api-reference/experimental/deferred-lighting.md) defines the material attachment convention and fullscreen lighting resolve.
* [`ShaderPassRenderer`](https://luma.gl/docs/api-reference/engine/passes/shader-pass-renderer.md) documents routing, named targets, runtime bindings, and temporal history.
* [`WBOITRenderer`](https://luma.gl/docs/api-reference/experimental/wboit-renderer.md) and [`ABufferRenderer`](https://luma.gl/docs/api-reference/experimental/a-buffer-renderer.md) add transparent geometry capture and resolve pipelines.
