...
For one global compacted result across independently partitioned input and output vectors, use [`GPUHashJoin`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-hash-join.md). It preserves caller storage while allowing matches to fill the total output capacity across chunk boundaries.
