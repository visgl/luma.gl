...
Use a mask when downstream work benefits from source-aligned membership. Add [`GPUCompaction`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-compaction.md) or [`GPUVisibilityWorkflow`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-visibility-workflow.md) when the consumer instead needs a dense list and count. `GPUMask` only combines existing decisions—it does not evaluate geometric, temporal, or application-specific predicates itself.
