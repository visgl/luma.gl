...
Run [`GPUSceneDrawGeneration`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-scene-draw-generation.md) before group classification in the same command graph. A command with a zero instance count is inactive. An active command uses `firstInstance` to locate its scene row, then reads that row's group and geometry identities.
