...
<GPUCoreDocsTabs active="scene-resource-groups" />
...
<GPUOperationContract operation="gpu-scene-resource-groups" />
...
Run [`GPUSceneDrawGeneration`](/docs/api-reference/experimental/gpu-core/gpu-scene-draw-generation)
...
Group windows, resource identities, and binding order remain explicit application policy; the GPU
classifies active generated commands and reports every incompatible or unassigned case. The
contract is designed to support both conventional-scene and preserved-table consumers.
