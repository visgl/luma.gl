...
| **Reads / writes**       | Reads dense uint32 keys, optional mask, and values; writes one row per group.        |
| **Ownership**            | Inputs/output are caller-owned; mean-count scratch is graph-owned transient storage. |
| **Output contract**      | Fixed group rows; invalid keys are ignored and numeric edge cases are explicit.      |
| **Expected work**        | Output initialization plus accumulation per nonempty chunk; mean adds division.      |
...
**Common mistake**Do not confuse fixed group capacity with the number of populated groups.
...
On subgroup-capable devices, aggregations with at most 16 groups combine lanes carrying the same key before issuing count, sum, minimum, maximum, or mean atomics. This targets compact dictionaries with high contention. Larger dictionaries and devices without both subgroup capabilities retain the existing local-count or direct-global-atomic implementations.
