...
Use [`GPUGraphTraversal`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-graph-traversal.md) instead when the question is which nodes are reachable through arbitrary edges. Projection follows exactly one parent chain per row and finds a representative; it does not select a neighborhood or rewrite the graph.
