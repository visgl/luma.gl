...
| **Reads / writes**       | Resolves logical values into backend views and preserves operation resource intent. |
| **Ownership**            | The backend compiler owns its registry, capabilities, and lowering decisions.       |
| **Output contract**      | A command graph plus immutable value, provenance, and decision reports.             |
| **Expected work**        | Visits the bounded semantic tree and emits the selected command nodes.              |
| **Chunks**               | Lowerers must explicitly preserve or transform logical storage boundaries.          |
| **Conditions / budgets** | WebGPU lowering uses command nodes; another backend may choose native graph forms.  |
...
**Common mistake**Do not let semantic operations mutate a backend graph directly.
...
## Lowering registry[​](#lowering-registry "Direct link to Lowering registry")
...
## Backend capabilities[​](#backend-capabilities "Direct link to Backend capabilities")
...
## Execution primitives[​](#execution-primitives "Direct link to Execution primitives")

Semantic operations use registered backend lowerers. Concrete `GPUProgramPrimitive` objects return nodes through `getCommandNodes(graph)`; their decision is recorded as `explicit-command-nodes`. Construction may allocate graph-owned scratch but cannot schedule work. There is no legacy mutation fallback.

## Why record decisions?[​](#why-record-decisions "Direct link to Why record decisions?")
