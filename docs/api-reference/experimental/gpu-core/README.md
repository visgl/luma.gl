...
| Data movement | `GPUUint32Gather` selects or reorders packed uint32 rows; `GPUByteRangeGather` concatenates variable byte ranges from source offsets, lengths, and output offsets. Both contribute one bounded graph-native compute operation. |
