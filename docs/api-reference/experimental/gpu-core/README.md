...
- [Vector Field Lab](/examples/showcase/vector-field-lab) evaluates gradient, divergence, curl, and
  Laplacian contributors over sampled fields and renders their storage-buffer outputs directly.
...
| Sampled fields | `GPUFiniteDifference2D` for gradient, divergence, curl, and Laplacian evaluation with explicit spacing and boundary policy |
