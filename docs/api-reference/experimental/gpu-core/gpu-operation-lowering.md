...
Operation lowering preserves the relationship between semantic operations and the command nodes selected by a backend compiler.
...
| **Reads / writes**       | Records identities and paths without changing declared graph resource uses. |
| **Ownership**            | The compiler owns the immutable provenance report.                          |
| **Output contract**      | Operation paths paired with lowered command-node identifiers and types.     |
| **Expected work**        | One small provenance record per emitted semantic command node.              |
| **Chunks**               | Does not alter logical or physical data partitioning.                       |
| **Conditions / budgets** | Produced during lowering and consumed by inspectors and planners.           |
...
**Common mistake**Do not discard semantic identity once a backend command is selected.

## Why preserve lowering provenance?[​](#why-preserve-lowering-provenance "Direct link to Why preserve lowering provenance?")
...
## Why this is compiler infrastructure[​](#why-this-is-compiler-infrastructure "Direct link to Why this is compiler infrastructure")
...
## Dynamic control-flow boundary[​](#dynamic-control-flow-boundary "Direct link to Dynamic control-flow boundary")
