...
<GPUCoreDocsTabs active="histogram" />
...
<GPUOperationContract operation="gpu-histogram" />
...
measurements such as trace durations and request latency, where values may range from microseconds
to seconds. A linear histogram can collapse most observations into one short-duration bin while
leaving much of the long tail almost empty.
...
```ts
addGPUCommandNodes(graph, new GPUHistogram({input: values, output: counts, domain: 'auto'}).getCommandNodes(graph));

addGPUCommandNodes(graph, new GPUHistogram({
  input: durations,
