...
| **Reads / writes**       | Reads keys/values; writes a fixed-capacity open-addressed table and probe diagnostics.                        |
...
**Common mistake**Do not ignore duplicate-key policy or probe-limit diagnostics.
...
| [`GPUBatchHashIndex`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-batch-hash-index.md) | Several ordered right-side key chunks with batch metadata.     | Supply per-chunk generated row-ID bases or validity masks with matching topology.                          |
...
| [`GPUHashJoin`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-hash-join.md)              | One logical left-side sequence against either index type.      | Only matching left/right row pairs should be published, with an explicit output capacity.                  |
| [`GPUBatchHashJoin`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-batch-hash-join.md)   | Several preserved left-side chunks against either index type.  | Independent source batches need separate stable pairs, counts, capacities, and diagnostics.                |
