...
| [`GPUBatchHashIndex`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-batch-hash-index.md) | Several ordered right-side key chunks.                        | Streamed record batches must populate one shared index without repacking or losing source-row offsets. |
...
| [`GPUHashJoin`](https://luma.gl/docs/api-reference/experimental/gpu-core/gpu-hash-join.md)              | One left-side key batch against either index type.            | Only matching left/right row pairs should be published, with an explicit output capacity.              |
