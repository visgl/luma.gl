---
title: GPU Graph operations
description: Index of GPU Graph topology, traversal, connectivity, community, metrics, ranking, and layout operations.
---
...
<ExperimentalDocsTabs active="gpu-graph-operations" />
...
The [interactive GPU Graph explorer](/examples/experimental/gpu-graph-explorer) answers that question
...
### Use GPU Graph from deck.gl without copying graph buffers
...
The optional [GPU Graph + deck.gl network explorer](/examples/deck/gpu-graph-explorer) answers that
question with the reusable `GPUGraphDeckEffect`, `GPUGraphNodeLayer`, and `GPUGraphEdgeLayer`
...
readback have real costs. GPU Graph is most useful when graph data or downstream consumers are already
GPU-resident and several operations reuse the same topology.

## When should I use GPU Graph?

Use GPU Graph for browser applications that already own typed GPU relationship columns and need to
...
around high-degree hubs. GPU Graph operates on one browser WebGPU device and does not provide the
...
**local clustering coefficient (LCC)**, and **PageRank (PR)**. GPU Graph supplies an actual
...
| Family | Use it for |
| --- | --- |
| [Topology](/docs/api-reference/experimental/gpu-graph-topology) | Edge views, compressed adjacency, and degree counts. |
| [Traversal and pathfinding](/docs/api-reference/experimental/gpu-graph-traversal) | Unweighted neighborhoods and weighted routes. |
| [Connectivity and communities](/docs/api-reference/experimental/gpu-graph-connectivity) | Components, community discovery, optimization, and quality. |
| [Metrics and ranking](/docs/api-reference/experimental/gpu-graph-metrics) | Core numbers, clustering, and PageRank. |
| [Layouts](/docs/api-reference/experimental/gpu-graph-layouts) | Exact and accelerated progressive spatial layouts. |

## Attribution and licensing

GPU Graph is inspired by [NVIDIA RAPIDS cuGraph](https://github.com/rapidsai/cugraph) and the NVIDIA
...
GPU Graph is an independently written, [MIT-licensed](https://github.com/visgl/luma.gl/blob/master/LICENSE)
...
- [GPU Core overview](/docs/api-reference/experimental/gpu-core)
- [Graph explorer example](/examples/experimental/gpu-graph-explorer)
