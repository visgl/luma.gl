...
<ExperimentalDocsTabs active="gpu-raster-concepts" />
...
GPURaster addresses those problems inside an explicitly submitted WebGPU command graph. This guide
explains the vocabulary and decisions before the
[complete GPURaster API reference](/docs/api-reference/experimental/gpu-raster). The
...
GPURaster does not download GeoTIFFs, select a network transport, decode image formats, submit GPU
commands, or implicitly copy raster pixels back to JavaScript. The application keeps those
decisions. A GPURaster tile cache can explicitly own uploads, resident source buffers, and reusable
...
An affine transform and coordinate reference system identify where data already lives; GPURaster
...
GPURaster combines independent signals instead of assuming one special numeric value always means
missing:
...
valid. GPURaster's normalized difference vegetation index is:
...
geometric pixel count would invent a missing measurement. GPURaster therefore publishes separate
...
the fraction. GPURaster keeps the centroid local on the GPU and applies the retained raster
...
GPURaster uses **half-open bounds**: `[start, end)` includes `start` and excludes `end`.
...
GPURaster canonicalizes equivalent source requests before caching them. A default full window and
...
GPURaster gathers available neighboring source tiles on the GPU, runs the operation over the
...
documented strategy. GPURaster does not infer how that level was produced.
...
For GPURaster, **replay** means executing the analytical work for a tile again after another
...
percentile. GPURaster's global percentile uses the finalized histogram: minimum and maximum
...
GPU counters are finite. GPURaster saturates unsigned 32-bit counts at `4,294,967,295` instead of
...
GPURaster cache. Reusing a resident tile avoids repeating a source read and GPU upload.
...
## What belongs to GPURaster and what belongs elsewhere
...
GPURaster is a GPU analytical layer, not a replacement for loaders.gl. A future loaders.gl 5
integration can adapt decoded raster tiles into the GPURaster source contract while leaving
...
| Define or upload a single application-owned raster | Raster metadata and bands in the [storage and residency reference](/docs/api-reference/experimental/gpu-raster/operations-storage-residency#raster-bands-and-validity). |
...
| Look up complete constructors, format constraints, ownership, and code examples | The [GPURaster API reference](/docs/api-reference/experimental/gpu-raster). |
...
continue to the [GPURaster API reference](/docs/api-reference/experimental/gpu-raster).
