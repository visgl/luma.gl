---
title: GPURaster storage and residency
description: Raster validity, decoded tile sources, bounded residency, cancellation, halos, and owned tile cores.
---
...
contain reflectance, elevation, temperature, or a land-cover category. GPURaster keeps those
...
[GPURaster Concepts and Execution Model](/docs/api-reference/experimental/gpu-raster/concepts).
...
already exists in application-owned decoded data; GPURaster can select it, but its sampling
...
GPURaster does not ship a GeoTIFF/COG parser, HTTP range transport, reprojection system, or
...
Use GPURaster when the result must remain a scientifically meaningful GPU observation that a
...
performance; GPURaster does not promise that every analytical operation outruns a visual effect.
...
`createApplicationOwnedRasterViews` represents application code; it is not a GPURaster loader
...
not an implicit GPURaster loader. `GPURasterTileHaloFill` requires exact, nonoverlapping source
...
appearance. GPURaster retains native scientific samples, validity, calibration, explicit tile
...
- [GPURaster concepts](/docs/api-reference/experimental/gpu-raster/concepts)
- [GPURaster operations index](/docs/api-reference/experimental/gpu-raster/operations)
