...
<DeviceTabs />
<ShaderHooksExample showHeader={false} showStats={false} />
...
The default shader assembler is now maintained per shader language, so GLSL hook registrations
should use `ShaderAssembler.getDefaultShaderAssembler('glsl')` and WGSL hook registrations should
use `ShaderAssembler.getDefaultShaderAssembler('wgsl')`.
