...
[Testing](https://luma.gl/docs/developer-guide/testing.md)[Debugging](https://luma.gl/docs/developer-guide/debugging.md)[Profiling](https://luma.gl/docs/developer-guide/profiling.md)[Bundling](https://luma.gl/docs/developer-guide/bundling.md)[AI agents](https://luma.gl/docs/developer-guide/working-with-ai.md)

GPU programming is all about performance, so having tools to systematically measure the performance impact of code changes is critical. luma.gl offers several built-in facilities.

## probe.gl Stats[​](#probegl-stats "Direct link to probe.gl Stats")

`probe.gl` is a companion framework focused on instrumentation and logging of JavaScript applications. It provides a `Stats` class which can be thought of as a "bag" of different stats (or performance measurements), and luma.gl itself automatically populates `Stats` objects that can be inspected by the application.
...
## Memory Profiling[​](#memory-profiling "Direct link to Memory Profiling")

luma.gl automatically tracks GPU memory usage.

Note that JavaScript is a garbage collected language and while memory allocations can always be tracked, it is only possible for luma.gl to track GPU memory deallocations if they are performed through the luma.gl API (by calling the `.destroy()` methods on `Buffer` and `Texture` objects).

Apart from GPU memory tracking for luma.gl also maintain counts of the various other luma.gl API objects. Such object generally do not consume a lot of memory, however tracking allocations can help spot resource leaks or unnecessary work being done.

## Performance Profiling[​](#performance-profiling "Direct link to Performance Profiling")

[QuerySet](https://luma.gl/docs/api-reference/core/resources/query-set.md) can be used to capture GPU-side profiling data.
...
### Profiling With QuerySet[​](#profiling-with-queryset "Direct link to Profiling With QuerySet")
...
Most engines can also use automatic per-pass profiling by constructing a command encoder with the profiling query set:
