...
While usually not recommended, it is also possible to activate the developer tools directly. Import the optional registration entry, then call [`luma.createDevice`](https://luma.gl/docs/api-reference/core/luma.md#lumacreatedevice) with `debugWebGL: true`:
