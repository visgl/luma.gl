---
title: Rendering and visualization capabilities
description: Portable GPU, engine, shader, effects, glTF, scene, splat, simulation, and immersive rendering capabilities in luma.gl.
---

# Rendering and visualization capabilities

This matrix groups luma.gl's portable rendering, scene, asset, effects, and simulation capabilities.
Return to the [capabilities overview](/docs/capabilities) for GPU data and compute features.
...
| Meshopt compression | Available | WebGPU + WebGL2 | `@loaders.gl/gltf` | Decode supported `EXT_meshopt_compression` and `KHR_meshopt_compression` buffer views before luma.gl creates GPU geometry. |
...
Explore the [ANARI Playground](/examples/experimental/scene-playground), or read about
[retained animation](/docs/api-reference/scene/anari-animation) and
[switchable scene renderers](/docs/api-reference/scene/anari-rendering).
...
Read the [ANARI ray-tracing renderer](/docs/api-reference/scene/anari-rendering) and the
...
| Row-accurate RAD hierarchy | Experimental | WebGPU + WebGL2 | `@luma.gl/splats` | Retarget retained authored child-row trees across camera changes with best-first angular refinement, coherent visible detail, parent fallback, and sparse source-row identity. |
...
review the supported contracts in the [`@luma.gl/splats` reference](/docs/api-reference/splats).
