...
description: GPU tables, Arrow, dataframe, raster, query, and visualization capabilities in luma.gl.
---
...
The `@luma.gl/experimental/gpu-dataframe` entry point is experimental; its GPU-native
operations are implemented, but non-unique, outer, or multi-key joins, temporal windows, and
cross-batch global ordering remain opportunities. See the reusable higher-level APIs.
