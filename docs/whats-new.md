...
- **Arrow `Utf8View` text** - `ArrowTextRenderer` accepts plain and dictionary-encoded `Utf8View` columns from Apache Arrow 21.2 and later, detects them without importing version-specific Arrow symbols, and normalizes their inline and variadic buffers for existing text strategies while retaining Apache Arrow 17 compatibility.
