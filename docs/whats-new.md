...
- **[Automatic screen-space crowd LOD](/docs/api-reference/gltf/gltf-animated-crowd#screen-space-levels-of-detail)** - Resolve authored `MSFT_lod` meshes or generate safe index-only alternatives, classify independently animated actors by projected size, cull offscreen/tiny actors, and submit one instanced draw per occupied primitive/detail bucket on WebGPU and WebGL 2.
- **[Global animated-crowd vertex budgets](/docs/api-reference/gltf/gltf-animated-crowd#keep-visible-geometry-within-a-global-vertex-budget)** - Cap submitted index references across every visible actor and source primitive, preserve nearby character detail through deterministic farthest-first demotion, and expose explicit diagnostics when a requested budget cannot be satisfied without hiding actors.
...
- **[LuRaster GPU raster analytics](/docs/api-reference/experimental/luraster)** - The optional `@luma.gl/experimental/luraster` entry point validates application-owned raster tile sources, selected native-format bands, explicit source overviews/windows, canonical request identity, CRS/affine metadata, cancellation, CPU/GPU-budgeted tile residency with full retained backing-allocation accounting, deterministic LRU eviction, fence-safe leases, compatible compiled-graph reuse, seam-safe cumulative neighborhood halos with GPU-native neighbor assembly and half-open owned-core extraction, and generated nodata-aware weighted floating-point or exact categorical nearest/mode analytical overviews; it then composes calibrated NDVI and band math, nodata-aware histograms and scalar statistics, GPU-native contrast/gamma/histogram equalization, analytical thresholds, deterministic Otsu selection, explicit-border neighborhood stencils, direct convolution, separable Gaussian/box smoothing, signed Sobel/Scharr/Laplacian derivatives, gradient magnitude, binary/grayscale dilation, erosion, opening, and closing, and marching-squares contour geometry inside caller-owned command graphs. The [Satellite Raster Lab](/examples/showcase/raster-lab) visualizes full/west/east source windows, source-nearest versus GPU-generated nodata-aware mean overviews, categorical nearest/mode policies, bounded cache capacity and hit/eviction/pin/memory/graph-reuse diagnostics, explicit tile-only/seamless-halo modes, cumulative receptive-field and owned-core/neighbor diagnostics, live source-band selection, filter radius/sigma, directional edges, morphological mode/operation/footprint/radius/nodata/border controls, contrast, threshold masks, changing GPU-computed histograms, and GPU-driven indirect contour overlays without downloading raster pixels; generated-overview and seamless-halo demo modes are explicitly mutually exclusive.
