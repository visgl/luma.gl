...
- **Graph-native GPU data analysis** - `GPUReduction`, `GPUHistogram`, `GPUGridBinning`, and `GPUGridAggregation` add deterministic scalar aggregates, reusable histogram counts, row-major spatial counts, and weighted floating-point cell sums. Grid operations clear once and accumulate aligned fixed-width vector chunks without packing. The [GPU Data Analysis example](/examples/experimental/gpu-data-analysis) composes the operations without hidden submission or readback.
