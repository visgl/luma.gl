...
- **Graph-native GPU data analysis** - `GPUReduction`, `GPUHistogram`, `GPUGridBinning`, and `GPUGridAggregation` add deterministic scalar aggregates, reusable histogram counts, row-major spatial counts, and weighted floating-point sum/min/max/mean cell statistics. Grid operations initialize once and accumulate aligned fixed-width vector chunks without packing. The [GPU Data Analysis example](/examples/experimental/gpu-data-analysis) composes the operations without hidden submission or readback.
