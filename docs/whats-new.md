...
- **Pluggable graph-based ray tracing** - `ANARIDevice.registerRenderer()` registers lazy custom runtimes, while the WebGPU-only `raytrace` subtype adapts committed scenes to the shared experimental `RayTracingSceneRenderer` for analytic sphere/triangle intersections, direct-light shadows, progressive sampling, and HDR presentation.
...
- **Shared software ray-traced scene rendering** - `RayTracingSceneRenderer` consumes format-independent scene descriptors through a WebGPU command graph, traces analytic spheres and mesh triangles, accumulates primary-ray samples, supports direct shadow rays, and leaves command submission application-owned.
