...
*This page contains news for recent luma.gl releases. For older releases (through v8.5) refer to the [Legacy What's New](https://luma.gl/docs/legacy/legacy-whats-new) page.*
...
* **Next-generation 3D scenes** - Combine physical materials, deferred lighting, ray tracing, shadows, reflections, fog, and temporal effects. Explore the stack in [Visualization City](https://luma.gl/examples/experimental/advanced-effects).
* **Large GPU-driven worlds** - Move culling, selection, sorting, and indirect drawing off the CPU. [Virtual Geometry Canyon](https://luma.gl/examples/experimental/virtual-geometry-canyon) renders a potential 41 million triangles with GPU-selected detail.
* **Interactive scientific simulations** - Build oceans, fluids, fire, rasters, and volume tools on reusable GPU algorithms. See [Tempest Ocean](https://luma.gl/examples/showcase/tempest-ocean), [Volumetric Fire Forge](https://luma.gl/examples/experimental/volumetric-fire-forge), and the [Satellite Raster Lab](https://luma.gl/examples/showcase/raster-lab).
* **GPU-native analytical applications** - Connect dataframes, graph traversal, geospatial operations, trace analysis, and rendering as one GPU workflow. The [GPU Trace Viewer](https://luma.gl/examples/experimental/gpu-trace-viewer) demonstrates the approach at millions of spans.
...
* **GPU-native analysis through focused APIs** - Sorting, aggregation, [dataframes](https://luma.gl/docs/api-reference/experimental/gpu-dataframe.md), joins, graph traversal, geospatial operations, and trace analysis keep intermediate data on the GPU.
...
* **Record work once, replay it cheaply** - `RenderPass` now owns draw state and supports [WebGPU render bundles](https://luma.gl/examples/api/render-bundles), direct draws, and indirect draws.
...
* **Grow and stream GPU resources** - [`DynamicBuffer`](https://luma.gl/docs/api-reference/engine/dynamic-buffer.md) handles resizable model data. [`VideoTexture`](https://luma.gl/docs/api-reference/engine/video-texture.md) handles live video on WebGPU and WebGL.
