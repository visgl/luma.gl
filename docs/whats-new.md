...
- **[LuRaster GPU raster analytics](/docs/api-reference/experimental/luraster)** - The optional `@luma.gl/experimental/luraster` entry point validates application-owned raster tile sources, selected native-format bands, explicit source overviews/windows, canonical request identity, CRS/affine metadata, cancellation, CPU/GPU-budgeted tile residency with full retained backing-allocation accounting, deterministic LRU eviction, fence-safe leases, compatible compiled-graph reuse, seam-safe cumulative neighborhood halos with GPU-native neighbor assembly and half-open owned-core extraction, generated nodata-aware weighted floating-point or exact categorical nearest/mode analytical overviews, and replayable dataset-wide GPU extrema, stable-domain histogram merges, approximate percentiles, and explicit saturating/sticky overflow diagnostics; it then composes calibrated NDVI and band math, nodata-aware histograms and scalar statistics, GPU-native contrast/gamma/histogram equalization, analytical thresholds, deterministic Otsu selection, explicit-border neighborhood stencils, direct convolution, separable Gaussian/box smoothing, signed Sobel/Scharr/Laplacian derivatives, gradient magnitude, binary/grayscale dilation, erosion, opening, and closing, and marching-squares contour geometry inside caller-owned command graphs. The [Satellite Raster Lab](/examples/showcase/raster-lab) visualizes full/west/east source windows, source-nearest versus GPU-generated nodata-aware mean overviews, categorical nearest/mode policies, bounded cache capacity and hit/eviction/pin/memory/graph-reuse diagnostics, explicit tile-only/seamless-halo modes, TILE/FULL GLOBAL analytical scope, reversible west/east replay, global domain/count/histogram/median/Otsu diagnostics, cumulative receptive-field and owned-core/neighbor diagnostics, live source-band selection, filter radius/sigma, directional edges, morphological mode/operation/footprint/radius/nodata/border controls, contrast, threshold masks, changing GPU-computed histograms, and GPU-driven indirect contour overlays without downloading raster pixels. Global replay uses source-nearest pointwise analysis and explicitly exits incompatible generated-overview, seamless-halo, and neighborhood-effect modes; global median and Otsu reuse the existing compact scalar slot.
