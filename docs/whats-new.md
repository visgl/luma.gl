...
Version 9.4 keeps the stable core familiar while opening a powerful experimental stack for GPU data, analysis, text, splats, and scenes.

**Highlights**

* **Data that goes straight to pixels** - Typed, chunked columns can move from Arrow into analysis, text, paths, and rendering without a CPU-side object layer.
...
* **Five new experimental modules** - `@luma.gl/arrow`, `@luma.gl/text`, `@luma.gl/splats`, `@luma.gl/scene`, and `@luma.gl/experimental` are now published for direct use. These APIs may evolve outside the 9.4 semver contract.

**A clearer GPU data stack**

* **Primitive GPU data has a dedicated home** - `GPUData`, `GPUVector`, views, formats, and layout helpers live in `@luma.gl/gpgpu/gpu-data`, independent of Arrow and rendering models.
...
* **Adapters preserve what matters** - `@luma.gl/arrow` keeps source batches and type metadata. `@luma.gl/text` supports streamed text and shared font resources.
