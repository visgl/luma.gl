...
| Cross-tile connected-region identities | Experimental | WebGPU | `@luma.gl/experimental/luraster` | Reconcile four/eight-connected owned-core boundaries and diagonal tile junctions into globally row-major dense IDs independent of tile arrival order. |
| Bounded globally merged region metrics | Experimental | WebGPU | `@luma.gl/experimental/luraster` | Merge distinct geometry/intensity populations, calibrated extrema, weighted sums, translated spatial moments, and affine areas with explicit convergence, capacity, and overflow. |
